...
| **Agents** | Build multi-step automations with the visual agent editor. Add loops, conditionals, and data passing between steps. |
| **Runs** | Execution history for every task and agent. Filter by status, drill into any run to see actions, recordings, and extracted data. |
...
| **Credentials** | Store website logins, credit card details, and secret values securely. Skyvern uses these to authenticate automatically, fill payment forms, and inject secrets into agents. |
