...
4. You will then need to create a new workflow or "recipe"
   ...
   i. If you are a returning user, you will configure setup, location, and starting point

   ii. If you are a new user, you will configure recipe and project name, location, project description, and starting point

5. Once you've created your new workflow, set the trigger, add an "action in app" and search for Skyvern

   i. If Skyvern is not showing up in this search, it is likely that there was a misstep in tasks 1-3
...
1. Select what workflow you would like to run
   ...
   i. If there are no selections, make sure to check your Skyvern account and see if you have any saved workflows you can run
...
2. Once you've selected the workflow you'd like to run, the only thing you need to configure are whatever parameters you have linked in your workflow.
   ...
   i. If you don't have parameters, and instead have fixed values, don't worry about this step!
...
2. Depending on whether you selected a "Get a Task" or "Get a Workflow" action, find the appropriate ID
   ...
   i. The Workflow Run ID starts with a WR and the Task ID starts with a TSK
