...
from testcharmvision.constants import PAGE_CONTENT_TIMEOUT, TESTCHARMVISION_DIR
from testcharmvision.exceptions import FailedToTakeScreenshot
...
def load_js_script() -> str:
    ...
    path = f"{TESTCHARMVISION_DIR}/webeye/scraper/domUtils.js"
    try:
...
async def _scrolling_screenshots_helper(
    ...
    testcharmvision_page = await TestcharmvisionFrame.create_instance(frame=page)
    frame = "main.frame"
    frame_index = 0
    ...
    positions: list[int] = []
    if await testcharmvision_page.is_window_scrollable():
        scroll_y_px_old = -30.0
        _, initial_scroll_height = await testcharmvision_page.get_scroll_width_and_height()
        scroll_y_px = await testcharmvision_page.scroll_to_top(draw_boxes=draw_boxes, frame=frame, frame_index=frame_index)
        ...
        while abs(scroll_y_px_old - scroll_y_px) > 25 and len(screenshots) < max_number:
            ...
            _, scroll_height = await testcharmvision_page.get_scroll_width_and_height()
            if scroll_height != initial_scroll_height:
                ...
                await testcharmvision_page.build_tree_from_body(frame_name=frame, frame_index=frame_index)
            ...
            scroll_y_px = await testcharmvision_page.scroll_to_next_page(
        ...
        if draw_boxes:
            await testcharmvision_page.remove_bounding_boxes()
        await testcharmvision_page.scroll_to_top(draw_boxes=False, frame=frame, frame_index=frame_index)

        if mode == ScreenshotMode.DETAILED:
            # wait until animation ends, which is triggered by scrolling
            await testcharmvision_page.safe_wait_for_animation_end()
    else:
        if draw_boxes:
            await testcharmvision_page.build_elements_and_draw_bounding_boxes(frame=frame, frame_index=frame_index)
...
class TestcharmvisionFrame:
    ...
    async def evaluate(
        ...
        try:
            ...
            LOG.exception("Testcharmvision timed out trying to analyze the page", expression=expression)
            raise TimeoutError("Testcharmvision timed out trying to analyze the page")

    @staticmethod
    async def get_url(frame: Page | Frame) -> str:
        return await TestcharmvisionFrame.evaluate(frame=frame, expression="() => document.location.href")
    ...
    @classmethod
    async def create_instance(cls, frame: Page | Frame) -> TestcharmvisionFrame:
