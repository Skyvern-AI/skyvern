...
from testcharmvision.constants import BROWSER_DOWNLOAD_TIMEOUT, BROWSER_DOWNLOADING_SUFFIX, REPO_ROOT_DIR
from testcharmvision.exceptions import DownloadFileMaxSizeExceeded, DownloadFileMaxWaitingTime
from testcharmvision.forge import app
from testcharmvision.forge.sdk.api.aws import AsyncAWSClient
from testcharmvision.utils.url_validators import encode_url
...
def validate_download_url(url: str) -> bool:
    ...
    try:
        ...
        # Allow S3 URIs for Testcharmvision uploads bucket
        if scheme == "s3":
            if url.startswith(f"s3://{settings.AWS_S3_BUCKET_UPLOADS}/{settings.ENV}/o_"):
                return True
            return False

        # Allow Azure URIs for Testcharmvision uploads container
        if scheme == "azure":
    ...
    try:
        ...
        if parsed.scheme == "s3":
            ...
            if url.startswith(uploads_prefix):
                LOG.info("Downloading Testcharmvision file from S3", url=url)
            ...
            if url.startswith(uploads_prefix):
                LOG.info("Downloading Testcharmvision file from Azure Blob Storage", url=url)
                data = await app.STORAGE.download_uploaded_file(url)
                if data is None:
        ...
        # and the file is in the testcharmvision downloads directory
        ...
        async with aiohttp.ClientSession(raise_for_status=True) as session:
            ...
            async with session.get(URL(encoded_url, encoded=True), headers=headers) as response:
                ...
                if output_dir:
                    ...
                    download_dir = make_temp_directory(prefix="testcharmvision_downloads_")
...
def get_testcharmvision_temp_dir() -> str:
...
def clean_up_testcharmvision_temp_dir() -> None:
    return clean_up_dir(get_testcharmvision_temp_dir())
