...
from testcharmvision.schemas.webhooks import RunWebhookPreviewResponse, RunWebhookReplayResponse
from testcharmvision.services import run_service, task_v2_service
...
def build_sample_task_payload(run_id: str | None = None) -> dict:
    ...
    task_run_response = TaskRunResponse(
        ...
        started_at=now,
        finished_at=now,
        app_url=f"https://app.testcharmvision.com/tasks/{task_id}",
...
def build_sample_workflow_run_payload(run_id: str | None = None) -> dict:
    ...
    app_url = f"{settings.TESTCHARMVISION_APP_URL.rstrip('/')}/runs/{workflow_run_id}"
...
async def build_run_preview(organization_id: str, run_id: str) -> RunWebhookPreviewResponse:
    ...
    signed_data = generate_testcharmvision_webhook_signature(payload=payload.payload, api_key=api_key)
...
async def replay_run_webhook(
    ...
    signed_data = generate_testcharmvision_webhook_signature(payload=payload.payload, api_key=signing_key)
...
async def _build_webhook_payload(organization_id: str, run_id: str) -> _WebhookPayload:
    ...
    if not run:
        ...
        raise TestcharmvisionHTTPException(
...
def _validate_target_url(url: str) -> str:
    try:
        ...
        if not validated_url:
            raise TestcharmvisionHTTPException("Invalid webhook URL.", status_code=status.HTTP_400_BAD_REQUEST)
    ...
    except TestcharmvisionHTTPException:
        raise
    except Exception as exc:  # pragma: no cover - defensive guard
