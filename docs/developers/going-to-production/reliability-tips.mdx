...
Agents can silently produce wrong results. The AI might fill a form with incorrect data, navigate to the wrong page, or extract stale information. Validation blocks let you assert conditions at critical points and fail fast when something goes wrong, rather than discovering the problem downstream.
...
Use a For Loop when you have a known list of items before the loop begins. Use a While Loop when the agent should keep running until page state changes, such as paginating while a Next button is enabled or polling until a status changes. While loops need an especially clear exit condition because the number of iterations is discovered at run time.
...
For While Loop pagination, each pass should leave the page ready for the next condition check. Extract the state that controls the loop, then advance the page:
...
- block_type: while_loop
  ...
  loop_blocks:
    - block_type: extraction
      label: extract_page
      data_extraction_goal: Extract rows on this page and whether the Next button is enabled
      data_schema:
        type: object
        properties:
          rows:
    ...
    - block_type: action
      label: click_next
      navigation_goal: "Click Next if it is enabled. COMPLETE after one click or if Next is disabled."
```

The first condition check uses `current_index == 0` to enter the loop before `extract_page` exists. Later checks use the previous extraction output so the loop exits naturally when no next page is available.
...
Browser sessions have a default timeout. If you're using human-in-the-loop agents where a person must approve an action before proceeding, the session can expire while waiting for approval, especially if the approver is slow or in a different timezone.

**Symptoms:** Runs that worked during testing fail in production with session expiry errors. The agent waits for human approval, but by the time approval comes, the browser session has timed out.

**Solutions:**

1. **Increase session timeout**: Set a longer `timeout` when creating browser sessions for agents that include human approval steps
2. **Remove the human step for time-sensitive flows**: If session expiry is causing failures, consider making the agent fully automated and reviewing results after completion
3. **Split into multiple agents**: Run the pre-approval steps, wait for human approval outside Skyvern, then trigger a second agent for post-approval steps using a browser profile to maintain login state
...
<Note>
For agents with predictable human approval times, schedule runs to arrive in the approver's inbox during working hours when they can act quickly.
</Note>
...
| **Session expired** | Human-in-the-loop timeout | Increase timeout or split into multiple agents |
