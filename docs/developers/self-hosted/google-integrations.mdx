...
Generate the key with `openssl rand -base64 32`. Skyvern refuses to start if `ENABLE_ENCRYPTION=true` and the key is empty or still the `fillmein` placeholder. `ENCRYPTOR_AES_SALT` and `ENCRYPTOR_AES_IV` are optional; when unset they are derived from `ENCRYPTOR_AES_SECRET_KEY`. Set them only if you want to pin them explicitly, and never change them afterwards — rotating any of the three values makes previously stored credentials undecryptable.
...
| `ENCRYPTOR_AES_SECRET_KEY` | Yes | AES key used to encrypt stored credentials. Empty or `fillmein` stops the backend from starting. |
| `ENCRYPTOR_AES_SALT` / `ENCRYPTOR_AES_IV` | No | Derived from `ENCRYPTOR_AES_SECRET_KEY` when unset. Changing them after credentials are stored makes those credentials undecryptable. |
