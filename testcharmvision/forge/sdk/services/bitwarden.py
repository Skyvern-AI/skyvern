...
from testcharmvision.forge.sdk.core.aiohttp_helper import aiohttp_delete, aiohttp_get_json, aiohttp_post
...
from testcharmvision.forge.sdk.services.credentials import parse_totp_secret
from testcharmvision.utils.strings import is_uuid
...
class BitwardenConstants(StrEnum):
    ...
    TESTCHARMVISION_AUTH_BITWARDEN_ORGANIZATION_ID = "TESTCHARMVISION_AUTH_BITWARDEN_ORGANIZATION_ID"
    TESTCHARMVISION_AUTH_BITWARDEN_MASTER_PASSWORD = "TESTCHARMVISION_AUTH_BITWARDEN_MASTER_PASSWORD"
    TESTCHARMVISION_AUTH_BITWARDEN_CLIENT_ID = "TESTCHARMVISION_AUTH_BITWARDEN_CLIENT_ID"
    TESTCHARMVISION_AUTH_BITWARDEN_CLIENT_SECRET = "TESTCHARMVISION_AUTH_BITWARDEN_CLIENT_SECRET"
...
class BitwardenService:
    ...
    async def create_credential_item(
        ...
        try:
            master_password, bw_organization_id, _, _ = await BitwardenService._get_testcharmvision_auth_secrets()
    ...
    @staticmethod
    async def _get_testcharmvision_auth_master_password() -> str:
        master_password = settings.TESTCHARMVISION_AUTH_BITWARDEN_MASTER_PASSWORD
        if not master_password:
            master_password = await aws_client.get_secret(BitwardenConstants.TESTCHARMVISION_AUTH_BITWARDEN_MASTER_PASSWORD)
        if not master_password:
            raise BitwardenSecretError("Testcharmvision auth master password is not set")
        return master_password

    @staticmethod
    async def _get_testcharmvision_auth_organization_id() -> str:
        bw_organization_id = settings.TESTCHARMVISION_AUTH_BITWARDEN_ORGANIZATION_ID
        if not bw_organization_id:
            bw_organization_id = await aws_client.get_secret(BitwardenConstants.TESTCHARMVISION_AUTH_BITWARDEN_ORGANIZATION_ID)
        if not bw_organization_id:
            raise BitwardenSecretError("Testcharmvision auth organization ID is not set")
        return bw_organization_id

    @staticmethod
    async def _get_testcharmvision_auth_client_id() -> str:
        client_id = settings.TESTCHARMVISION_AUTH_BITWARDEN_CLIENT_ID
        if not client_id:
            client_id = await aws_client.get_secret(BitwardenConstants.TESTCHARMVISION_AUTH_BITWARDEN_CLIENT_ID)
        if not client_id:
            raise BitwardenSecretError("Testcharmvision auth client ID is not set")
        return client_id

    @staticmethod
    async def _get_testcharmvision_auth_client_secret() -> str:
        client_secret = settings.TESTCHARMVISION_AUTH_BITWARDEN_CLIENT_SECRET
        if not client_secret:
            client_secret = await aws_client.get_secret(BitwardenConstants.TESTCHARMVISION_AUTH_BITWARDEN_CLIENT_SECRET)
        if not client_secret:
            raise BitwardenSecretError("Testcharmvision auth client secret is not set")
    ...
    @staticmethod
    async def _get_testcharmvision_auth_secrets() -> Tuple[str, str, str, str]:
        master_password, bw_organization_id, client_id, client_secret = await asyncio.gather(
            BitwardenService._get_testcharmvision_auth_master_password(),
            BitwardenService._get_testcharmvision_auth_organization_id(),
            BitwardenService._get_testcharmvision_auth_client_id(),
            BitwardenService._get_testcharmvision_auth_client_secret(),
        )
    ...
    async def get_items_by_item_ids(
        ...
        try:
            master_password, _, _, _ = await BitwardenService._get_testcharmvision_auth_secrets()
            await BitwardenService._unlock_using_server(master_password)
