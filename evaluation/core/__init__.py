...
class Evaluator:
    ...
    def _wait_for_task_finish(self, task_id: str) -> None:
        while True:
    ...
    async def create_and_eval_skyvern_task(
        ...
        task_id = self.client.create_task(task_request_body=task, max_steps=max_step)
        self._save_artifact("task_id.txt", task_id.encode())
        self._wait_for_task_finish(task_id=task_id)
        ...
        time.sleep(30)
        await self.eval_skyvern_task(task_id=task, question=question, answer=answer)
