...
| `engine` | `RunEngine` | No | `skyvern_v1` / `"skyvern-1.0"` | AI engine. Python enum options: `skyvern_v1`, `skyvern_v2`, `openai_cua`, `anthropic_cua`, `ui_tars`. TypeScript uses the wire values: `"skyvern-1.0"`, `"skyvern-2.0"`, `"openai-cua"`, `"anthropic-cua"`, `"ui-tars"`. |
...
| `file_ids` | `list[str]` | No | `None` | IDs of files uploaded through `POST /v1/upload_file` to attach to this run (up to 50). Reference an attached file by its ID anywhere a file URL is accepted (for example in the prompt) and no presigned URL is created for it. Skyvern deletes an attached file when the run finishes. A file can be attached to one run at a time and must not already be deleted. |
