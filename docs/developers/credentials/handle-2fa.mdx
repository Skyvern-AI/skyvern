  ...
  <Step title="Create a Zapier Zap">
    ...
    <Frame>
      <video
        ...
        src="https://cdn.jsdelivr.net/gh/Skyvern-AI/skyvern-docs-media@383c5fb7f1c10a89e57a34e503b05b7ee4e80a6a/media/authentication/39a279a5540ad3aeed3717a9fb307c4fb2d0eac61a462b91320b44f54a4bd60b-zapier-forwarding-setup.mp4"
      ></video>
    </Frame>
...
Skyvern polls this endpoint every 10 seconds for up to 15 minutes, so it will call you many
times before a code arrives. While you are still waiting for one, answer `HTTP 200` with an
empty value rather than a `204`, `404`, or `5xx`:
...
<Warning>
Any response that is not `HTTP 200` with a JSON object is treated as a contract violation, not
as "no code yet". If every poll in a run gets one, the run fails with an `OTP_TIMEOUT` whose
`failure_reason` reports the status codes Skyvern actually received — check that field first
when a run fails at the 2FA step.
</Warning>
...
For magic link flows, structure your agent in two parts:
...
4. Reference the item in your Skyvern agent

**TOTP stored in a plain field**

Some 1Password items keep the OTP secret in a plain custom field (for example, one named "digits") instead of a native one-time password field. Skyvern only auto-detects native one-time password fields. To use a plain field, tell Skyvern which field holds the TOTP secret:

- **Login API**: Pass `onepassword_totp_field_name` in the request to `POST /v1/run/login`. Set it to the field's id or label.
- **Workflows**: Set `totp_field_name` on the 1Password credential parameter, or fill in the **1Password TOTP Field** input when editing the credential parameter in the agent editor.

Leave the field unset to keep 1Password's own field-type detection.
