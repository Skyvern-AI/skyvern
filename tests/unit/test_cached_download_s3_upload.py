...
@pytest.mark.asyncio
async def test_cached_download_calls_save_downloaded_files(setup):
    """save_downloaded_files must be called so get_downloaded_files can find the file."""
    refs = setup(get_side_effect=[[], ["file1.pdf"]])
    try:
...
async def test_rename_happens_before_s3_upload(setup, tmp_path):
    ...
    refs = setup(
        ...
        list_files_side_effect=[[], [fake_file]],
...
async def test_download_suffix_rename_uses_file_path_directly(setup, tmp_path):
    ...
    refs = setup(
        get_side_effect=[[], ["invoice.pdf"]],
        list_files_side_effect=[[], [abs_path]],
        rename_mock=rename_mock,
    )
    try:
        from skyvern.services.script_service import download
...
async def test_verification_raises_when_no_new_file(setup):
    """When get_downloaded_files shows no increase, the cached path should raise
    and fall back to AI."""
    refs = setup(get_side_effect=[[], [], [], []])
    try:
        from skyvern.services.script_service import download
        ...
        assert "Files before: 0, after: 0" in str(error_arg)
    finally:
        _cleanup(refs)


@pytest.mark.asyncio
async def test_verification_retries_before_failing(setup):
    """get_downloaded_files retries up to 3 times before declaring failure."""
    # Before: 0 files. After: [], [], ["file.pdf"] (succeeds on 3rd attempt)
    refs = setup(get_side_effect=[[], [], [], ["file.pdf"]])
    try:
        from skyvern.services.script_service import download
...
@pytest.mark.asyncio
async def test_save_timeout_skips_verification(setup):
    """TimeoutError on save_downloaded_files should skip verification entirely.
    ...
    try:
        ...
        # Verification skipped → block completes, no AI fallback
        refs["fallback"].assert_not_called()
...
@pytest.mark.asyncio
async def test_save_generic_exception_skips_verification(setup):
    """Non-timeout S3 failure (e.g., permission error) should also skip verification.
...
@pytest.mark.asyncio
async def test_get_before_timeout_skips_verification(setup):
    """If the before-check times out, verification should be skipped entirely
    to avoid spurious AI fallbacks under degraded storage."""
    refs = setup(get_side_effect=asyncio.TimeoutError())
    try:
        from skyvern.services.script_service import download

        await download(prompt="Download invoice", label="test_block")

        # Should NOT fall back — verification was skipped
        refs["fallback"].assert_not_called()
    finally:
        _cleanup(refs)
...
@pytest.mark.asyncio
async def test_get_after_timeout_skips_verification(setup):
    """If the after-check times out, verification should be skipped."""
    call_count = {"n": 0}
    ...
    refs = setup(get_side_effect=get_side_effect)
    try:
        from skyvern.services.script_service import download
...
@pytest.mark.asyncio
async def test_no_rename_without_download_suffix(setup):
    """When download_suffix is not provided, rename should not be called."""
    rename_mock = MagicMock()
    refs = setup(get_side_effect=[[], ["file.pdf"]], rename_mock=rename_mock)
    try:
        from skyvern.services.script_service import download
...
async def test_rename_skips_crdownload_files(setup, tmp_path):
    ...
    refs = setup(
        get_side_effect=[[], ["invoice.pdf"]],
        list_files_side_effect=[[], [incomplete, complete]],
        rename_mock=rename_mock,
    )
    try:
        from skyvern.services.script_service import download
...
async def test_rename_handles_name_collision(setup, tmp_path):
    ...
    refs = setup(
        ...
        list_files_side_effect=[[], [new_file]],
        rename_mock=rename_mock,
    )
    try:
        from skyvern.services.script_service import download
...
@pytest.mark.asyncio
async def test_block_marked_completed_on_success(setup):
