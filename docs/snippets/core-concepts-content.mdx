...
Skyvern supports multiple AI engines. `skyvern-1.0` is the default for the Cloud UI and SDK. `skyvern-2.0` remains available for existing V2 workflows and explicitly requested V2 task runs. Other options include `openai-cua` (OpenAI Computer Use Agent), `anthropic-cua` (Anthropic Computer Use Agent), and `ui-tars`.
