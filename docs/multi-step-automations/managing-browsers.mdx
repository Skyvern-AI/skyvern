...
slug: multi-step-automations/managing-browsers
keywords:
...
Every browser automation starts with a browser. Skyvern gives you a cloud Chromium instance with a Playwright context - cookies, localStorage, and auth state persist across every page you open inside it.
...
|-----------|------|---------|-------------|
| `timeout` | `int` | `60` | Session timeout in minutes (5-1440) |
...
Returns the most recent page. If no pages are open, creates a new one.
...
| Value | Region |
|-------|--------|
| `RESIDENTIAL` | Default residential proxy |
| `RESIDENTIAL_US` | United States |
