...
from testcharmvision.constants import TESTCHARMVISION_ID_ATTR
from testcharmvision.exceptions import DisabledBlockExecutionError, StepUnableToExecuteError, TaskAlreadyTimeout
from testcharmvision.forge import app
from testcharmvision.forge.async_operations import AsyncOperation
...
from testcharmvision.forge.sdk.schemas.tasks import Task, TaskStatus
from testcharmvision.forge.sdk.trace import traced
from testcharmvision.forge.sdk.workflow.models.block import BlockTypeVar
from testcharmvision.webeye.actions.actions import Action
from testcharmvision.webeye.browser_state import BrowserState
from testcharmvision.webeye.scraper.scraped_page import ELEMENT_NODE_ATTRIBUTES, CleanupElementTreeFunc, json_to_html
from testcharmvision.webeye.utils.dom import TestcharmvisionElement
from testcharmvision.webeye.utils.page import TestcharmvisionFrame

LOG = structlog.get_logger()

_LLM_CALL_TIMEOUT_SECONDS = 30  # 30s
USELESS_SHAPE_ATTRIBUTE = [TESTCHARMVISION_ID_ATTR, "id", "aria-describedby"]
...
def _get_svg_cache_key(hash: str) -> str:
    return f"testcharmvision:svg:{hash}"


def _get_shape_cache_key(hash: str) -> str:
    return f"testcharmvision:shape:{hash}"


def _remove_testcharmvision_attributes(element: Dict) -> Dict:
    """
    To get the original HTML element without testcharmvision attributes
    """
    ...
    for child in children:
        trimmed_children.append(_remove_testcharmvision_attributes(child))
...
async def _check_svg_eligibility(
    ...
    try:
        locater = testcharmvision_frame.get_frame().locator(f'[{TESTCHARMVISION_ID_ATTR}="{element_id}"]')
        ...
        testcharmvision_element = TestcharmvisionElement(locator=locater, frame=testcharmvision_frame.get_frame(), static_element=element)

        _, blocked = await testcharmvision_frame.get_blocking_element_id(
            await testcharmvision_element.get_element_handler(timeout=1000)
        )
        if not testcharmvision_element.is_interactable() and blocked:
            _mark_element_as_dropped(element, hashed_key=None)
            return False
    except Exception:
        LOG.warning(
...
async def _convert_svg_to_string(
    ...
    svg_element = _remove_testcharmvision_attributes(element)
...
async def _convert_css_shape_to_string(
    ...
    shape_element = _remove_testcharmvision_attributes(element)
    ...
    if css_shape:
        ...
        try:
            ...
            _, blocked = await testcharmvision_frame.get_blocking_element_id(await testcharmvision_element.get_element_handler())
            if blocked:
                LOG.debug(
...
class AgentFunction:
    ...
    def cleanup_element_tree_factory(
        ...
        async def cleanup_element_tree_func(frame: Page | Frame, url: str, element_tree: list[dict]) -> list[dict]:
            ...
            testcharmvision_frame = await TestcharmvisionFrame.create_instance(frame=frame)
            ...
            while queue:
                ...
                if queue_ele.get("frame_index") != current_frame_index:
                    ...
                    testcharmvision_frame = await TestcharmvisionFrame.create_instance(frame=new_frame)
                ...
                if await _check_svg_eligibility(testcharmvision_frame, queue_ele, task, step, always_drop=disable_conversion):
                    eligible_svgs.append((queue_ele, testcharmvision_frame))
            ...
            if eligible_svgs and task and step:
                ...
                current_context = testcharmvision_context.current()
