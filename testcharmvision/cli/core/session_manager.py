...
@dataclass
class SessionState:
    browser: TestcharmvisionBrowser | None = None
...
async def resolve_browser(
    ...
    headless: bool = False,
) -> tuple[TestcharmvisionBrowser, BrowserContext]:
    ...
    Cleanup is done via explicit testcharmvision_session_close() call. For scripts that need
    guaranteed cleanup, use the browser_session() context manager instead.
    """
    testcharmvision = get_testcharmvision()
    current = get_current_session()

    browser: TestcharmvisionBrowser | None = None
    try:
        if session_id:
            browser = await testcharmvision.connect_to_cloud_browser_session(session_id)
            ctx = BrowserContext(mode="cloud_session", session_id=session_id)
            set_current_session(SessionState(browser=browser, context=ctx))
            return browser, ctx

        if cdp_url:
            browser = await testcharmvision.connect_to_browser_over_cdp(cdp_url)
            ctx = BrowserContext(mode="cdp", cdp_url=cdp_url)
            set_current_session(SessionState(browser=browser, context=ctx))
            return browser, ctx

        if local:
            browser = await testcharmvision.launch_local_browser(headless=headless)
            ctx = BrowserContext(mode="local")
            set_current_session(SessionState(browser=browser, context=ctx))
            return browser, ctx

        if create_session:
            browser = await testcharmvision.launch_cloud_browser(timeout=timeout)
...
async def get_page(
    session_id: str | None = None,
    cdp_url: str | None = None,
) -> tuple[TestcharmvisionBrowserPage, BrowserContext]:
...
async def browser_session(
    ...
    timeout: int | None = None,
    headless: bool = False,
) -> AsyncIterator[tuple[TestcharmvisionBrowser, BrowserContext]]:
...
def no_browser_error() -> dict[str, Any]:
    return make_error(
        ...
        "Create a session with testcharmvision_session_create, provide session_id, or cdp_url",
    )
