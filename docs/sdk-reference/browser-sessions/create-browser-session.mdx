...
|-----------|------|----------|---------|-------------|
| `timeout` | `int` | No | `60` | Session timeout in minutes (min 5, max 240; higher values are capped at 240 with a warning). A running session can be extended to 360 with [`extend_browser_session`](/sdk-reference/browser-sessions/extend-browser-session). Timer starts after the session is ready. |
...
| `needs_live_view` | `bool` | No | `False` | Declare that a person will watch this session's browser live. Defaults to `False`, which suits unattended automation. Requests a capability (the session runs somewhere that can serve a live view stream) — it does not select where the session runs. The Skyvern app sets this because sessions opened in the UI are watched. |
