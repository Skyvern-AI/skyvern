...
from testcharmvision.constants import DEFAULT_MAX_TOKENS, TESTCHARMVISION_DIR, TESTCHARMVISION_ID_ATTR
...
from testcharmvision.experimentation.wait_utils import empty_page_retry_wait
from testcharmvision.forge.sdk.api.crypto import calculate_sha256
...
def clean_element_before_hashing(element: dict) -> dict:
    def clean_nested(element: dict) -> dict:
        ...
        if "attributes" in element:
            attributes_cleaned = {key: value for key, value in element["attributes"].items() if key != TESTCHARMVISION_ID_ATTR}
...
def build_element_dict(
    ...
    for element in elements:
        ...
        # get_interactable_element_tree marks each interactable element with a TESTCHARMVISION_ID_ATTR attribute
        id_to_css_dict[element_id] = f"[{TESTCHARMVISION_ID_ATTR}='{element_id}']"
...
async def get_frame_text(iframe: Frame) -> str:
    ...
    try:
        text = await TestcharmvisionFrame.evaluate(frame=iframe, expression=js_script)
        if text is None:
            text = ""
    except Exception:
        LOG.warning(
...
async def scrape_web_unsafe(
    ...
    testcharmvision_frame = await TestcharmvisionFrame.create_instance(page)
    ...
    if take_screenshots:
        element_tree_trimmed_html_str = "".join(
            json_to_html(element, need_testcharmvision_attrs=False) for element in element_tree_trimmed
        )
        ...
        screenshots = await TestcharmvisionFrame.take_split_screenshots(
            page=page,
            url=url,
            draw_boxes=draw_boxes,
...
async def add_frame_interactable_elements(
    ...
    try:
        ...
        testcharmvision_id = await frame_element.get_attribute(TESTCHARMVISION_ID_ATTR)
        if not testcharmvision_id:
            LOG.info(
                "No Testcharmvision id found for frame, skipping",
                frame_index=frame_index,
                attr=TESTCHARMVISION_ID_ATTR,
            )
            return elements, element_tree
    except Exception:
        LOG.warning(
            "Unable to get Testcharmvision id from frame_element",
            attr=TESTCHARMVISION_ID_ATTR,
            exc_info=True,
        )
        return elements, element_tree
    ...
    try:
        testcharmvision_frame = await TestcharmvisionFrame.create_instance(frame)
        await testcharmvision_frame.safe_wait_for_animation_end()

        frame_elements, frame_element_tree = await testcharmvision_frame.build_tree_from_body(
            frame_name=testcharmvision_id, frame_index=frame_index, must_included_tags=must_included_tags
        )

        for element in elements:
            if element["id"] == testcharmvision_id:
                element["children"] = frame_element_tree

        elements = elements + frame_elements
    except Exception:
        LOG.warning("Failed to build the tree of the frame, skipping frame", frame_id=testcharmvision_id, exc_info=True)
...
async def get_interactable_element_tree(
    ...
    testcharmvision_page = await TestcharmvisionFrame.create_instance(page)
    elements, element_tree = await testcharmvision_page.build_tree_from_body(
...
class IncrementalScrapePage(ElementTreeBuilder):
    ...
    async def get_incremental_element_tree(
        ...
        frame = self.testcharmvision_frame.get_frame()

        try:
            incremental_elements, incremental_tree = await self.testcharmvision_frame.get_incremental_element_tree(
        ...
        # we listen the incremental elements seperated by frames, so all elements will be in the same TestcharmvisionFrame
    ...
    async def start_listen_dom_increment(self, element: ElementHandle | None = None) -> None:
        ...
        await TestcharmvisionFrame.evaluate(frame=self.testcharmvision_frame.get_frame(), expression=js_script, arg=element)
    ...
    async def stop_listen_dom_increment(self) -> None:
        ...
        if await TestcharmvisionFrame.evaluate(frame=self.testcharmvision_frame.get_frame(), expression=js_script):
            return
    ...
    async def get_incremental_elements_num(self) -> int:
        ...
        return await TestcharmvisionFrame.evaluate(frame=self.testcharmvision_frame.get_frame(), expression=js_script)
    ...
    async def __validate_element_by_value(self, value: str, element: dict) -> tuple[Locator | None, bool]:
        ...
        if element_id:
            parent_locator = self.testcharmvision_frame.get_frame().locator(f'[{TESTCHARMVISION_ID_ATTR}="{element_id}"]')
    ...
    def build_html_tree(self, element_tree: list[dict] | None = None, need_testcharmvision_attrs: bool = True) -> str:
        return "".join(
            [
                json_to_html(element, need_testcharmvision_attrs=need_testcharmvision_attrs)
    ...
    def build_element_tree(
        self, fmt: ElementTreeFormat = ElementTreeFormat.HTML, html_need_testcharmvision_attrs: bool = True
    ) -> str:
        if fmt == ElementTreeFormat.HTML:
            return self.build_html_tree(
                element_tree=self.element_tree_trimmed, need_testcharmvision_attrs=html_need_testcharmvision_attrs
            )
...
def _remove_unique_id(element: dict) -> None:
    if "attributes" not in element:
        return
    if TESTCHARMVISION_ID_ATTR in element["attributes"]:
        del element["attributes"][TESTCHARMVISION_ID_ATTR]
