...
| `max_concurrency` | integer \| null | `null` | Maximum number of iterations to run in parallel. When set, loop iterations execute concurrently up to this limit. When `null` or omitted, iterations run sequentially. |
...
Set `max_concurrency` to run loop iterations in parallel. For example, `max_concurrency: 3` processes up to 3 iterations at the same time. Each parallel iteration gets its own browser session. Omit or set to `null` for sequential execution.
</Info>
