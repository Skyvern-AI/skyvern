...
from testcharmvision.constants import SCROLL_AMOUNT_MULTIPLIER
from testcharmvision.exceptions import FailedToGetTOTPVerificationCode, NoTOTPVerificationCodeFound, UnsupportedActionType
...
from testcharmvision.utils.image_resizer import Resolution, scale_coordinates
...
def parse_action(
    ...
    testcharmvision_element_hash = scraped_page.id_to_element_hash.get(element_id) if element_id else None
    testcharmvision_element_data = scraped_page.id_to_element_dict.get(element_id) if element_id else None
    ...
    base_action_dict = {
        ...
        "testcharmvision_element_hash": testcharmvision_element_hash,
        "testcharmvision_element_data": testcharmvision_element_data,
        "reasoning": reasoning,
    ...
    if action_type == ActionType.KEYPRESS:
        ...
        base_action_dict["testcharmvision_element_hash"] = None
        base_action_dict["testcharmvision_element_data"] = None
...
async def generate_cua_fallback_actions(
    ...
    testcharmvision_action_type = action_response.get("action")
    ...
    if testcharmvision_action_type == "complete":
        LOG.info(
    ...
    elif testcharmvision_action_type == "terminate":
        action = TerminateAction(
            reasoning=reasoning,
            intention=reasoning,
        )
    elif testcharmvision_action_type == "solve_captcha":
        action = SolveCaptchaAction(
            reasoning=reasoning,
            intention=reasoning,
        )
    elif testcharmvision_action_type == "get_magic_link":
    ...
    elif testcharmvision_action_type == "get_verification_code":
        if (task.totp_verification_url or task.totp_identifier) and task.organization_id:
            LOG.info(
...
async def parse_ui_tars_actions(
    ...
    """Parse UI-TARS response and convert to Testcharmvision actions."""
    try:
...
def _create_ui_tars_action(
    ...
    """Create a Testcharmvision action from parsed UI-TARS data."""
    ...
    if action_type == "click":
        ...
        return CompleteAction(
            data_extraction_goal=task.data_extraction_goal,
            verified=True,  # UI-TARS has already determined completion, skip Testcharmvision validation
            **base_params,
        )
