...
export async function evaluateDom(tabScope, args) {
  ...
  return tabScope.runTabOperation(tabId, async (lease) => {
    ...
    try {
      ...
      throw new ProtocolError(
        ERROR_CODES.CDP_ERROR,
        "Chrome could not run this page expression. Direct evaluation requires the optional Allow User Scripts setting. Use browser controls if evaluation is unavailable; keep the user's current permissions.",
      );
    }
    lease.assertCurrent();
    assertWebTab(await tabScope.assertControllableLocked(tabId, lease));
