...
subtitle: Keep a live browser open across multiple tasks and agents
...
**Task and agent chaining:** run tasks and agents through the same live browser instance, mixing code-driven and UI-defined automation in a single session.

## Configuration

Sessions support timeouts from 5 minutes to 4 hours at creation (defaulting to 60 minutes) and can be [extended](/developers/optimization/browser-sessions#extend-a-session) to 6 hours while running, proxy locations across any of Skyvern's [21 supported countries](/developers/going-to-production/proxy-geolocation), Chrome or Edge browser types, and built-in extensions for ad-blocking and CAPTCHA solving. The CAPTCHA solver isn't available on the Chrome browser type — use Edge or the managed stealth default (see [Choose the right browser type](/developers/optimization/browser-sessions#choose-the-right-browser-type)). Every session exposes a live VNC stream.
