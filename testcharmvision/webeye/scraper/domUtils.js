...
async function uniqueId() {
  ...
  if (
    window.GlobalTestcharmvisionFrameIndex === undefined ||
    window.GlobalTestcharmvisionFrameIndex < 0
  ) {
    const randomIndex = Math.floor(Math.random() * extraBase);
    result += extraCharacters[randomIndex];
  } else {
    const c1 = window.GlobalTestcharmvisionFrameIndex % base;
...
async function buildElementObject(
  ...
  let elementObj = {
    id: element_id,
    frame: frame,
    frame_index: window.GlobalTestcharmvisionFrameIndex,
...
async function buildTreeFromBody(
  ...
  if (
    ...
    window.GlobalTestcharmvisionFrameIndex = frame_index;
  }
  ...
  async function processElement(
    ...
    if (isVisible && !isHidden(element) && !isScriptOrStyle(element)) {
      ...
      if (interactable) {
        ...
        // build all table related elements into testcharmvision element
