...
subtitle: Connect Bitwarden, 1Password, Azure Key Vault, AWS, or your own vault API
slug: cloud/managing-credentials/external-providers
---

If your organization already manages secrets in a dedicated vault, you can connect it as a credential source and reference items directly from workflow parameters — no need to duplicate credentials in Skyvern.
...
<Warning>
Make sure your Bitwarden account is on `bitwarden.com`, not `bitwarden.eu`.
</Warning>
...
<Steps>
  ...
  <Step title="Configure access with the Skyvern team">
    Go to the **Access** tab on your collection and follow the instructions provided by the enterprise support team. Contact [sales@skyvern.com](mailto:sales@skyvern.com) for this step.
  </Step>
...
{/* <img src="/images/cloud/credentials-bitwarden-param.png" alt="Bitwarden tab in credential parameter configuration" /> */}
...
For identity fields (SSN, address, phone numbers), specify an **Identity Key** and a comma-separated list of **Identity Fields** (e.g., `ssn, address, phone`).
...
Connects via a [service account token](https://developer.1password.com/docs/service-accounts/get-started/). Supports passwords and credit cards.

### One-time setup

<Steps>
  <Step title="Open Settings">
    Go to **Settings** and find the **1Password** card.
  </Step>
  <Step title="Enter your service account token">
    Paste the service account token from your 1Password admin console.
  </Step>
...
{/* <img src="/images/cloud/credentials-1password-settings.png" alt="1Password configuration card in Settings" /> */}

### Using in a workflow

In the workflow editor, add a **Credential Parameter** and select **1Password** as the source. Provide the **Vault ID** and **Item ID** from your 1Password web URLs.

{/* <img src="/images/cloud/credentials-1password-param.png" alt="1Password tab in credential parameter configuration" /> */}

<Warning>
Credit cards from 1Password need a text field named **"Expire Date"** on the item in `MM/YYYY` format. This is a workaround for how 1Password structures card data internally.
</Warning>
...
{/* <img src="/images/cloud/credentials-azure-settings.png" alt="Azure Key Vault configuration card in Settings" /> */}

### Using in a workflow

In the workflow editor, add a **Credential Parameter** and select **Azure Key Vault** as the source. Provide the vault name and the **secret names** that store the username, password, and optionally a TOTP secret — not the values themselves, just the secret names.

{/* <img src="/images/cloud/credentials-azure-param.png" alt="Azure Key Vault tab in credential parameter configuration" /> */}

---

## Webhook (Custom API)

Connect your own HTTP API as a credential backend. Skyvern calls your API to create, retrieve, and delete credentials — giving you full control over where sensitive data lives.

### API contract

Your service must implement three endpoints:
...
<Steps>
  ...
  <Step title="Test the connection">
    Click **Test Connection** to verify Skyvern can reach your API.
  </Step>
  <Step title="Save">
...
{/* <img src="/images/cloud/credentials-webhook-settings.png" alt="Custom Credential Service configuration card in Settings" /> */}
...
|---------|--------------|
| Connection test fails | Verify API base URL is correct and accessible. Check that the API token is valid. Note: 404/405 responses are considered successful if the server is reachable. |
| Credentials not created | Review your API logs for auth errors. Ensure the response includes an `id` field. |
| Env config not working | Restart Skyvern after setting variables. Verify `CREDENTIAL_VAULT_TYPE=custom` is set and both URL and token are provided. |
