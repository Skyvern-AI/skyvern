...
LAMINAR_API_BASE=http://laminar:8443
```

You can also use Laminar-native variable names:

```bash .env
LMNR_PROJECT_API_KEY=your-laminar-api-key
LMNR_BASE_URL=http://laminar:8443
```

Under the hood, Skyvern exports OpenTelemetry traces to your Laminar OTLP endpoint (`/v1/traces`) and automatically maps the Laminar variables above to OTEL exporter settings. No code changes are needed.
