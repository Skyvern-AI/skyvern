...
function getParameterId(param: Parameter): string {
  if ("workflow_parameter_id" in param && param.workflow_parameter_id) return param.workflow_parameter_id;
  if ("credential_parameter_id" in param && param.credential_parameter_id) return param.credential_parameter_id;
  if ("aws_secret_parameter_id" in param && param.aws_secret_parameter_id) return param.aws_secret_parameter_id;
  if ("onepassword_credential_parameter_id" in param && param.onepassword_credential_parameter_id)
    return param.onepassword_credential_parameter_id;
  if ("azure_vault_credential_parameter_id" in param && param.azure_vault_credential_parameter_id)
  ...
  if ("bitwarden_credit_card_data_parameter_id" in param && param.bitwarden_credit_card_data_parameter_id)
    return param.bitwarden_credit_card_data_parameter_id;
  if ("output_parameter_id" in param && param.output_parameter_id) return param.output_parameter_id;
  return param.key;
}
...
function getParameterDisplayValue(param: Parameter): string | null {
  switch (param.parameter_type) {
    ...
    case "credential": {
      ...
      return "credential_id" in param ? String((param as any).credential_id) : null;
    }
    case "aws_secret": {
      // Show the AWS secret key reference only
      return "aws_key" in param ? String((param as any).aws_key) : null;
    }
    case "onepassword": {
    ...
    case "context": {
      ...
      if ("value" in p && p.value !== undefined) {
        try {
          return typeof p.value === "string" ? p.value : JSON.stringify(p.value);
        } catch {
...
export function WorkflowParametersDialog({ open, onOpenChange, workflowId, workflows }: Props) {
  const workflow = useMemo(() => workflows?.find((w) => w.workflow_permanent_id === workflowId), [workflows, workflowId]);
  ...
  const items = useMemo(() => {
    ...
    return params.map((param) => ({
      ...
      key: param.key,
      description: "description" in param ? (param as any).description ?? undefined : undefined,
