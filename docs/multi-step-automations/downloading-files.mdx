...
slug: multi-step-automations/downloading-files
keywords:
  - download_files
  - downloaded_files
  - presigned URL
...
| `download_suffix` | `str` / `string` | No | Expected file extension (e.g., `".pdf"`, `".csv"`). The agent waits for a file matching this extension. |
| `download_timeout` | `float` / `number` | No | Seconds to wait for the download to start. If the file doesn't begin downloading within this time, the run fails. |
...
| `output` | `dict \| None` | Any extracted data |
...
**Use `download_suffix`** to tell the agent what file type to wait for. Without it, the agent may return before the download completes.

**Set `download_timeout`** for large files. The default may not be enough for files over 100MB.
...
Files downloaded during a workflow accumulate in a temporary directory and can be passed between blocks using workflow parameters. See [Workflow Blocks Reference](/multi-step-automations/workflow-blocks-reference) for block configuration details.
