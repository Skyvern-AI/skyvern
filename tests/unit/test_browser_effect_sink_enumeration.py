...
_DISCOVERED_BROWSER_API_CALLS = {
    "skyvern/forge/agent.py": Counter({"goto": 1, "new_page": 1}),
    ...
    "skyvern/webeye/real_browser_state.py": Counter({"close": 5, "evaluate": 1, "goto": 1, "new_page": 2, "reload": 2}),
}
...
_EVALUATE_CALLERS = {
    ...
    "skyvern/webeye/utils/dom.py": Counter({"apply_secret_visual_mask": 1, "blur": 1}),
}
...
def test_discovered_browser_api_lower_bound_is_stable() -> None:
    ...
    assert sum(sum(methods.values()) for methods in observed.values()) == 140
    ...
    assert sum(sum(methods.values()) for methods in observed.values()) - sum(_NON_BROWSER_CANDIDATES.values()) == 135
