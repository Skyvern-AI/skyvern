...
Download and parse a file from a URL. Supports CSV, Excel, PDF, image, and DOCX files. By default, the file type is auto-detected from the URL and file contents.
...
| `file_type` | string | `auto_detect` | One of: `auto_detect`, `csv`, `excel`, `pdf`, `image`, `docx`. When set to `auto_detect`, Skyvern infers the format from the URL and file contents. |
