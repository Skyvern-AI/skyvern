...
log_event() {
    if [ -n $1 ]; then
        uv run python skyvern/analytics.py $1
    fi
}
...
# Function to remove uv environment
remove_uv_env() {
    if [ -d ".venv" ]; then
        rm -rf .venv
        echo "Removed the uv environment at .venv."
    else
...
choose_python_version_or_fail() {
    if command -v python3.13 >/dev/null 2>&1; then
        UV_PYTHON=$(command -v python3.13)
        echo "Using Python 3.13 at $UV_PYTHON"
    else
        echo "Error: Python 3.13 is not installed. Please install Python 3.13 and try again."
        exit 1
    fi
}
...
install_dependencies_after_uv_env() {
...
setup_postgresql() {
    ...
    if command_exists psql; then
        if pg_isready; then
            ...
            if psql skyvern-cloud -U skyvern -c '\q'; then
                echo "Connection successful. Database and user exist."
            else
                createuser skyvern
                createdb skyvern-cloud -O skyvern
    ...
    if docker ps | grep -q postgresql-container; then
        ...
        docker run --name postgresql-container -e POSTGRES_HOST_AUTH_METHOD=trust -d -p 5432:5432 postgres:18
    ...
    if docker exec postgresql-container psql -U postgres -lqt | cut -d \| -f 1 | grep -qw "skyvern-cloud"; then
        echo "Database exists."
    else
        echo "Creating database..."
        docker exec postgresql-container createdb -U postgres skyvern-cloud -O skyvern
        echo "Database and user created successfully."
    fi
}
...
create_organization() {
    ...
    org_output=$(uv run python scripts/create_organization.py skyvern skyvern.local)
