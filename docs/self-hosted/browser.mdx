...
Skyvern uses Playwright with Chromium to execute browser automations. This guide covers the browser configuration options available for self-hosted deployments.
...
The browser runs with a visible window. In Docker, this displays on a virtual framebuffer (Xvfb). Useful for:
- Debugging automation issues
- Watching tasks execute in real-time
...
The browser runs without any display. Useful for:
- Production deployments
- CI/CD pipelines
- Servers without display capabilities
...
The default 1920x1080 works for most sites. Adjust if you need to:
- Test mobile layouts (e.g., 375x812 for iPhone)
- Handle sites that require specific dimensions
- Reduce memory usage with smaller viewports
...
Path to a Chrome policy file for enterprise configurations. Use this to:
- Disable specific Chrome features
- Configure proxy settings at the browser level
...
Browser instances are memory-intensive. Guidelines:
