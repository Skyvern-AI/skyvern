...
Once the services are healthy, retrieve your API key:
...
Copy the `cred` value—this is your API key.
...
<Warning>
Skyvern has no built-in authentication. If exposing to the internet, add authentication at the reverse proxy layer or use a VPN.
</Warning>
