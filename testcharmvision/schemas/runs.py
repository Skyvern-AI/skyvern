...
from testcharmvision.schemas.docs.doc_examples import (
...
from testcharmvision.schemas.docs.doc_strings import (
...
from testcharmvision.utils.url_validators import validate_url
...
class RunEngine(StrEnum):
    testcharmvision_v1 = "testcharmvision-1.0"
    testcharmvision_v2 = "testcharmvision-2.0"
...
class TaskRunRequest(BaseModel):
    ...
    engine: RunEngine = Field(
        default=RunEngine.testcharmvision_v2,
        description=TASK_ENGINE_DOC_STRING,
    )
    title: str | None = Field(
        default=None, description="The title for the task", examples=["The title of my first testcharmvision task"]
    )
    ...
    publish_workflow: bool = Field(
        default=False,
        description="Whether to publish this task as a reusable workflow. Only available for testcharmvision-2.0.",
    )
    ...
    def validate_urls(cls, url: str | None) -> str | None:
        """
        Validates that URLs provided to Testcharmvision are properly formatted.

        Args:
            url: The URL for Testcharmvision to validate
...
class WorkflowRunRequest(BaseModel):
    ...
    webhook_url: str | None = Field(
        default=None,
        description="URL to send workflow status updates to after a run is finished. Refer to https://www.testcharmvision.com/docs/running-tasks/webhooks-faq for webhook questions.",
    )
    totp_url: str | None = Field(
        default=None,
        description=TOTP_URL_DOC_STRING,
    ...
    browser_session_id: str | None = Field(
        default=None,
        description="ID of a Testcharmvision browser session to reuse, having it continue from the current screen state",
    )
...
class BaseRunResponse(BaseModel):
    ...
    app_url: str | None = Field(
        ...
        examples=["https://app.testcharmvision.com/tasks/tsk_123", "https://app.testcharmvision.com/workflows/wpid_123/wr_123"],
    )
    browser_session_id: str | None = Field(
        default=None, description="ID of the Testcharmvision persistent browser session used for this run", examples=["pbs_123"]
    )
    browser_profile_id: str | None = Field(
        default=None,
