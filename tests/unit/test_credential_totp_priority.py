...
class TestPayloadVerificationCodePriority:
    ...
    @pytest.mark.asyncio
    async def test_agent_prefers_navigation_goal_code_over_credential_totp(self) -> None:
        """When prompt includes a 2FA code, agent should use it and skip poll_otp_value."""
        with (
            patch(
                "skyvern.forge.agent.try_generate_totp_from_credential",
                return_value=OTPValue(value="999999", type=OTPType.TOTP),
    ...
    @pytest.mark.asyncio
    async def test_agent_prefers_navigation_goal_input_step_code_over_poll(self) -> None:
        """When prompt has 'MFA code' context and a separate 'Input 520265' step, skip poll."""
        with (
            patch(
                "skyvern.forge.agent.try_generate_totp_from_credential",
                return_value=OTPValue(value="999999", type=OTPType.TOTP),
    ...
    @pytest.mark.asyncio
    async def test_cua_fallback_uses_navigation_goal_input_step_code_and_skips_poll(self) -> None:
        """CUA fallback should parse code from 'Input 520265' prompt step and skip poll."""
        mock_task = MagicMock()
        mock_task.totp_verification_url = None
        mock_task.totp_identifier = None
        mock_task.organization_id = "org_123"
    ...
    @pytest.mark.asyncio
    async def test_cua_fallback_uses_navigation_goal_code_and_skips_poll(self) -> None:
        """CUA get_verification_code should use prompt-provided code directly when available."""
        mock_task = MagicMock()
        mock_task.totp_verification_url = None
        mock_task.totp_identifier = None
        mock_task.organization_id = "org_123"
