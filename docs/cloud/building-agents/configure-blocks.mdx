...
Automatic starts with Google. It tries Exa only if Google fails before returning results. An empty search succeeds and does not trigger fallback. Selecting Google or Exa uses only that provider.

Leave Prompt blank to make no LLM calls. When supplied, Prompt receives only the normalized search results and still runs when the list is empty. It cannot fetch result pages. Exa supports a single positive `site:domain` restriction; unsupported site expressions fail instead of expanding the search.
...
For example, use `{{ web_search_1_output.results }}` in a downstream block. Search results remain available for inspection if a later page or Prompt fails; the block reports failure.
...
Code blocks can search the web with the `search_web` helper instead of scripting
a search engine's pages directly:

```python
observation = await search_web("industrial fastener suppliers in Ohio", max_results=10)
...
|-----------|------|-------------|
| `query` | str | Search query. Must be a non-empty string. |
| `max_results` | int | Maximum number of results to return. Must be a positive integer. Defaults to 10. |

The helper returns an observation describing what the search actually produced:

| Field | Type | Description |
|-------|------|-------------|
| `results` | list | Up to `max_results` entries, each with `title`, `url`, and `snippet`. Each `url` is a direct absolute link to the result site. |
| `extracted_count` | int | Results found on the search page before any were withheld. |
| `withheld_count` | int | Results withheld because their destination is not allowed. |
| `http_status` | int or null | Status the search page was served with. `null` when the transport does not report one. |
| `error_kind` | str or null | Failure kind when the fetch itself failed, else `null`. |
| `page_title` | str | Title of the page that was served. |
| `capture_truncated` | bool | `true` when the page was too large to read in full. |

Read the observation rather than treating an empty `results` list as "no
matches". `extracted_count` of 0 with a 2xx `http_status` means the page carried
no results, which is a refusal page as often as a genuine miss; `page_title`
usually says which. A non-2xx `http_status` or a non-null `error_kind` means the
fetch failed and says nothing about whether matches exist. A `withheld_count`
greater than 0 with an empty `results` list means the page had results and they
were filtered.

The search runs through the run's own browser, so `search_web` is only available
while the run browser is open. If no web search provider is configured for the
deployment, the observation carries `error_kind: "not_configured"`. Open-source
deployments have no provider configured by default; Skyvern Cloud uses its
configured provider.
