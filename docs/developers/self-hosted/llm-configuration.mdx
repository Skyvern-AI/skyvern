...
OpenAI-compatible inference for DeepSeek, Qwen, GLM, Kimi and MiniMax models at [atlascloud.ai](https://www.atlascloud.ai/).
...
`ATLASCLOUD_API_BASE` defaults to `https://api.atlascloud.ai/v1` and only needs setting for a
non-default endpoint.

<Note>
`deepseek-ai/deepseek-v4-pro` is a reasoning model: it spends completion tokens on a hidden chain of
thought before the answer, so a small `LLM_CONFIG_MAX_TOKENS` can return an empty response with
`finish_reason="length"`. Non-reasoning ids such as `deepseek-ai/DeepSeek-V3.1` are unaffected.
</Note>
