...
- **`file_type`** tells Skyvern how to read the file. Use `auto_detect`, `csv`, `excel`, `pdf`, `image`, or `docx`. Defaults to `auto_detect`, which infers the format from the URL and file contents.
...
| `file_type` | string | Specify format: `auto_detect`, `csv`, `excel`, `pdf`, `image`, or `docx`. Defaults to `auto_detect`. |
