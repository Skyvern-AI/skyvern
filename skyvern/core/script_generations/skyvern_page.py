...
class SkyvernPage(Page):
    ...
    async def dynamic_field_map(
        ...
        *,
        ...
        """Map applicant data to form fields via a single cheap text-only LLM call.

        Designed for ATS platforms where form fields vary per employer but applicant
        data is consistent.  One LLM call sees ALL fields + ALL applicant data and
        produces a complete mapping — no deterministic matching, no caching, no
        accumulated state.
    ...
    async def validate_mapping(
        ...
        try:
            ...
            result = await app.SECONDARY_LLM_API_HANDLER(
                prompt=prompt_text,
                prompt_name="ats-validate-mapping",
                organization_id=org_id,
            )
