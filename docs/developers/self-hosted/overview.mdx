...
For the `pip install skyvern` path (Option A in the README), the default is a SQLite database at `~/.skyvern/data.db` — zero-config for local dev. To opt into Postgres for the pip path, run `skyvern quickstart --database-string=postgresql+psycopg://user:pass@host:5432/dbname`.
