...
4. Write the MCP configuration file for each detected desktop client automatically
...
Add this to your MCP client's configuration file (including Claude Code manual setup):
...
| **Claude Desktop** (Linux) | `~/.config/Claude/claude_desktop_config.json` (or `~/.local/share/Claude/claude_desktop_config.json`) |
| **Cursor** | `~/.cursor/mcp.json` |
| **Windsurf** | `~/.codeium/windsurf/mcp_config.json` |
| **Claude Code** (project scope) | `<project>/.mcp.json` |
| **Claude Code** (user/local scopes) | `~/.claude.json` |
...
1. For project-scoped setup, add the `mcpServers.Skyvern` config to `<project>/.mcp.json`.
2. For user/local scopes, add the same `mcpServers.Skyvern` config to `~/.claude.json`.
