...
slug: multi-step-automations/overview
keywords:
...
A single `run_task` handles one goal on one page. When your automation needs to log in, navigate across pages, extract data, and download files, you build a browser automation.

This guide walks through a real example: logging into a vendor portal, extracting invoice data, and downloading a PDF. Five steps:
...
5. **Close the browser** to release resources
...
For browser launch options (timeouts, proxies, connecting to local browsers), see [Managing Browsers](/multi-step-automations/managing-browsers).

---

## Step 2: Navigate and interact with the page

Once you have a page, you can interact with it using standard Playwright, AI actions, or both. For the full list of available methods and parameters, see the [Actions Reference](/multi-step-automations/actions-reference).
...
# Check page state (returns True/False)
is_logged_in = await page.validate("The user is logged in")

# Ask the AI a question about the page
answer = await page.prompt("What is the total at the bottom of the table?")
```
...
// Ask the AI a question about the page
const answer = await page.prompt("What is the total at the bottom of the table?");
```
</CodeGroup>
...
print(result.output)  # {"invoice_number": "INV-2025-042", "amount": "$1,250.00"}
```
...
console.log(result.output); // {"invoice_number": "INV-2025-042", "amount": "$1,250.00"}
```
</CodeGroup>
...
Every agent call returns a response with the same shape:

| Field | Description |
|-------|-------------|
| `run_id` | Unique identifier (`tsk_*` for tasks, `wr_*` for workflows) |
| `status` | `completed`, `failed`, `terminated`, `timed_out`, or `canceled` |
| `output` | Extracted data matching your schema |
| `failure_reason` | Error description if the run failed |
| `recording_url` | Video recording of the entire execution |
| `screenshot_urls` | Screenshots captured at each step |
| `downloaded_files` | Files downloaded during the run (with URLs) |
| `step_count` | Number of AI steps taken (affects billing) |

<CodeGroup>
```python Python
result = await page.agent.run_task("Extract the account balance")
...
```typescript TypeScript
const result = await page.agent.runTask("Extract the account balance");

if (result.status === "completed") {
  console.log(result.output);
  console.log(`Recording: ${result.recording_url}`);
  console.log(`Steps used: ${result.step_count}`);
...
Always close the browser when done to release cloud resources:
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="browser"
    href="/multi-step-automations/managing-browsers"
  >
...
<CardGroup cols={2}>
  ...
  <Card
    title="SDK Reference"
    icon="code"
    href="/sdk-reference/browser-automation/act"
  >
    Full method documentation for Page, Agent, and Browser
  </Card>
</CardGroup>
