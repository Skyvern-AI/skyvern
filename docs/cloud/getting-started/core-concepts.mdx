...
The engine picker appears in API/SDK task requests and on supported browser-driven blocks in the workflow editor.

| Engine | When to use |
|--------|-------------|
| **Skyvern 1.0** | Default for new tasks. Lighter and faster for simple, single-page interactions. |
| **Skyvern 2.0** | Available for existing V2 workflows and explicitly requested V2 task runs. |
...
If you're not sure, use the default. Move repeated automations into workflows when you need scheduling, reusable parameters, or cached code replays.
