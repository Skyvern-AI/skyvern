...
def _build_wrc_with_password_credential():
    from unittest.mock import MagicMock
    ...
    username_secret_id = wrc.generate_random_secret_id()
    password_secret_id = wrc.generate_random_secret_id()
    wrc.secrets[username_secret_id] = "demo_business_user"
    wrc.secrets[password_secret_id] = "Demo!Pass123"
    wrc.values[_CREDENTIAL_KEY] = {
        "context": "placeholder note",
...
def _build_wrc_with_scalar_secret():
    from unittest.mock import MagicMock
    ...
    secret_id = wrc.generate_random_secret_id()
    wrc.secrets[secret_id] = "scalar-secret-token"
    wrc.values[_SCALAR_SECRET_KEY] = secret_id
    return wrc
...
async def _run_scalar_secret_code_block(monkeypatch: "pytest.MonkeyPatch", wrc, code: str, label: str):
    from unittest.mock import MagicMock
    ...
    secret_parameter = AWSSecretParameter(
        parameter_type=ParameterType.AWS_SECRET,
        key=_SCALAR_SECRET_KEY,
        description="secret",
        workflow_id="w",
        aws_secret_parameter_id=f"sp_{label}",
        aws_key="test/secret",
        created_at=now,
        modified_at=now,
    )
...
class TestCodeBlockOpaqueCredentialFields:
    @pytest.mark.asyncio
    async def test_bound_credential_fields_are_not_plaintext_strings(self, monkeypatch: pytest.MonkeyPatch) -> None:
        wrc = _build_wrc_with_password_credential()

        result, output_value = await _run_credential_code_block(
            monkeypatch,
            wrc,
            code=(
                f"password_is_plaintext = {_CREDENTIAL_KEY}.password == 'Demo!Pass123'\n"
                f"username_is_plaintext = {_CREDENTIAL_KEY}.username == 'demo_business_user'"
            ),
            label="opaque_password",
        )

        assert result.success is True
        assert output_value == {
            "password_is_plaintext": False,
            "username_is_plaintext": False,
        }

    @pytest.mark.asyncio
    async def test_bound_credential_fields_stringify_to_mask(self, monkeypatch: pytest.MonkeyPatch) -> None:
        wrc = _build_wrc_with_password_credential()
        ...
        result, output_value = await _run_credential_code_block(
            ...
            wrc,
            code=(
                f"password_text = str({_CREDENTIAL_KEY}.password)\n"
                f"username_text = str({_CREDENTIAL_KEY}.username)\n"
                f"bare_password_text = str(password)"
            ),
            label="opaque_str",
        )
        ...
        assert "Demo!Pass123" not in json.dumps(output_value)
        assert "demo_business_user" not in json.dumps(output_value)

    @pytest.mark.asyncio
    async def test_bound_credential_fields_cannot_be_concatenated_to_exfiltrate(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        wrc = _build_wrc_with_password_credential()
        ...
        result, _ = await _run_credential_code_block(
            monkeypatch,
            wrc,
            code=f"leak = {_CREDENTIAL_KEY}.password + '-suffix'",
            label="opaque_concat",
        )

        assert result.success is False
        assert result.failure_reason is not None
        assert "Demo!Pass123" not in result.failure_reason

    def test_user_code_cannot_reach_secret_handle_private_slot(self) -> None:
        with pytest.raises(InsecureCodeDetected, match="private"):
            CodeBlock.is_safe_code("credential.password._CodeBlockSecret__value")

    def test_user_code_cannot_reach_secret_handle_protected_reveal(self) -> None:
        with pytest.raises(InsecureCodeDetected, match="private"):
            CodeBlock.is_safe_code("credential.password._reveal_for_input_action()")

    def test_user_code_cannot_reach_recording_page_protected_hooks(self) -> None:
        with pytest.raises(InsecureCodeDetected, match="private"):
            CodeBlock.is_safe_code("page._recorded_actions()")
        with pytest.raises(InsecureCodeDetected, match="private"):
            CodeBlock.is_safe_code("page._last_recorded_exception()")

    def test_public_attribute_with_double_underscore_is_not_treated_as_private(self) -> None:
        CodeBlock.is_safe_code("value = record.public__field")

    @pytest.mark.asyncio
    async def test_bound_credential_fields_have_no_public_plaintext_reveal_method(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        wrc = _build_wrc_with_password_credential()
        ...
        result, _ = await _run_credential_code_block(
            monkeypatch,
            wrc,
            code=f"leak = {_CREDENTIAL_KEY}.password.reveal_for_input_action()",
            label="opaque_public_reveal",
        )

        assert result.success is False
        assert result.failure_reason is not None
        assert "reveal_for_input_action" in result.failure_reason
        assert "Demo!Pass123" not in result.failure_reason


class TestCodeBlockOpaqueScalarSecrets:
    @pytest.mark.asyncio
    async def test_scalar_secret_parameter_is_not_plaintext_string(self, monkeypatch: pytest.MonkeyPatch) -> None:
        wrc = _build_wrc_with_scalar_secret()

        result, output_value = await _run_scalar_secret_code_block(
            monkeypatch,
            wrc,
            code=(
                f"secret_is_plaintext = {_SCALAR_SECRET_KEY} == 'scalar-secret-token'\n"
                f"secret_text = str({_SCALAR_SECRET_KEY})"
            ),
            label="opaque_scalar",
        )
        ...
        assert "scalar-secret-token" not in json.dumps(output_value)

    @pytest.mark.asyncio
    async def test_scalar_secret_parameter_cannot_be_concatenated_to_exfiltrate(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        wrc = _build_wrc_with_scalar_secret()

        result, _ = await _run_scalar_secret_code_block(
            monkeypatch,
            wrc,
            code=f"leak = {_SCALAR_SECRET_KEY} + '-suffix'",
            label="opaque_scalar_concat",
        )

        assert result.success is False
        assert result.failure_reason is not None
        assert "scalar-secret-token" not in result.failure_reason
