...
const constants = {
  TestcharmvisionOptimized: "Testcharmvision Optimized",
} as const;
...
function ModelSelector({
  ...
  const chosen = value
    ? models[value.model_name] ?? constants.TestcharmvisionOptimized
    : constants.TestcharmvisionOptimized;
  const choices = [constants.TestcharmvisionOptimized, ...labels];
    ...
    <div className="flex items-center justify-between">
      ...
      <div className="relative flex items-center">
        <Select
          value={chosen}
          onValueChange={(v) => {
            const newValue = v === constants.TestcharmvisionOptimized ? null : v;
        ...
        >
          ...
          >
            <SelectValue placeholder={constants.TestcharmvisionOptimized} />
          </SelectTrigger>
          <SelectContent>
            {choices.map((m) => (
              <SelectItem key={m} value={m}>
                {m === constants.TestcharmvisionOptimized ? (
                  <span>Testcharmvision Optimized ✨</span>
                ) : (
                  m
                )}
              </SelectItem>
