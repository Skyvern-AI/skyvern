...
import {
  ...
  parameterIsTestcharmvisionCredential,
...
type CredentialSource = "bitwarden" | "testcharmvision" | "onepassword" | "azurevault";
...
function getAvailableSourcesForDataType(
  ...
  switch (dataType) {
    case "password":
      return [
        ...(isCloud ? [{ value: "testcharmvision" as const, label: "Testcharmvision" }] : []),
...
function detectInitialCredentialSource(
  ...
  if (!initialValues) return isCloud ? "testcharmvision" : "bitwarden";
  ...
  if (initialValues.parameterType === "credential") {
    if (parameterIsTestcharmvisionCredential(initialValues)) return "testcharmvision";
  ...
  return isCloud ? "testcharmvision" : "bitwarden";
}
...
function WorkflowParameterEditPanel({
  ...
  const isTestcharmvisionCredential =
    initialValues?.parameterType === "credential" &&
    parameterIsTestcharmvisionCredential(initialValues);
  ...
  const [credentialId, setCredentialId] = useState(
    isTestcharmvisionCredential ? initialValues.credentialId : "",
  );
  ...
  const showTestcharmvisionCredentialSelector =
    showCredentialFields && credentialSource === "testcharmvision" && isCloud;
    ...
    <ScrollArea>
      <ScrollAreaViewport className="max-h-[500px]">
        <div className="space-y-4 p-1 px-4">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    if (wasCredential && !isNowCredential) {
                      ...
                      setCredentialSource(isCloud ? "testcharmvision" : "bitwarden");
            ...
            <>
              ...
              <div className="space-y-1">
                <div className="flex gap-2">
                  ...
                  <HelpTooltip content="Select where your credentials are stored. Testcharmvision uses managed credentials, while Bitwarden, 1Password, and Azure Key Vault connect directly to your vault." />
                </div>
                <Select
            ...
            <>
              <div className="space-y-1">
                <div className="flex gap-2">
                  ...
                  <HelpTooltip content="Optional. The workflow parameter key that holds the URL. If provided, Testcharmvision will match the credential based on this URL." />
                </div>
                <Input
          ...
          {/* Testcharmvision Managed Credential Selector */}
          {showTestcharmvisionCredentialSelector && (
            <div className="space-y-1">
              <div className="flex gap-2">
                <Label className="text-xs text-slate-300">
                  Testcharmvision Credential
                </Label>
                <HelpTooltip content="Select a credential from your Testcharmvision credential store. These are managed credentials you've previously added to Testcharmvision." />
              </div>
            ...
            <Button
              onClick={() => {
                ...
                if (type === "credential" || isCredentialSelected) {
                  // Testcharmvision managed credentials
                  if (credentialSource === "testcharmvision") {
                    if (!credentialId) {
                      toast({
                        variant: "destructive",
                        title: "Failed to save parameter",
