...
from testcharmvision.constants import TESTCHARMVISION_ID_ATTR, TEXT_INPUT_DELAY
...
from testcharmvision.experimentation.wait_utils import get_or_create_wait_config, get_wait_time, scroll_into_view_wait
from testcharmvision.webeye.actions import handler_utils
from testcharmvision.webeye.scraper.scraped_page import ScrapedPage, json_to_html
from testcharmvision.webeye.scraper.scraper import IncrementalScrapePage, trim_element
...
async def resolve_locator(scrape_page: ScrapedPage, page: Page, frame: str, css: str) -> tuple[Locator, Page | Frame]:
    ...
    while frame != "main.frame":
        ...
        if not parent_frame:
            raise TestcharmvisionException(f"element without frame: {frame_element}")
    ...
    while len(iframe_path) > 0:
        ...
        frame_handler = await current_frame.query_selector(f"[{TESTCHARMVISION_ID_ATTR}='{child_frame}']")
        ...
        current_page = current_page.frame_locator(f"[{TESTCHARMVISION_ID_ATTR}='{child_frame}']")
...
class TestcharmvisionOptionType(typing.TypedDict):
    optionIndex: int
    text: str
    value: str


class TestcharmvisionElement:
    """
    TestcharmvisionElement is a python interface to interact with js elements built during the scarping.
    When you try to interact with these elements by python, you are supposed to use this class as an interface.
    """

    # TODO: support to create TestcharmvisionElement from incremental page by xpath
    @classmethod
    async def create_from_incremental(cls, incre_page: IncrementalScrapePage, element_id: str) -> TestcharmvisionElement:
        ...
        frame = incre_page.testcharmvision_frame.get_frame()
    ...
    def __repr__(self) -> str:
        return f"TestcharmvisionElement({str(self.__static_element)})"

    def build_HTML(self, need_trim_element: bool = True, need_testcharmvision_attrs: bool = True) -> str:
        element_dict = self.get_element_dict()
        if need_trim_element:
            element_dict = trim_element(copy.deepcopy(element_dict))

        return json_to_html(element_dict, need_testcharmvision_attrs)
    ...
    async def is_disabled(self, dynamic: bool = False) -> bool:
        ...
        try:
            ...
            testcharmvision_frame = await TestcharmvisionFrame.create_instance(self.get_frame())
            style_disabled = await testcharmvision_frame.get_disabled_from_style(await self.get_element_handler())
    ...
    async def is_visible(self, must_visible_style: bool = True) -> bool:
        ...
        if not must_visible_style:
            return True
        testcharmvision_frame = await TestcharmvisionFrame.create_instance(self.get_frame())
        return await testcharmvision_frame.get_element_visible(await self.get_element_handler())
    ...
    async def is_parent_of(self, target: ElementHandle) -> bool:
        testcharmvision_frame = await TestcharmvisionFrame.create_instance(self.get_frame())
        return await testcharmvision_frame.is_parent(await self.get_element_handler(), target)

    async def is_child_of(self, target: ElementHandle) -> bool:
        testcharmvision_frame = await TestcharmvisionFrame.create_instance(self.get_frame())
        return await testcharmvision_frame.is_parent(target, await self.get_element_handler())

    async def is_sibling_of(self, target: ElementHandle) -> bool:
        testcharmvision_frame = await TestcharmvisionFrame.create_instance(self.get_frame())
        return await testcharmvision_frame.is_sibling(await self.get_element_handler(), target)
    ...
    def get_options(self) -> list[TestcharmvisionOptionType]:
        options = self.__static_element.get("options", None)
        if options is None:
            return []

        return typing.cast(typing.List[TestcharmvisionOptionType], options)
    ...
    ) -> tuple[TestcharmvisionElement | None, bool]:
        testcharmvision_frame = await TestcharmvisionFrame.create_instance(self.get_frame())
        blocking_element_id, blocked = await testcharmvision_frame.get_blocking_element_id(await self.get_element_handler())
        if not blocking_element_id:
            return None, blocked

        if await dom.check_id_in_dom(blocking_element_id):
            return await dom.get_testcharmvision_element_by_id(blocking_element_id), blocked

        if incremental_page and incremental_page.check_id_in_page(blocking_element_id):
            return await TestcharmvisionElement.create_from_incremental(incremental_page, blocking_element_id), blocked
    ...
    async def find_element_in_label_children(
        ...
        if not element_id:
            return None
        return await dom.get_testcharmvision_element_by_id(element_id=element_id)
    ...
    async def find_label_for(
        ...
        unique_id = await locator.get_attribute(TESTCHARMVISION_ID_ATTR, timeout=timeout)
        if unique_id is None:
            return None

        return await dom.get_testcharmvision_element_by_id(unique_id)
    ...
    async def find_selectable_child(self, dom: DomUtil) -> TestcharmvisionElement | None:
        ...
        while index < len(queue):
            ...
            for child in children:
                ...
                child_element = await dom.get_testcharmvision_element_by_id(child_id)
    ...
    async def click_in_javascript(self) -> None:
        testcharmvision_frame = await TestcharmvisionFrame.create_instance(self.get_frame())
        await testcharmvision_frame.click_element_in_javascript(await self.get_element_handler())
    ...
    async def blur(self) -> None:
        if not await self.is_visible():
            return
        await TestcharmvisionFrame.evaluate(
    ...
    async def scroll_into_view(self, timeout: float = settings.BROWSER_ACTION_TIMEOUT_MS) -> None:
        ...
        try:
            ...
            if target_x is not None and target_y is not None:
                await testcharmvision_frame.safe_scroll_to_x_y(target_x, target_y)
        except Exception:
            LOG.info(
    ...
    async def refresh_select_options(self) -> tuple[list, str] | None:
        ...
        frame = await TestcharmvisionFrame.create_instance(self.get_frame())
...
class DomUtil:
    ...
    async def get_testcharmvision_element_by_id(self, element_id: str) -> TestcharmvisionElement:
        ...
        return TestcharmvisionElement(locator, frame_content, element, hash_value)

    async def safe_get_testcharmvision_element_by_id(self, element_id: str) -> TestcharmvisionElement | None:
        try:
            return await self.get_testcharmvision_element_by_id(element_id)
        except Exception:
            LOG.warning("Failed to get testcharmvision element by id", element_id=element_id, exc_info=True)
            return None
