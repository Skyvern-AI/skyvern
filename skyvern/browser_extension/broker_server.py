...
class BrowserExtensionBrokerServer:
    """M1 exclusive broker from spec-v3.md lines 3-53, 79-105, and 199-205."""
    ...
    def __init__(
        ...
        *,
        ...
        self._active_client: _ClientConnection | None = None
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            initialize_empty_journal(self.paths)
    ...
    async def stop(self) -> None:
        ...
        if active is not None and active.sender_task is not None:
            with suppress(Exception):
                await self._send(active, event_frame("broker.draining", {}))
        ...
        if active is not None:
            await self._close_connection(active)
        self._relay = None
    ...
    async def _authenticate(
        ...
        async with self._client_lock:
            ...
            if not operator and self._extension_reset_quarantined and relay is not None and relay.connected:
                ...
                raise error
            if op == "client.enroll":
                if not operator and (self._active_client is not None or self._has_forwarded_work()):
                    error = BrowserExtensionBrokerError("BROKER_BUSY", "Browser-extension broker already has a client")
                    await write_frame(writer, error_frame(request_id, error), max_size=PREAUTH_FRAME_LIMIT)
                    raise error
                ...
                if previous is not None and previous.client_id != credential.client_id:
                    error = BrowserExtensionBrokerError("BROKER_BUSY", "Browser-extension broker already has a client")
                    await write_frame(writer, error_frame(request_id, error), max_size=PREAUTH_FRAME_LIMIT)
                    raise error
                if self._has_forwarded_work():
                    error = BrowserExtensionBrokerError(
                        "BROKER_BUSY", "Browser-extension broker is draining an outstanding extension request"
                    )
                    await write_frame(writer, error_frame(request_id, error), max_size=PREAUTH_FRAME_LIMIT)
                    raise error
                if previous is not None:
                    await self._close_connection(previous)
                    await self._release_active_client_locked(previous)
            else:
                error = BrowserExtensionBrokerError("AUTH_FAILED", "Broker authentication failed")
                await write_frame(writer, error_frame(request_id, error), max_size=PREAUTH_FRAME_LIMIT)
                raise error
        ...
        relay = self._relay
        if not connection.operator and relay is not None and relay.connected and not self._extension_reset_quarantined:
            await self._send_event(
                connection,
                "extension.event",
                {"event": "extension.hello", "params": {"scopedTabs": list(relay.scoped_tabs)}},
            )
        ...
        *,
        ...
        if op == "broker.status":
            ...
            if not connection.operator and not self._extension_reset_quarantined:
                tab_ids = [tab["tabId"] for tab in relay.scoped_tabs if type(tab.get("tabId")) is int]
            return {
                ...
                "extensionConnected": relay.connected and not self._extension_reset_quarantined,
                "clientCount": 1 if self._active_client is not None else 0,
                "tabIds": tab_ids,
                "quarantines": [],
            }
        if op == "extension.wait_connected":
            ...
            return {"connected": await self._wait_extension_ready(relay, float(timeout))}
        if op == "extension.request":
        ...
        raise BrowserExtensionBrokerError("OP_NOT_ALLOWED", "Broker operation is not available in milestone 1")
    ...
    def _expire_pairing(self, relay: Relay) -> None:
        if self._pairing_owner is not None and time.monotonic() >= self._pairing_expires_at:
            relay.cancel_pairing_nonce()
            self._pairing_owner = None
            self._pairing_expires_at = 0.0

    async def _handle_pairing_complete(self) -> None:
        self._pairing_owner = None
        self._pairing_expires_at = 0.0
    ...
    async def _handle_extension_event(self, event: str, params: dict) -> None:
        ...
        if event == "extension.reset_ack":
            ...
            if (
                ...
                connection = self._active_client
                if ok is True and relay is not None and connection is not None and not connection.closed:
                    await self._send_event(
                        connection,
                        "extension.event",
                        {"event": "extension.hello", "params": {"scopedTabs": list(relay.scoped_tabs)}},
        ...
        if self._extension_reset_quarantined:
            ...
            return
        connection = self._active_client
        if connection is not None and not connection.closed and await self._is_current(connection):
            if event == "extension.hello":
    ...
    async def _connection_closed(self, connection: _ClientConnection) -> None:
        ...
        async with self._client_lock:
            await self._release_active_client_locked(connection)

    async def _release_active_client_locked(self, connection: _ClientConnection) -> None:
        if self._active_client is not connection or connection.ownership_released:
            return
        self._arm_extension_reset(increment_generation=True)
        reset_epoch = self._extension_reset_epoch
        reset_generation = self._extension_reset_generation
        relay = self._relay
        if relay is None or self._stopping or not relay.connected:
            self._extension_reset_error = None
        else:
            try:
            ...
            if reset_complete is True:
                ...
                self._extension_reset_error = None
            elif self._extension_reset_error not in {"EXTENSION_RESET_FAILED", "EXTENSION_RESET_TIMEOUT"}:
    ...
    async def _is_current(self, connection: _ClientConnection) -> bool:
        ...
        async with self._client_lock:
            ...
            return (
                ...
                and current.client_id == connection.client_id
                and current.generation == connection.generation
    ...
    def _has_forwarded_work(self) -> bool:
        relay = self._relay
        return bool(self._forwarded_tasks) or (relay is not None and relay.pending_request_count > 0)
    ...
    def _state(self, *, lifecycle: str, clean_shutdown: bool) -> BrokerState:
        return BrokerState(
            ...
            features=("exclusive-client", "explicit-pairing", "persistent-daemon"),
            brokerGeneration=BROKER_GENERATION,
            buildFingerprint=BROKER_BUILD_FINGERPRINT,
        )
