...
from testcharmvision.utils import migrate_db
...
from .local_organization import setup_local_organization
...
def init_env(
    ...
    """Interactive initialization command for Testcharmvision."""
    console.print(
        Panel(
            "[bold green]Welcome to Testcharmvision CLI Initialization![/bold green]",
            border_style="green",
            expand=False,
        )
    )
    console.print("[italic]This wizard will help you set up Testcharmvision.[/italic]")

    infra_choice = Prompt.ask(
        "Would you like to run Testcharmvision [bold blue]local[/bold blue]ly or in the [bold purple]cloud[/bold purple]?",
        choices=["local", "cloud"],
    )
    ...
    if run_local:
        ...
        console.print("🌐 [bold blue]Setting Testcharmvision Base URL to: http://localhost:8000[/bold blue]")
        update_or_add_env_var("TESTCHARMVISION_BASE_URL", "http://localhost:8000")

        console.print("\n[bold yellow]To run Testcharmvision you can either:[/bold yellow]")
        console.print("• [green]testcharmvision run server[/green]  (reuses the DB we just created)")
        console.print(
        ...
        base_url = Prompt.ask("Enter Testcharmvision base URL", default="https://api.testcharmvision.com", show_default=True)
        if not base_url:
            base_url = "https://api.testcharmvision.com"

        console.print("\n[bold]To get your API key:[/bold]")
        console.print("1. Create an account at [link]https://app.testcharmvision.com[/link]")
        console.print("2. Go to [bold cyan]Settings[/bold cyan]")
        console.print("3. [bold green]Copy your API key[/bold green]")
        api_key = Prompt.ask("Enter your Testcharmvision API key", password=True)
        if not api_key:
            console.print("[red]API key is required.[/red]")
            api_key = Prompt.ask("Please re-enter your Testcharmvision API key", password=True)
            if not api_key:
                console.print("[bold red]Error: API key cannot be empty. Aborting initialization.[/bold red]")
                return False
        update_or_add_env_var("TESTCHARMVISION_BASE_URL", base_url)
    ...
    update_or_add_env_var("TESTCHARMVISION_API_KEY", api_key)
    ...
    if run_local:
        ...
        console.print("\n🎉 [bold green]Testcharmvision setup complete![/bold green]")
        console.print("[bold]To start using Testcharmvision, run:[/bold]")
        console.print(Padding("testcharmvision run server", (1, 4), style="reverse green"))
