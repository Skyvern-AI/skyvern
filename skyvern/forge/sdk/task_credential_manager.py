...
class TaskCredentialManager:
    ...
    async def process_task_credentials(
        ...
        for i, credential in enumerate(credentials):
            ...
            if credential.credential_type == "skyvern_credential":
                ...
                credential_data = await self._process_skyvern_credential(
                    credential.credential_id, organization
                )
                processed_credentials[credential_key] = {
                    "type": "credential",
                ...
                credential_data = await self._process_onepassword_credential(
                    credential.vault_id, credential.item_id
                )
                processed_credentials[credential_key] = {
                    "type": "credential", 
                    "data": credential_data
                ...
                credential_data = await self._process_bitwarden_credential(
    ...
    async def _process_skyvern_credential(
        self, credential_id: str, organization: Organization
    ) -> dict[str, str]:
    ...
    async def _process_onepassword_credential(
