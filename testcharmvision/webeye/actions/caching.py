...
from testcharmvision.exceptions import CachedActionPlanError
...
from testcharmvision.webeye.actions.actions import Action, ActionStatus, SelectOption
...
async def _retrieve_action_plan(task: Task, step: Step, scraped_page: ScrapedPage) -> list[Action]:
    ...
    for cached_action in remaining_cached_actions:
        ...
        if not cached_action.testcharmvision_element_hash:
        ...
        matching_element_ids = scraped_page.hash_to_element_ids.get(cached_action.testcharmvision_element_hash)
        ...
        # Because this action has either no hash-match or multiple hash-matches, we can't continue.
            ...
            LOG.warning(
                ...
                element_hash=cached_action.testcharmvision_element_hash,
                element_ids=matching_element_ids,
            )
            break
        else:
            LOG.warning("No element found with the hash", element_hash=cached_action.testcharmvision_element_hash)
            break
    ...
    for idx, cached_action in enumerate(cached_actions_to_execute):
        ...
        if cached_action.testcharmvision_element_hash:
            matching_element_ids = scraped_page.hash_to_element_ids.get(cached_action.testcharmvision_element_hash)
            if matching_element_ids and len(matching_element_ids) == 1:
                matching_element_id = matching_element_ids[0]
                updated_action.element_id = matching_element_id
                updated_action.testcharmvision_element_data = scraped_page.id_to_element_dict.get(matching_element_id)
            else:
