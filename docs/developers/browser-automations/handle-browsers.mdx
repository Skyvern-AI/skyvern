...
|-----------|------|---------|-------------|
| `timeout` | `int` | `None` | Session timeout in minutes (min 5, max 240; higher values are capped at 240 with a warning). A running session can be extended to 360 with [`extend_browser_session`](/sdk-reference/browser-sessions/extend-browser-session). Server default: 60 minutes |
