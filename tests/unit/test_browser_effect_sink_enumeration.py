...
def test_discovered_browser_api_lower_bound_is_stable() -> None:
    ...
    assert sum(sum(methods.values()) for methods in observed.values()) == 157
    ...
    assert sum(sum(methods.values()) for methods in observed.values()) - sum(_NON_BROWSER_CANDIDATES.values()) == 152
...
def test_every_raw_evaluate_call_is_classified() -> None:
    ...
    assert sum(sum(callers.values()) for callers in observed.values()) == 26
