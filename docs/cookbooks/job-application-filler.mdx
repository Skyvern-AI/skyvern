...
```json JSON
{
  "parameters": [
    {
      "key": "resume",                      // <-- parameter name
      "parameter_type": "workflow",         // <-- always set to "workflow"
      "workflow_parameter_type": "file_url" // <-- can be string, file_url, credential_id, etc
    },
    {
      "key": "credentials",
...
5. **For loop + Extraction + Text Prompt + Task** — Iterates over each job to extract details, generates relevant answers using an LLM, and submits the form.
...
async def main():
    ...
    run = await client.run_workflow(
        ...
        parameters={
            ...
            "job_portal_url": "https://jobs.example.com/login"   # <-- replace this
        }
    )
...
RUN=$(curl -s -X POST "https://api.skyvern.com/v1/run/workflows" \
  ...
  -d "{
    ...
    \"parameters\": {
      ...
      \"job_portal_url\": \"https://jobs.example.com/login\"
    }
  }")
