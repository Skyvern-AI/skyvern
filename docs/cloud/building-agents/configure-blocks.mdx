...
| **Goal** | text | Optional natural-language description of what the code block accomplishes. When set, browser interactions in the code are recorded as actions and screenshots in the run timeline, and the agent can take over if the code fails. |
| **Steps** | list | Optional plain-language summary used to render the block in the editor and run timeline. Auto-derived from the code when omitted. |
