...
You need a `run_id` to retrieve artifacts. You get this when you [run a task](/running-automations/run-a-task) or agent.

---

## Retrieve artifacts

Use `get_run_artifacts` with the `run_id` from your task or agent run:
