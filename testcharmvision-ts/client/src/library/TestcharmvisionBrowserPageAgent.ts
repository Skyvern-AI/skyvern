...
import type * as Testcharmvision from "../api/index.js";
...
import type { TestcharmvisionBrowser } from "./TestcharmvisionBrowser.js";
import { LOG } from "./logger.js";

function getAppUrlForRun(runId: string): string {
    return `https://app.testcharmvision.com/runs/${runId}`;
}
 ...
 */
export class TestcharmvisionBrowserPageAgent {
    private readonly _browser: TestcharmvisionBrowser;
    private readonly _page: Page;

    constructor(browser: TestcharmvisionBrowser, page: Page) {
     ...
     * @param options.engine - The execution engine to use. Defaults to testcharmvision_v2.
    ...
    async runTask(
        prompt: string,
        options?: {
            engine?: Testcharmvision.RunEngine;
    ...
    ): Promise<Testcharmvision.TaskRunResponse> {
        LOG.info("AI run task", { prompt });

        const taskRun = await this._browser.testcharmvision.runTask({
            "x-user-agent": "testcharmvision-sdk",
            body: {
                prompt: prompt,
        ...
        if (this._browser.testcharmvision.environment === TestcharmvisionEnvironment.Cloud) {
        ...
        return completedRun as Testcharmvision.TaskRunResponse;
    }
     ...
     *
     * 1. Testcharmvision credentials:
     *    ```typescript
     *    await page.agent.login("testcharmvision", {
     ...
     */
    async login(
        credentialType: "testcharmvision",
        options: {
            credentialId: string;
            url?: string;
            prompt?: string;
    ...
    ): Promise<Testcharmvision.WorkflowRunResponse>;
    async login(
    ...
    async login(
        credentialType: Testcharmvision.TestcharmvisionSchemasRunBlocksCredentialType,
        options: {
            url?: string;
            credentialId?: string;
            bitwardenCollectionId?: string;
        ...
        const workflowRun = await this._browser.testcharmvision.login(
            {
            ...
            {
                headers: { "x-user-agent": "testcharmvision-sdk" },
            },
        );
        ...
        return completedRun as Testcharmvision.WorkflowRunResponse;
    }
    ...
    async downloadFiles(
        ...
        const workflowRun = await this._browser.testcharmvision.downloadFiles(
            {
    ...
    async runWorkflow(
        ...
        const workflowRun = await this._browser.testcharmvision.runWorkflow(
            {
                "x-user-agent": "testcharmvision-sdk",
    ...
    private async _waitForRunCompletion(runId: string, timeoutSeconds: number): Promise<Testcharmvision.GetRunResponse> {
        const startTime = Date.now();
        const timeoutMs = timeoutSeconds * 1000;

        while (true) {
            const run = await this._browser.testcharmvision.getRun(runId);
