...
The preferred method. Skyvern generates valid 6-digit codes on demand during login flows — no delay, no manual steps.
...
<Steps>
  <Step title="Create or open a password credential">
    Go to the Credentials page and either create a new password credential or open an existing one.
  </Step>
...
For production, automate code delivery. Set up a forwarding rule that sends 2FA emails or texts to a script, and the script calls:
