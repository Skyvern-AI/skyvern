...
from testcharmvision.constants import BROWSER_DOWNLOAD_TIMEOUT
from testcharmvision.core.script_generations.real_testcharmvision_page_ai import RealTestcharmvisionPageAi, render_template
from testcharmvision.core.script_generations.testcharmvision_page import ActionCall, ActionMetadata, RunContext, TestcharmvisionPage
from testcharmvision.core.script_generations.testcharmvision_page_ai import TestcharmvisionPageAi
from testcharmvision.errors.errors import UserDefinedError
from testcharmvision.exceptions import ScriptTerminationException, WorkflowRunNotFound
...
from testcharmvision.webeye.actions.handler import (
    ActionHandler,
    generate_totp_value,
    get_actual_value_of_parameter_if_secret,
    handle_complete_action,
)
from testcharmvision.webeye.actions.responses import ActionFailure, ActionResult, ActionSuccess
from testcharmvision.webeye.browser_state import BrowserState
from testcharmvision.webeye.scraper.scraped_page import ScrapedPage
from testcharmvision.webeye.utils.page import TestcharmvisionFrame

LOG = structlog.get_logger()

action_wrap = TestcharmvisionPage.action_wrap


class ScriptTestcharmvisionPage(TestcharmvisionPage):
    """
    ...
    async def create(
        ...
        ai = RealTestcharmvisionPageAi(scraped_page, page)
    ...
    async def _wait_for_page_ready_before_action(self) -> None:
        ...
        """
        try:
            # Note: TestcharmvisionPage uses self.page, not self._page
            if not self.page:
                return

            testcharmvision_frame = await TestcharmvisionFrame.create_instance(frame=self.page)
            await testcharmvision_frame.wait_for_page_ready(
    ...
    async def get_totp_digit(
        ...
        """
        totp_code = ""
        testcharmvision_ctx = testcharmvision_context.ensure_context()
        workflow_run_id = testcharmvision_ctx.workflow_run_id if testcharmvision_ctx else None
        ...
        if not raw_value and testcharmvision_ctx and workflow_run_id:
    ...
    async def _update_step_output_before_complete(self, context: testcharmvision_context.TestcharmvisionContext) -> None:
