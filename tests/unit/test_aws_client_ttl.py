...
# Create a dynamic exception class that mirrors what botocore generates at runtime.
# ExpiredTokenException is a subclass of ClientError, created by botocore.errorfactory.
_ExpiredTokenException = type("ExpiredTokenException", (ClientError,), {})

_EXPIRED_TOKEN_ERROR = _ExpiredTokenException(
...
def _make_s3_client_mock(client_obj: AsyncMock) -> MagicMock:
    ...
    # Attach the dynamic exception so `except client.exceptions.ExpiredTokenException` works
    exceptions = MagicMock()
    exceptions.ExpiredTokenException = _ExpiredTokenException
    client_obj.exceptions = exceptions
...
async def test_upload_file_retries_on_expired_token():
    """upload_file should refresh the session and retry once on ExpiredTokenException."""
...
async def test_upload_file_stream_resets_cursor_on_retry():
    ...
    mock_s3 = AsyncMock()
    mock_s3.upload_fileobj = mock_upload
...
@pytest.mark.asyncio
async def test_non_token_error_does_not_retry():
    """Non-token errors (e.g., NoSuchKey) should propagate immediately without retry."""
    mock_get = AsyncMock(side_effect=_NON_TOKEN_ERROR)
    client = aws.get_aws_client()
    ...
    with patch.object(client, "_s3_client", return_value=_make_s3_client_mock(mock_s3)):
        with patch.object(client, "refresh_session") as mock_refresh:
            # download_file catches Exception and returns None, so we verify
            # the error was NOT retried (no refresh, single call)
            result = await client.download_file(uri="s3://test-bucket/missing-key.bin")
            assert result is None
