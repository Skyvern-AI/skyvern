...
| [Data and Extraction](#data-and-extraction) | Text Prompt, File Parser, Split PDF |
...
Split a PDF into smaller sections using a natural-language prompt. Useful for isolating specific pages or ranges (for example, invoices, receipts, or a single chapter) before parsing them.

| Field | Type | Description |
|-------|------|-------------|
| **File URL** | text | URL or parameter reference to the source PDF |
| **Prompt** | text | Natural-language instructions describing which pages or sections to keep |
| **Model** | selector | Which LLM model to use |
