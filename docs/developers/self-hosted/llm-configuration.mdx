...
description: Configure LLM providers for self-hosted Skyvern including OpenAI, xAI Grok, Anthropic, Azure OpenAI, Google Gemini, AWS Bedrock, Ollama, and OpenAI-compatible endpoints via LiteLLM.
...
| **xAI** | `XAI_GROK_4_5` | `XAI_GROK_4_5` | Grok 4.5 |
...
Grok models from [x.ai](https://x.ai/api). Requires an xAI API key.

```bash .env
ENABLE_XAI=true
XAI_API_KEY=xai-...
LLM_KEY=XAI_GROK_4_5
```
...
| LLM_KEY | Notes |
|---------|-------|
| `XAI_GROK_4_5` | Grok 4.5 for coding, agentic tasks, and general chat |
...
# Supported values: low, medium, high. Unset to use xAI's default.
...
[Cheaper Inference](https://cheaperinference.com/) is an OpenAI-compatible LLM gateway. Each model costs 15–60% less than the list price of its lab.

```bash .env
ENABLE_CHEAPER_INFERENCE=true
CHEAPER_INFERENCE_API_KEY=your-cheaper-inference-api-key
LLM_KEY=CHEAPER_INFERENCE_GPT5_4_MINI
```

Get an API key at [cheaperinference.com/signup](https://cheaperinference.com/signup). See the [Cheaper Inference docs](https://cheaperinference.com/docs) for details.

### Available models

| LLM_KEY | Model | Notes |
|---------|-------|-------|
| `CHEAPER_INFERENCE_GPT5_4_MINI` | gpt-5.4-mini | Recommended, vision support, 400K context |
| `CHEAPER_INFERENCE_GPT5_4` | gpt-5.4 | Vision support, 1M context |
