...
async def _get_object_info_safe(storage: "GcsStorage", uri: str) -> dict | None:
    try:
...
class GcsStorage(BaseStorage):
    ...
    def __init__(
        ...
        self.async_client = RealAsyncGcsStorageClient(project_id=project_id)
        self.bucket = bucket or settings.GCS_BUCKET_ARTIFACTS
    ...
    async def retrieve_global_workflows(self) -> list[str]:
        uri = f"gs://{self.bucket}/{settings.ENV}/global_workflows.txt"
    ...
    def _build_base_uri(self, organization_id: str) -> str:
        return f"gs://{self.bucket}/{self._PATH_VERSION}/{settings.ENV}/{organization_id}"
    ...
    async def store_artifact(self, artifact: Artifact, data: bytes) -> None:
        ...
        await self.async_client.upload_file(artifact.uri, data, storage_class=storage_class, tags=tags)

    async def _get_storage_class_for_org(self, organization_id: str | None) -> str:
    ...
    async def store_browser_session(self, organization_id: str, workflow_permanent_id: str, directory: str) -> None:
        ...
        browser_session_uri = (
            f"gs://{settings.GCS_BUCKET_BROWSER_SESSIONS}/{settings.ENV}/{organization_id}/{workflow_permanent_id}.zip"
        )
        storage_class = await self._get_storage_class_for_org(organization_id)
        tags = await self._get_tags_for_org(organization_id)
        LOG.debug(
        ...
        await self.async_client.upload_file_from_path(
            browser_session_uri, zip_file_path, storage_class=storage_class, tags=tags
        )
    ...
    async def list_downloaded_files_in_browser_session(
        ...
        """Return GCS URIs of completed downloads. DB-backed; mirrors s3.py."""
        ...
        *,
        ...
        if settings.ARTIFACT_CONTENT_HMAC_KEYRING:
            try:
                ...
                LOG.warning(
                    "Failed to list browser-session download artifacts; falling back to GCS LIST",
                    organization_id=organization_id,
                    browser_session_id=browser_session_id,
        ...
        uri = f"gs://{settings.GCS_BUCKET_ARTIFACTS}/v1/{settings.ENV}/{organization_id}/browser_sessions/{browser_session_id}/downloads"
        files = [f"gs://{settings.GCS_BUCKET_ARTIFACTS}/{file}" for file in await self.async_client.list_files(uri=uri)]
    ...
    async def get_shared_downloaded_files_in_browser_session(
        ...
        if settings.ARTIFACT_CONTENT_HMAC_KEYRING:
            try:
                ...
                LOG.warning(
                    "Failed to look up browser-session download artifacts; falling back to signed URLs",
                    organization_id=organization_id,
                    browser_session_id=browser_session_id,
                    exc_info=True,
                )
    ...
    async def _get_shared_downloaded_files_in_browser_session_via_listing(
        ...
        # Direct GCS LIST: legacy fallback for sessions pre-cutover.
        # ``list_downloaded_files_in_browser_session`` is now DB-backed, so
        # we can't reuse it here.
        listing_uri = f"gs://{settings.GCS_BUCKET_ARTIFACTS}/v1/{settings.ENV}/{organization_id}/browser_sessions/{browser_session_id}/downloads"
        object_keys = [
            f"gs://{settings.GCS_BUCKET_ARTIFACTS}/{file}"
        ...
        for key in object_keys:
            ...
            # Get signed URL
            signed_urls = await self.async_client.create_signed_urls([key])
            if not signed_urls:
                continue
    ...
    async def list_downloading_files_in_browser_session(
        ...
        """Return GCS URIs of in-progress (``*.crdownload``) downloads. DB-backed."""
        return await self._list_downloads_for_session(
            organization_id=organization_id,
            browser_session_id=browser_session_id,
            in_progress=True,
    ...
    async def get_shared_recordings_in_browser_session(
        ...
        if settings.ARTIFACT_CONTENT_HMAC_KEYRING:
            try:
                ...
                LOG.warning(
                    "Failed to look up browser-session recording artifacts; falling back to signed URLs",
                    organization_id=organization_id,
                    browser_session_id=browser_session_id,
                    exc_info=True,
                )
    ...
    async def _get_shared_recordings_in_browser_session_via_listing(
        ...
        # Direct GCS LIST: legacy fallback for pre-cutover sessions and
        # OSS deployments without a keyring.
        listing_uri = f"gs://{settings.GCS_BUCKET_ARTIFACTS}/v1/{settings.ENV}/{organization_id}/browser_sessions/{browser_session_id}/videos"
        object_keys = [
            f"gs://{settings.GCS_BUCKET_ARTIFACTS}/{file}"
            for file in await self.async_client.list_files(uri=listing_uri)
        ]
        ...
        # Prefer the newest recording first (GCS list order is not guaranteed).
    ...
    async def save_downloaded_files(
        ...
        tags = await self._get_tags_for_org(organization_id)
        base_uri = (
            f"gs://{settings.GCS_BUCKET_UPLOADS}/{DOWNLOAD_FILE_PREFIX}/{settings.ENV}/{organization_id}/{run_id}"
        )
    ...
    async def _save_downloaded_files_from_local(
        ...
        for file in files:
            ...
            # GCS custom metadata values must be ASCII (sent as HTTP headers);
            # preserve the full filename via the object path / Artifact URI instead.
            ...
            try:
                ...
                LOG.warning(
                    "Skipping downloaded file — GCS upload failed",
                    file=file,
                    organization_id=organization_id,
                    run_id=run_id,
                    exc_info=True,
            ...
            # the signed /v1/artifacts/{id}/content endpoint. Persist the SHA-256 we already
            # computed so retrieval doesn't need an extra object HEAD per file.
            if run_id is not None:
                try:
                    ...
                    LOG.warning(
                        "Failed to register downloaded file as artifact; falling back to signed URLs for retrieval",
                        file=file,
                        organization_id=organization_id,
                        run_id=run_id,
                        exc_info=True,
    ...
    async def _list_download_artifacts_safe(self, *, organization_id: str, run_id: str) -> list[Artifact]:
        try:
            ...
            LOG.warning(
                "Failed to look up download artifacts; falling back to signed URLs",
                organization_id=organization_id,
                run_id=run_id,
                exc_info=True,
            )
    ...
    async def _get_downloaded_files_via_blob_listing(
        ...
        file_infos: list[FileInfo] = []
        for key in object_keys:
            object_uri = f"gs://{settings.GCS_BUCKET_UPLOADS}/{key}"
    ...
    async def save_legacy_file(
        ...
        bucket = settings.GCS_BUCKET_UPLOADS
        storage_class = await self._get_storage_class_for_org(organization_id)
        tags = await self._get_tags_for_org(organization_id)
        # First try uploading with original filename
        try:
            sanitized_filename = os.path.basename(filename)  # Remove any path components
            gcs_uri = f"gs://{bucket}/{settings.ENV}/{organization_id}/{todays_date}/{sanitized_filename}"
            uploaded_uri = await self.async_client.upload_file_stream(
                gcs_uri, fileObj, storage_class=storage_class, tags=tags
            )
        except Exception:
            LOG.error("Failed to upload file to GCS", exc_info=True)
            uploaded_uri = None

        # If upload fails, try again with UUID prefix
        if not uploaded_uri:
            uuid_prefixed_filename = f"{str(uuid.uuid4())}_{filename}"
            gcs_uri = f"gs://{bucket}/{settings.ENV}/{organization_id}/{todays_date}/{uuid_prefixed_filename}"
        ...
        if not uploaded_uri:
            LOG.error(
                "Failed to upload file to GCS after retrying with UUID prefix",
                organization_id=organization_id,
                storage_class=storage_class,
                filename=filename,
                exc_info=True,
        ...
        signed_urls = await self.async_client.create_signed_urls([uploaded_uri])
        if not signed_urls:
            LOG.error(
    ...
    def _build_browser_session_uri(
        ...
        """Build the GCS URI for a browser session file."""
        base = f"gs://{self.bucket}/{self._PATH_VERSION}/{settings.ENV}/{organization_id}/browser_sessions/{browser_session_id}/{artifact_type}"
        if date:
    ...
    async def delete_browser_session_file(
        ...
        """Delete a file from browser session storage in GCS.
        ...
        if artifact_type == "downloads":
            try:
                ...
                LOG.warning(
                    "Failed to delete browser-session download artifact row; proceeding with GCS delete",
                    organization_id=organization_id,
                    browser_session_id=browser_session_id,
    ...
    async def browser_session_file_exists(
        ...
        """Check if a file exists in browser session storage in GCS."""
        uri = self._build_browser_session_uri(organization_id, browser_session_id, artifact_type, remote_path, date)
        try:
    ...
    def assert_managed_file_access(self, uri: str, organization_id: str) -> None:
        try:
            parsed_uri = GcsUri(uri)
        except Exception as e:
            raise PermissionError(f"No permission to access storage URI: {uri}") from e

        # Uploads bucket: object paths use {env}/{org}/ or downloads/{env}/{org}/
        if parsed_uri.bucket == settings.GCS_BUCKET_UPLOADS:
            allowed_prefixes = (
    ...
    def storage_type(self) -> str:
        """Returns 'gcs' as the storage type."""
        return "gcs"
