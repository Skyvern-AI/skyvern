...
subtitle: Create multi-step automations with the visual agent editor
description: Create multi-step browser automations in the Skyvern Cloud UI using the visual agent editor with blocks for navigation, extraction, loops, conditionals, and email.
...
Agents are multi-step automations built visually in the Cloud UI. You drag blocks onto a canvas, wire them together, configure each one, and run the whole sequence with a click. No code required. Agents can be versioned, shared across your team, scheduled on a cron, and run with different parameters each time.

If you prefer writing automation code in Python or TypeScript, see [Browser Automation](/developers/browser-automations/overview) in the developer docs. Both approaches handle multi-step work across pages. Agents are the visual path, browser automations are the code path.

<Info>
**Agents vs. Browser Automation:** Agents are defined in this visual editor and run from the Cloud UI (or [triggered from code](/cloud/building-agents/run-from-code)). Browser automations are written in Python/TypeScript using Page, Agent, and Browser. Choose based on whether your team prefers a visual editor or a code editor.
</Info>

This page explains how agents work, walks you through building one from scratch, and covers the editor's features.

## How agents work

An agent is a sequence of **blocks** that run one after another. Each block does one thing: navigate to a page, extract data, send an email, run custom code, loop, or branch. You connect blocks on a visual canvas, pass data between them, and run the whole chain with one click.
...
| **Control Flow** | Branch, loop, validate, or pause | Loop, While Loop, Conditional, AI Validation, Code, Wait |
...
See [Block Reference](/cloud/building-agents/configure-blocks) for every block type and its configuration fields.
...
Let's build a real agent. You'll create an automation that finds the top 5 job listings from Hacker News's monthly "Who is Hiring" thread, summarizes each one, and emails you a digest.
...
<Steps>
  <Step title="Create a new agent">
    ...
    <img src="/images/cloud/create-workflow-dropdown.png" alt="Create agent dropdown showing Blank Agent and From Template options" />
  ...
  <Step title="Add a Loop with a Text Prompt inside">
    ...
    |-------|-------|
    | **Label** | `process_each` |
    | **Loop Value** | `{{ scrape_listings_output.extracted_information.listings }}` |
  ...
  <Step title="Save and run">
    ...
    <Warning>
    Agents are **not** auto-saved. Click the **Save** button (disk icon) in the header bar before navigating away or you'll lose your work.
    </Warning>

    Click **Save**, then click **Run** in the top right. A parameters page opens where you can review and confirm the agent's settings. Click **Run Agent** to start execution.
...
Five blocks, three categories, zero code. You can now re-run this agent anytime, tweak the extraction schema to pull different fields, swap the email for an HTTP Request to post to Slack, or add a Conditional block to only email when listings match certain criteria.
...
<img src="/images/cloud/workflow-editor-overview.png" alt="Agent editor overview" />

| Area | What it shows |
|------|---------------|
| **Header bar** (top) | Agent title (click to rename), Parameters button, Save button, Template toggle, History button, Run button |
...
The menu also offers **Record Browser** (record actions in a live browser to generate blocks) and **Upload SOP** (upload a PDF procedure to auto-generate an agent).

When you create a blank agent, hover over the **+** on the start edge to add your first block.
...
See [Block Reference](/cloud/building-agents/configure-blocks) for the full list of fields on each block type.
...
**Loop blocks** contain child blocks that repeat for each item in the loop's list. **While Loop blocks** contain child blocks that repeat while their condition remains true.
...
<Warning>
Agents are **not** auto-saved. Click the **Save** button (disk icon) in the header bar to persist your changes. The editor warns you if you try to navigate away with unsaved work.
</Warning>
...
| Button | Description |
|--------|-------------|
| **Save** (disk icon) | Save the current agent definition |
| **Template** (bookmark icon) | Toggle whether this agent is saved as an organization template |
| **History** (clock icon) | View past runs of this agent |
| **Schedule** (clock icon) | Create or manage recurring schedules for this agent (see [Create a Schedule](/cloud/building-agents/scheduling)) |
| **Parameters** | Open the parameters panel (see [Agent Parameters](/cloud/building-agents/add-parameters)) |
| **Run** (play icon) | Save and start a new execution (see [Running Agents](/cloud/building-agents/run-an-agent)) |
