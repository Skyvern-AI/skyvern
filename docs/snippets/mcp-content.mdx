...
| -------------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| **Best for**         | Claude Code, Claude.ai / Claude Desktop connectors, Codex CLI, Cursor, ChatGPT, and other clients that support MCP OAuth | Claude Desktop, Windsurf, Hermes, OpenClaw, headless/CI flows, or any client where you prefer a static key | Self-hosting Skyvern                                |
...
### Narrowing the hosted tool surface

Skyvern Cloud exposes the full MCP tool surface at `https://api.skyvern.com/mcp/` by default. To give a hosted client a narrower tool list, use a scoped URL such as `https://api.skyvern.com/mcp/x/operate`, or keep the normal URL and send `X-Skyvern-Scope: operate`. Supported hosted scopes are `operate`, `build`, `browser`, and `lean`; omit the scope, or use `all`, for the full surface.

## Option A: Sign in with browser (OAuth)

Supported clients today: **Claude Code**, **Claude.ai / Claude Desktop** (custom connector), **Codex CLI**, **Cursor**, **ChatGPT** (custom connector), and other MCP clients that support remote HTTP servers with OAuth. Skyvern Cloud publishes standard MCP OAuth discovery metadata, so supported clients can start browser sign-in without a static `CLIENT_ID` or `CLIENT_SECRET` in your config.
...
1. In Claude or Claude Desktop, open **Settings → Connectors**.
2. Click **Add custom connector**.
3. Enter `Skyvern` as the connector name and `https://api.skyvern.com/mcp/` as the connector URL.
4. Click **Add**, then **Connect**, and complete the browser sign-in with your Skyvern account.
5. Enable Skyvern from the **Search and tools** menu in a Claude chat.

No API key is required — OAuth handles authentication. This remote connector is separate from the local Claude Desktop extension install described in the API-key section below.
...
Use this path for Windsurf, Hermes, OpenClaw, headless/CI flows, and any client that does not yet support the MCP OAuth flow. You can also use it with Claude Code, Claude Desktop, Codex CLI, or Cursor if you prefer a static key (for example, on a server with no browser available for the OAuth callback).
...
Put your API key in the `env` block so it is not exposed on the process command line.
...
<Tip>
  Most Codex users should use the OAuth flow above (`codex mcp add skyvern --url
  https://api.skyvern.com/mcp/`). Use this manual TOML path only when you cannot
  complete a browser callback, for example on a headless server or in a CI
  runner.
</Tip>
...
<Tip>
  OpenClaw accepts JSON5 syntax in `openclaw.json`. `skyvern setup openclaw` can
  read that format, but when it rewrites the file it normalizes the config back
  to standard JSON, so comments and JSON5-only formatting are not preserved.
</Tip>
...
<Tip>
  Prefer a CLI? For Skyvern Cloud, `pip install skyvern` then run `skyvern
  setup`. For the local self-hosted path, use Docker Compose or a source
  checkout, then run `skyvern quickstart` or `skyvern init` and choose Claude
  Code during the MCP step.
</Tip>
...
- let you choose a local stdio tool scope (`all` by default)
...
The configs below serve the full default tool surface. To narrow an install to one job, append `"--scope", "operate"` (or `build` / `browser` / `lean`) to the `args` array — omitting the flag is identical to `--scope all`.

| Scope | Tools | Use when |
| --- | ---: | --- |
| `all` | 114 | Full tool surface; default local dev/QA choice |
| `operate` | 29 | Run, monitor, and inspect existing automations |
| `build` | 60 | Author and modify automations |
| `browser` | 54 | Drive a browser directly |
| `lean` | 32 | Focused browser surface with direct actions, selector-scoped page reading, and schema-constrained output |
...
<Tip>
  From Docker Compose or a source checkout, `skyvern init` and `skyvern
  quickstart` can write the local MCP config for you. In a project directory,
  choosing Claude Code defaults to project-local `.mcp.json` and installs
  `.claude/skills/qa`.
</Tip>
...
<video style={{ aspectRatio: "16 / 9", width: "100%" }} controls>
  <source
    src="https://github.com/user-attachments/assets/0c10dd96-c6ff-4b99-ad99-f34a5afd04fe"
    type="video/mp4"
  />
</video>
...
<video style={{ aspectRatio: "16 / 9", width: "100%" }} controls>
  <source
    src="https://github.com/user-attachments/assets/084c89c9-6229-4bac-adc9-6ad69b41327d"
    type="video/mp4"
  />
</video>
...
<video style={{ aspectRatio: "16 / 9", width: "100%" }} controls>
  <source
    src="https://github.com/user-attachments/assets/70cfe310-24dc-431a-adde-e72691f198a7"
    type="video/mp4"
  />
</video>
...
To drive the Chrome profile and tabs you already use from a local stdio MCP server, install the
[Skyvern Chrome extension](/developers/optimization/chrome-extension). You explicitly choose which tabs to share, and
the connection stays on your machine.
...
<Tip>
  This is especially powerful during development: make a code change, then ask
  your AI assistant to navigate to `localhost:3000` and verify it works. Fix
  code and validate in the same prompt.
</Tip>

<Warning>
  Always use `--api-key` when exposing your browser via tunnel. Without it,
  anyone with the ngrok URL has full browser control.
</Warning>
...
For the local self-hosted path, use Docker Compose or a Skyvern source checkout, then run `skyvern quickstart` or `skyvern init`, choose `local`, choose the default Skyvern tool scope (`all`), then choose `Claude Code` during the MCP step. That writes local stdio MCP config and installs Claude Code skills, including `/qa`, which reads your git diff, generates browser tests, and runs them against your dev server.
...
<Tip>
  The `/qa` skill uses `skyvern_evaluate` for fast DOM assertions (~10ms each)
  and `skyvern_act` for natural-language interactions. Tests are generated fresh
  from each diff, so there are no test files to maintain.
</Tip>
...
<Accordion title="OAuth session expires">
  OAuth sessions use the access-token lifetime described in the OAuth setup
  section above. When authentication expires, re-run the MCP client's
  authentication flow. To stop using Skyvern from a client before then, clear
  that client's saved Skyvern authentication or remove the Skyvern MCP server
  entry.
</Accordion>
