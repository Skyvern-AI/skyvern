...
description: Create and manage persistent browser sessions in the Skyvern Cloud UI to preserve cookies, login state, and page context across multiple tasks and agents.
...
|---------|-------------|
| **Timeout** | How long the session stays open, in minutes. Min: 5; max: 240 (4 hours) — requests above 240 are capped at 240 with a warning. A running session can be extended through the API up to 360 minutes (6 hours); see [extend_browser_session](/sdk-reference/browser-sessions/extend-browser-session). Longer durations are available on the enterprise plan (contact sales@skyvern.com). Default: 60. |
...
<img src="/images/cloud/workflow-run-session-id-field.png" alt="Agent run settings panel with a pbs_ session ID entered in the Browser Session ID field" />
