...
from testcharmvision.forge.sdk.models import TokenPayload
from testcharmvision.forge.sdk.schemas.organizations import Organization, OrganizationAuthToken, OrganizationAuthTokenType
...
async def get_current_org(
    x_api_key: Annotated[
        str | None,
        Header(
            description="Testcharmvision API key for authentication. API key can be found at https://app.testcharmvision.com/settings."
        ),
    ] = None,
...
async def _authenticate_helper(authorization: str) -> Organization:
    ...
    # set organization_id in testcharmvision context and log context
...
async def get_current_user_id(
    ...
    # Fall back to API key + testcharmvision-ui user agent
    if x_api_key and x_user_agent == "testcharmvision-ui":
        organization = await _get_current_org_cached(x_api_key, app.DATABASE)
        if organization:
...
async def resolve_org_from_api_key(
    ...
    if api_key_db_obj.valid is False:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Your API key has expired. Please retrieve the latest one from https://app.testcharmvision.com/settings",
        )
