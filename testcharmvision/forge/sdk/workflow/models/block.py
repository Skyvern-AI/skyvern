...
from testcharmvision.forge.sdk.api import email
from testcharmvision.forge.sdk.api.aws import AsyncAWSClient
...
from testcharmvision.forge.sdk.core.aiohttp_helper import aiohttp_request
from testcharmvision.forge.sdk.db.enums import TaskType
from testcharmvision.forge.sdk.db.exceptions import NotFoundError
from testcharmvision.forge.sdk.experimentation.llm_prompt_config import get_llm_handler_for_prompt_type
from testcharmvision.forge.sdk.schemas.files import FileInfo
from testcharmvision.forge.sdk.schemas.task_v2 import TaskV2Status
from testcharmvision.forge.sdk.schemas.tasks import Task, TaskOutput, TaskStatus
...
from testcharmvision.forge.sdk.utils.pdf_parser import extract_pdf_file, validate_pdf_file
from testcharmvision.forge.sdk.workflow.context_manager import BlockMetadata, WorkflowRunContext
...
from testcharmvision.forge.sdk.workflow.models.parameter import (
    PARAMETER_TYPE,
    AWSSecretParameter,
    ContextParameter,
    OutputParameter,
...
from testcharmvision.schemas.runs import RunEngine
from testcharmvision.schemas.workflows import BlockResult, BlockStatus, BlockType, FileStorageType, FileType
from testcharmvision.utils.strings import generate_random_string
from testcharmvision.utils.templating import get_missing_variables
from testcharmvision.utils.url_validators import prepend_scheme_and_validate_url
...
_JSON_TYPE_MARKER = "__TESTCHARMVISION_RAW_JSON__"
        ...
        **kwargs: dict,
        ...
        for parameter in self.parameters:
            ...
            if not parameter.parameter_type.is_secret_or_credential() and not (
                # NOTE: testcharmvision credential is a 'credential_id' workflow parameter type
            ...
            if isinstance(value, dict):
                ...
                for credential_field, credential_place_holder in value.items():
                    # "context" is a testcharmvision-defined field to reduce LLM hallucination
                    if credential_field == "context":
                        continue
...
class HumanInteractionBlock(BaseTaskBlock):
    ...
    # email options
    sender: str = "hello@testcharmvision.com"
    recipients: list[str] = []
    ...
    async def execute(
        ...
        **kwargs: dict,
        ...
        from testcharmvision.forge.sdk.workflow.models.workflow import WorkflowRunStatus  # noqa: PLC0415
        ...
        app_url = f"{settings.TESTCHARMVISION_APP_URL}/runs/{workflow_run_id}/overview"
...
class TaskV2Block(Block):
    ...
    async def execute(
        ...
        **kwargs: dict,
        ...
        from testcharmvision.forge.sdk.workflow.models.workflow import WorkflowRunStatus  # noqa: PLC0415
        from testcharmvision.services import task_v2_service  # noqa: PLC0415
        ...
        if not resolved_url:
            ...
            if browser_state:
                page = await browser_state.get_working_page()
                if page:
                    current_url = await TestcharmvisionFrame.get_url(frame=page)
        ...
        try:
            ...
            context: testcharmvision_context.TestcharmvisionContext | None = testcharmvision_context.current()
            ...
            testcharmvision_context.set(
                testcharmvision_context.TestcharmvisionContext(
                    organization_id=organization_id,
