...
async def test_copilot_advertised_frame_tool_chain_real_browser(
    ...
    assert any(frame["name"] == "payment" for frame in listed["data"]["frames"])

    entered = await _copilot_payload(server, "skyvern_frame_switch", {"selector": "#pay-frame"})
    assert entered["ok"] is True
    assert entered["data"]["frame_name"] == "payment"

    card_value_expression = "document.querySelector('#pay-frame').contentDocument.querySelector('#card').value"
    before = await _copilot_payload(server, "evaluate", {"expression": card_value_expression})
    typed = await _copilot_payload(server, "type_text", {"selector": "#card", "text": "4242"})
    after = await _copilot_payload(server, "evaluate", {"expression": card_value_expression})
    assert before["ok"] is True, before
    assert after["ok"] is True, after
    assert before["data"]["result"] == ""
    assert typed["ok"] is True
    assert after["data"]["result"] == "4242"
