...
<Info>
`pip install skyvern` is the lightweight Python SDK for Skyvern Cloud and remote API usage. If you are using embedded local mode with `Skyvern.local()` or `launch_local_browser()`, install the local extra: `pip install "skyvern[local]"`. For a local Skyvern server with the packaged web UI, use `pip install "skyvern[all]"`. For advanced partial installs, `skyvern[server]` installs only the local server stack and `skyvern[ui]` installs only the packaged UI assets.
</Info>
...
**Prerequisites:**
- Skyvern local server + packaged UI extra installed (`pip install "skyvern[all]"`)
- If you intentionally installed only `skyvern[server]`, add the packaged UI with `pip install "skyvern[ui]"`. If it is missing, `skyvern run ui` can install it interactively; use `skyvern run ui --install-ui` for non-interactive UI-only setup.
- An LLM API key (OpenAI, Anthropic, Azure OpenAI, Gemini, Ollama, or any OpenAI-compatible provider)

<Note>
The pip quickstart uses SQLite at `~/.skyvern/data.db` by default, so Docker and Postgres are optional. Use `skyvern quickstart --postgres` to start a local Postgres container, or `skyvern quickstart --database-string=...` for an existing database.
</Note>
...
1. Set up your database (SQLite by default, Postgres if requested)
