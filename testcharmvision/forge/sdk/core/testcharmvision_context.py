...
class TestcharmvisionContext:
    ...
    def __repr__(self) -> str:
        return f"TestcharmvisionContext(request_id={self.request_id}, organization_id={self.organization_id}, task_id={self.task_id}, step_id={self.step_id}, workflow_id={self.workflow_id}, workflow_run_id={self.workflow_run_id}, task_v2_id={self.task_v2_id}, max_steps_override={self.max_steps_override}, run_id={self.run_id})"
...
_context: ContextVar[TestcharmvisionContext | None] = ContextVar(
    "Global context",
    default=None,
)


def current() -> TestcharmvisionContext | None:
    """
...
def ensure_context() -> TestcharmvisionContext:
    """
    ...
    context = current()
    if context is None:
        raise RuntimeError("No testcharmvision context")
    return context


def set(context: TestcharmvisionContext) -> None:
    """
