...
function LoginBlockCredentialSelector({ nodeId, value, onChange }: Props) {
  ...
  // 1. Testcharmvision credential parameters (have credentialId)
  ...
  const selectedCredentialId = useMemo(() => {
    ...
    if (credentialParam && parameterIsTestcharmvisionCredential(credentialParam)) {
  ...
  // Only show non-Testcharmvision credential parameters (Bitwarden, 1Password, Azure Vault)
  // Testcharmvision credential parameters should never be shown - the actual credential is displayed directly
  const externalVaultParameterOptions = credentialParameters
    .filter((parameter) => {
      // Never show Testcharmvision credential parameters
      if (parameterIsTestcharmvisionCredential(parameter)) {
        return false;
      }
    ...
    <>
      <Select
        ...
        onValueChange={(newValue) => {
          ...
          // Check if current value references a Testcharmvision credential
          const currentParameter = workflowParameters.find((parameter) => {
            if (parameter.parameterType !== "credential") return false;
            if (!parameterIsTestcharmvisionCredential(parameter)) return false;
          ...
          if (selectedCredential) {
            ...
            const existingParameter = newParameters.find((parameter) => {
              return (
                parameter.parameterType === "credential" &&
                parameterIsTestcharmvisionCredential(parameter) &&
          ...
          // If user selected a parameter (non-Testcharmvision credential or input parameter)
