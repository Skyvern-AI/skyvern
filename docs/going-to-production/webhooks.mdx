...
This page explains how to set up webhooks, understand the payload, verify signatures, and retry failed deliveries.
...
Skyvern signs every webhook with your API key using HMAC-SHA256. Verify the signature to ensure the request is authentic.
...
Skyvern does not automatically retry failed webhook deliveries. If your server was down or returned an error, replay the webhook manually:
...
| Issue | Reason | How to Fix |
|-------|--------|------------|
| Webhook not received | Server unreachable, firewall blocking, URL incorrect, or delivery timed out | 1. Check your server logs for incoming requests. 2. Verify the URL is publicly accessible (not localhost). 3. Poll `get_run` to retrieve the data and check `webhook_failure_reason` for the specific error. |
| Signature validation fails | Raw payload bytes don't match what was signed, or API key mismatch | Use the raw request body for HMAC validation, not parsed/re-serialized JSON. Verify you're using the same API key that created the run. |
| Webhook times out | Server took longer than 30 seconds to respond | Return `200 OK` immediately upon receiving the request, then process the payload asynchronously in a background job. |
