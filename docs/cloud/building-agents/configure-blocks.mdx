...
With Google and a single `site:example.com` or `site:example.com/path` filter, the block removes results outside the filter. The block repeats the search up to two times when Google returns only results outside the filter. The block does not remove results when the query has quotation marks, parentheses, `|`, `OR`, or more than one `site:` filter.
...
Leave Prompt blank to make no LLM calls. When supplied, Prompt receives only the normalized search results. It runs on an empty list unless you set No Results Error Code. It cannot fetch result pages. Exa supports a single positive `site:domain` restriction; unsupported site expressions fail instead of expanding the search. With Exa, a result's `snippet` is empty when Exa has no cached copy of the page or when the request for page highlights fails.
