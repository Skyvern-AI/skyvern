...
| Field | Type | Description |
|-------|------|-------------|
| **Export as Parquet** | switch | Also write the extracted data to a downloadable Parquet file. No separate Data Export block needed. |
| **Data Schema** | JSON | [JSON Schema](https://json-schema.org/) for the exported records. When you first enable export, Skyvern seeds it from the block's extraction schema. |
| **Filename** | text | Optional custom name for the exported file |
| **Records** | text | Optional. Defaults to this block's own extracted data. Set it only to export something else, using `{{ parameter_name }}` to reference it. |
