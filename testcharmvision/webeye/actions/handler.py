...
from testcharmvision.constants import (
    AUTO_COMPLETION_POTENTIAL_VALUES_COUNT,
    BROWSER_DOWNLOAD_MAX_WAIT_TIME,
    BROWSER_DOWNLOAD_TIMEOUT,
    DROPDOWN_MENU_MAX_DISTANCE,
    TESTCHARMVISION_ID_ATTR,
)
from testcharmvision.errors.errors import TOTPExpiredError
from testcharmvision.exceptions import (
    EmptySelect,
    ErrEmptyTweakValue,
...
from testcharmvision.experimentation.wait_utils import get_or_create_wait_config, get_wait_time
from testcharmvision.forge import app
from testcharmvision.forge.prompts import prompt_engine
from testcharmvision.forge.sdk.api.files import (
    check_downloading_files_and_wait_for_download_to_complete,
    get_download_dir,
    list_files_in_directory,
)
from testcharmvision.forge.sdk.api.llm.api_handler_factory import LLMAPIHandlerFactory, LLMCallerManager
from testcharmvision.forge.sdk.api.llm.exceptions import LLMProviderError
from testcharmvision.forge.sdk.api.llm.schema_validator import validate_and_fill_extraction_result
from testcharmvision.forge.sdk.core import testcharmvision_context
from testcharmvision.forge.sdk.core.testcharmvision_context import current as testcharmvision_current
from testcharmvision.forge.sdk.core.testcharmvision_context import ensure_context
from testcharmvision.forge.sdk.models import Step
from testcharmvision.forge.sdk.schemas.tasks import Task
from testcharmvision.forge.sdk.services.bitwarden import BitwardenConstants
from testcharmvision.forge.sdk.services.credentials import AzureVaultConstants, OnePasswordConstants
from testcharmvision.forge.sdk.trace import traced
from testcharmvision.services import service_utils
from testcharmvision.services.action_service import get_action_history
from testcharmvision.utils.prompt_engine import (
    CheckDateFormatResponse,
    CheckPhoneNumberFormatResponse,
    load_prompt_with_elements,
)
from testcharmvision.webeye.actions import actions, handler_utils
from testcharmvision.webeye.actions.action_types import ActionType
from testcharmvision.webeye.actions.actions import (
    Action,
    ActionStatus,
    CheckboxAction,
    ClickAction,
...
from testcharmvision.webeye.actions.responses import ActionAbort, ActionFailure, ActionResult, ActionSuccess
from testcharmvision.webeye.browser_factory import initialize_download_dir
from testcharmvision.webeye.scraper.scraped_page import (
...
from testcharmvision.webeye.scraper.scraper import IncrementalScrapePage, hash_element, trim_element_tree
from testcharmvision.webeye.utils.dom import COMMON_INPUT_TAGS, DomUtil, InteractiveElement, TestcharmvisionElement
from testcharmvision.webeye.utils.page import TestcharmvisionFrame

LOG = structlog.get_logger()


class CustomSingleSelectResult:
    def __init__(self, testcharmvision_frame: TestcharmvisionFrame) -> None:
        self.reasoning: str | None = None
        self.action_result: ActionResult | None = None
        self.action_type: ActionType | None = None
        self.value: str | None = None
        self.dropdown_menu: TestcharmvisionElement | None = None
        self.testcharmvision_frame = testcharmvision_frame
    ...
    async def is_done(self) -> bool:
        ...
        return not await self.testcharmvision_frame.get_element_visible(await self.dropdown_menu.get_element_handler())
...
def is_ul_or_listbox_element_factory(
    ...
    async def wrapper(element_dict: dict) -> bool:
        ...
        try:
            element = await TestcharmvisionElement.create_from_incremental(incremental_scraped, element_id)
        except Exception:
            LOG.debug(
...
def check_existed_but_not_option_element_in_dom_factory(
    ...
    async def helper(element_dict: dict, frame: Page | Frame) -> bool:
        ...
        if not element_id:
            return False
        try:
            locator = frame.locator(f"[{TESTCHARMVISION_ID_ATTR}={element_id}]")
            current_element = TestcharmvisionElement(locator=locator, frame=frame, static_element=element_dict)
...
def check_disappeared_element_id_in_incremental_factory(
    ...
    async def helper(element_dict: dict, frame: Page | Frame) -> bool:
        ...
        try:
            testcharmvision_element = await TestcharmvisionElement.create_from_incremental(
                incre_page=incremental_scraped, element_id=element_id
            )
        except Exception:
            LOG.debug(
                "Failed to create testcharmvision element, going to drop the element from incremental tree",
                exc_info=True,
                element_id=element_id,
            )
            return True

        testcharmvision_frame = incremental_scraped.testcharmvision_frame
        return not await testcharmvision_frame.get_element_visible(await testcharmvision_element.get_element_handler())
...
async def check_phone_number_format(
    ...
    testcharmvision_element: TestcharmvisionElement,
    ...
    LOG.info(
        ...
        action=action,
        element_id=testcharmvision_element.get_id(),
    )

    new_scraped_page = await scraped_page.generate_scraped_page_without_screenshots()
    html = new_scraped_page.build_element_tree(html_need_testcharmvision_attrs=False)
    prompt = prompt_engine.load_prompt(
        ...
        elements=html,
        local_datetime=datetime.now(testcharmvision_context.ensure_context().tz_info).isoformat(),
    )
...
class ActionHandler:
    ...
    async def handle_action(
        ...
        context = testcharmvision_context.current()
        download_dir = Path(
            get_download_dir(
...
async def handle_click_action(
    ...
    if action.x is not None and action.y is not None:
        ...
        if element_id:
            if testcharmvision_element := await dom.safe_get_testcharmvision_element_by_id(element_id):
                if await testcharmvision_element.navigate_to_a_href(page=page):
                    return [ActionSuccess()]
    ...
    testcharmvision_element = await dom.get_testcharmvision_element_by_id(action.element_id)
    ...
    if await testcharmvision_element.is_disabled(dynamic=True):
        LOG.warning(
            "Try to click on a disabled element",
            action_type=action.action_type,
            element_id=testcharmvision_element.get_id(),
        )
        return [ActionFailure(InteractWithDisabledElement(testcharmvision_element.get_id()))]

    try:
        await testcharmvision_element.scroll_into_view()
    except Exception:
        LOG.info(
    ...
    if action.download:
        ...
        try:
            testcharmvision_frame = await TestcharmvisionFrame.create_instance(testcharmvision_element.get_frame())
            incremental_scraped = IncrementalScrapePage(testcharmvision_frame=testcharmvision_frame)
            await incremental_scraped.start_listen_dom_increment(await testcharmvision_element.get_element_handler())

            has_onclick_attr = await testcharmvision_element.has_attr("onclick", mode="static")
            results = await chain_click(
                task,
                scraped_page,
                page,
                action,
                testcharmvision_element,
            ...
            try:
                if has_onclick_attr:
                    ...
                    await testcharmvision_frame.safe_wait_for_animation_end(before_wait_sec=1)

                if sequential_click_result := await handle_sequential_click_for_dropdown(
                    action=action,
                    action_history=results,
                    anchor_element=testcharmvision_element,
                    dom=dom,
                    page=page,
                    testcharmvision_frame=testcharmvision_frame,
                    scraped_page=scraped_page,
...
async def handle_sequential_click_for_dropdown(
    ...
    anchor_element: TestcharmvisionElement,
    dom: DomUtil,
    page: Page,
    testcharmvision_frame: TestcharmvisionFrame,
    scraped_page: ScrapedPage,
    incremental_scraped: IncrementalScrapePage,
    task: Task,
    step: Step,
    ...
    await testcharmvision_frame.safe_wait_for_animation_end()
    ...
    new_interactable_element_ids = [
        ...
        if (await dom_after_open.get_testcharmvision_element_by_id(element_id)).is_interactable()
    ]
    ...
    dropdown_select_context = await _get_input_or_select_context(
        ...
        testcharmvision_element=anchor_element,
...
async def handle_click_to_download_file_action(
    ...
    dom = DomUtil(scraped_page=scraped_page, page=page)
    testcharmvision_element = await dom.get_testcharmvision_element_by_id(action.element_id)
    locator = testcharmvision_element.locator

    try:
        if not await testcharmvision_element.navigate_to_a_href(page=page):
...
async def _handle_multi_field_totp_sequence(
    ...
    current_context = testcharmvision_context.ensure_context()
...
async def handle_input_text_action(
    ...
    current_text = await get_input_value(testcharmvision_element.get_tag_name(), testcharmvision_element.get_locator())
    ...
    if await testcharmvision_element.get_selectable():
        LOG.info(
    ...
    if (
        ...
        try:
            current_context = testcharmvision_current()
    ...
    if skip_context_parsing:
        ...
        input_or_select_context = await _get_input_or_select_context(
            action=action,
            element_tree_builder=scraped_page,
            testcharmvision_element=testcharmvision_element,
            step=step,
        )
    ...
    if (
        ...
        and testcharmvision_element.get_tag_name() == InteractiveElement.INPUT
        and not await testcharmvision_element.is_raw_input()
    ):
        has_onclick_attr = await testcharmvision_element.has_attr("onclick", mode="static")
        await testcharmvision_element.scroll_into_view()
        ...
        await incremental_scraped.start_listen_dom_increment(await testcharmvision_element.get_element_handler())
        try:
            await testcharmvision_element.input_clear()
        except Exception:
            LOG.info(
        ...
        try:
            await testcharmvision_element.press_key("ArrowDown")
        except TimeoutError:
        ...
        await testcharmvision_frame.safe_wait_for_animation_end(before_wait_sec=wait_sec)
        ...
        if len(incremental_element) == 0:
            ...
            try:
                ...
                if await testcharmvision_element.is_visible():
                    blocking_element, exist = await testcharmvision_element.find_blocking_element(
                ...
                if try_to_quit_dropdown and await testcharmvision_element.is_visible():
                    await testcharmvision_element.press_key("Escape")
                    await testcharmvision_element.blur()
                await incremental_scraped.stop_listen_dom_increment()

    ### Start filling text logic
    # check if the element has hidden attribute
    if await testcharmvision_element.has_hidden_attr():
        return [ActionFailure(InputToInvisibleElement(testcharmvision_element.get_id()), stop_execution_on_failure=False)]

    # force to move focus back to the element
    await testcharmvision_element.get_locator().focus(timeout=timeout)

    # check if the element is readonly(some elements will be non-readonly after focused)
    if await testcharmvision_element.is_readonly(dynamic=True):
        LOG.warning(
        ...
        return [ActionFailure(InputToReadonlyElement(element_id=testcharmvision_element.get_id()))]

    # check the phone number format when type=tel and the text is not a secret value
    if not is_secret_value and await testcharmvision_element.get_attr("type") == "tel":
        try:
            action.set_has_mini_agent()
    ...
    class_name: str | None = await testcharmvision_element.get_attr("class")
    if class_name and "blinking-cursor" in class_name:
        if is_totp_value:
            text = generate_totp_value_with_task(task=task, parameter=action.text)
        await testcharmvision_element.press_fill(text=text)
        return [ActionSuccess()]
    ...
    if not await testcharmvision_element.is_spinbtn_input() and (
        current_text or (not await testcharmvision_element.is_editable() and tag_name not in COMMON_INPUT_TAGS)
    ):
        try:
            await testcharmvision_element.input_clear()
        except TimeoutError:
    ...
    try:
        ...
        if blocking_element and exist:
            ...
            if await blocking_element.is_editable():
                testcharmvision_element = blocking_element
    ...
    if is_totp_value:
        ...
        text = generate_totp_value_with_task(task=task, parameter=action.text)
        await testcharmvision_element.input(text)
        return [ActionSuccess()]
    ...
    if action.totp_timing_info:
        ...
        if timing_info.get("is_totp_sequence"):
            ...
            current_totp = testcharmvision_context.ensure_context().totp_codes.get(f"{task.task_id}_totp_cache")
            action_index = timing_info["action_index"]
    ...
    try:
        ...
        if not await testcharmvision_element.is_editable() and tag_name not in COMMON_INPUT_TAGS:
            await testcharmvision_element.input_fill(text)
            return [ActionSuccess()]

        if len(text) == 0:
            return [ActionSuccess()]

        if tag_name == InteractiveElement.INPUT and await testcharmvision_element.get_attr("type") == "date":
            try:
                action.set_has_mini_agent()
            ...
            await testcharmvision_element.input_fill(text=text)
            return [ActionSuccess()]

        if not await testcharmvision_element.is_raw_input():
            is_location_input = input_or_select_context.is_location_input if input_or_select_context else False
            if input_or_select_context and (await testcharmvision_element.is_auto_completion_input() or is_location_input):
                action.set_has_mini_agent()
        ...
        try:
            await testcharmvision_element.input_sequentially(text=text)
        ...
        if auto_complete_hacky_flag and await testcharmvision_element.is_visible() and not await testcharmvision_element.is_raw_input():
            LOG.debug(
                "Trigger input-selection hack, pressing Tab to choose one",
                action=action,
            )
            await testcharmvision_element.press_key("Tab")
...
async def handle_upload_file_action(
    ...
    is_file_input = await testcharmvision_element.is_file_input()

    if not is_file_input:
        LOG.info("Trying to find file input in children", action=action)
        file_input_locator = await testcharmvision_element.find_file_input_in_children()
...
async def handle_select_option_action(
    ...
    tag_name = testcharmvision_element.get_tag_name()
    ...
    if await testcharmvision_element.is_custom_option():
        click_action = ClickAction(element_id=action.element_id)
        action.set_has_mini_agent()
        return await chain_click(task, scraped_page, page, click_action, testcharmvision_element)

    if not await testcharmvision_element.is_selectable():
        ...
        selectable_child: TestcharmvisionElement | None = None
        try:
            selectable_child = await testcharmvision_element.find_selectable_child(dom=dom)
        except Exception as e:
            LOG.error(
        ...
        if selectable_child:
            ...
            action = select_action
            testcharmvision_element = selectable_child
    ...
    if testcharmvision_element.get_tag_name() == InteractiveElement.SELECT:
        LOG.info(
        ...
        try:
            await testcharmvision_element.scroll_into_view()
            blocking_element, exist = await testcharmvision_element.find_blocking_element(dom=dom)
        except Exception:
            LOG.warning(
            ...
            return await normal_select(
                action=action, testcharmvision_element=testcharmvision_element, builder=dom.scraped_page, task=task, step=step
            )
    ...
    if await testcharmvision_element.is_checkbox():
        LOG.info(
    ...
    if await testcharmvision_element.is_radio():
        LOG.info(
    ...
    if await testcharmvision_element.is_btn_input():
        LOG.info(
    ...
    try:
        ...
        await testcharmvision_element.click(page=page, dom=dom, timeout=timeout)
        # wait for options to load
        await testcharmvision_frame.safe_wait_for_animation_end(before_wait_sec=0.5)
        ...
        if len(incremental_element) == 0 and testcharmvision_element.get_tag_name() == InteractiveElement.INPUT:
            LOG.info(
        ...
        input_or_select_context = await _get_input_or_select_context(
            action=action, element_tree_builder=scraped_page, step=step, testcharmvision_element=testcharmvision_element
        )
        ...
        if len(incremental_element) == 0:
            ...
            results.append(
                await select_from_emerging_elements(
                    current_element_id=testcharmvision_element.get_id(),
                    options=CustomSelectPromptOptions(
        ...
        if (
            await testcharmvision_element.is_visible()
            and is_open
            and len(results) > 0
            and not isinstance(results[-1], ActionSuccess)
        ):
            await testcharmvision_element.scroll_into_view()
            await testcharmvision_element.coordinate_click(page=page)
            await testcharmvision_element.press_key("Escape")
        is_open = False
        await testcharmvision_element.blur()
        await incremental_scraped.stop_listen_dom_increment()
    ...
    try:
        ...
        try:
            await testcharmvision_element.get_locator().click(timeout=timeout)
        except Exception:
            LOG.info(
...
async def handle_hover_action(
    ...
    try:
        await testcharmvision_element.hover_to_reveal()
        await testcharmvision_element.get_locator().scroll_into_view_if_needed()
        await testcharmvision_element.get_locator().hover(timeout=settings.BROWSER_ACTION_TIMEOUT_MS)
        ...
        return [ActionFailure(FailToHover(testcharmvision_element.get_id(), msg=str(exc)))]
...
async def handle_verification_code_action(
    ...
    LOG.info(
        "Setting verification code in testcharmvision context",
...
async def chain_click(
    ...
    try:
        ...
        if testcharmvision_element.get_tag_name() == "label":
            try:
                LOG.info(
                    "Chain click: it's a label element. going to try for-click",
                    action=action,
                    element=str(testcharmvision_element),
                    locator=locator,
                )
                if bound_element := await testcharmvision_element.find_label_for(dom=dom):
            ...
            try:
                ...
                if bound_element := await testcharmvision_element.find_element_in_label_children(
            ...
            try:
                ...
                if bound_locator := await testcharmvision_element.find_bound_label_by_attr_id():
            ...
            try:
                ...
                if bound_locator := await testcharmvision_element.find_bound_label_by_direct_parent():
                    # click on (0, 0) to avoid playwright clicking on the wrong element by accident
                    await bound_locator.click(timeout=timeout, position={"x": 0, "y": 0})
                    action_results.append(ActionSuccess())
                    return action_results
        ...
        if not await testcharmvision_element.is_visible():
            LOG.info(
        ...
        blocking_element, blocked = await testcharmvision_element.find_blocking_element(
            dom=DomUtil(scraped_page=scraped_page, page=page)
        )
        if blocking_element is None:
            if not blocked:
            ...
            try:
                await testcharmvision_element.click_in_javascript()
                action_results.append(ActionSuccess())
                return action_results
            except Exception as e:
        ...
        try:
            ...
            if await blocking_element.is_parent_of(
                await testcharmvision_element.get_element_handler()
            ) or await blocking_element.is_sibling_of(await testcharmvision_element.get_element_handler()):
                LOG.info(
...
async def choose_auto_completion_dropdown(
    ...
    current_frame = testcharmvision_element.get_frame()
    testcharmvision_frame = await TestcharmvisionFrame.create_instance(current_frame)
    incremental_scraped = IncrementalScrapePage(testcharmvision_frame=testcharmvision_frame)
    await incremental_scraped.start_listen_dom_increment(await testcharmvision_element.get_element_handler())

    try:
        await testcharmvision_element.press_fill(text)
        ...
        for element in preserved_elements:
            ...
            if not element_id:
                continue
            locator = current_frame.locator(f'[{TESTCHARMVISION_ID_ATTR}="{element_id}"]')
            ...
            current_element = await testcharmvision_frame.parse_element_from_html(
                testcharmvision_element.get_frame_id(),
                element_handler,
                testcharmvision_element.is_interactable(),
            )
        ...
        if len(confirmed_preserved_list) > 0:
            confirmed_preserved_list = await app.AGENT_FUNCTION.cleanup_element_tree_factory(task=task, step=step)(
                testcharmvision_frame.get_frame(), testcharmvision_frame.get_frame().url, copy.deepcopy(confirmed_preserved_list)
            )
        ...
        if len(incremental_element) > 0:
            ...
            if is_location_input and len(cleaned_incremental_element) == 1:
                ...
                if fast_path_element_id and input_normalized and input_normalized in option_text:
                    fast_path_locator = current_frame.locator(f'[{TESTCHARMVISION_ID_ATTR}="{fast_path_element_id}"]')
            ...
            if len(new_interactable_element_ids) == 0:
                raise NoIncrementalElementFoundForAutoCompletion(element_id=testcharmvision_element.get_id(), text=text)
            LOG.info(
        ...
        if json_response.get("direct_searching", False):
            ...
            await testcharmvision_element.press_key("Enter")
            return result
        ...
        await incremental_scraped.stop_listen_dom_increment()
        if clear_input and await testcharmvision_element.is_visible():
            await testcharmvision_element.input_clear()
...
async def input_or_auto_complete_input(
    ...
    # FIXME: try the whole loop for once now, to speed up testcharmvision
    ...
    while current_attemp < MAX_AUTO_COMPLETE_ATTEMP:
        ...
        # WARN: currently, we don't trigger this logic because MAX_AUTO_COMPLETE_ATTEMP is 1, to speed up testcharmvision
...
async def sequentially_select_from_dropdown(
    ...
    step: Step,
    task: Task,
    dropdown_menu_element: TestcharmvisionElement | None = None,
    ...
    for i in range(max_depth):
        ...
        if input_or_select_context.is_date_related:
            if testcharmvision_element.get_tag_name() == InteractiveElement.INPUT and action.option.label:
                try:
                    LOG.info("Try to input the date directly")
                    await testcharmvision_element.input_sequentially(action.option.label)
                    result = CustomSingleSelectResult(testcharmvision_frame=testcharmvision_frame)
                    result.action_result = ActionSuccess()
                    return result
...
async def select_from_emerging_elements(
    ...
    if value is not None and action_type == ActionType.INPUT_TEXT:
        ...
        input_element = await dom_after_open.get_testcharmvision_element_by_id(element_id)
        ...
        selected_element = await dom_after_open.get_testcharmvision_element_by_id(element_id)
...
async def select_from_dropdown(
    ...
    single_select_result = CustomSingleSelectResult(testcharmvision_frame=testcharmvision_frame)

    timeout = settings.BROWSER_ACTION_TIMEOUT_MS

    if dropdown_menu_element is None:
        dropdown_menu_element = await locate_dropdown_menu(
            current_anchor_element=testcharmvision_element,
            incremental_scraped=incremental_scraped,
            step=step,
            task=task,
        )
    ...
    if dropdown_menu_element:
        potential_scrollable_element = await try_to_find_potential_scrollable_element(
            testcharmvision_element=dropdown_menu_element,
            incremental_scraped=incremental_scraped,
            step=step,
            task=task,
        )

        if await testcharmvision_frame.get_element_scrollable(await potential_scrollable_element.get_element_handler()):
    ...
    html = incremental_scraped.build_element_tree(html_need_testcharmvision_attrs=True)

    testcharmvision_context = ensure_context()
    prompt = prompt_engine.load_prompt(
        "custom-select",
        ...
        local_datetime=datetime.now(testcharmvision_context.tz_info).isoformat(),
    )
    ...
    if value is not None and action_type == ActionType.INPUT_TEXT:
        ...
        try:
            actual_value = get_actual_value_of_parameter_if_secret_with_task(task, value)
            input_element = await TestcharmvisionElement.create_from_incremental(incremental_scraped, element_id)
            await input_element.scroll_into_view()
            current_text = await get_input_value(input_element.get_tag_name(), input_element.get_locator())
            if current_text == actual_value:
    ...
    try:
        selected_element = await TestcharmvisionElement.create_from_incremental(incremental_scraped, element_id)
        ...
        if selected_element.get_tag_name() == InteractiveElement.SELECT and value:
            ...
            results = await normal_select(
                action=action, testcharmvision_element=selected_element, task=task, step=step, builder=incremental_scraped
            )
...
async def select_from_dropdown_by_value(
    ...
    if not await testcharmvision_frame.get_element_scrollable(await potential_scrollable_element.get_element_handler()):
...
async def locate_dropdown_menu(
    current_anchor_element: TestcharmvisionElement,
    incremental_scraped: IncrementalScrapePage,
    step: Step,
    task: Task,
) -> TestcharmvisionElement | None:
    ...
    for idx, element_dict in enumerate(incremental_scraped.element_tree):
        ...
        try:
            head_element = await TestcharmvisionElement.create_from_incremental(incremental_scraped, element_id)
        except Exception:
            LOG.debug(
        ...
        if not await testcharmvision_frame.get_element_visible(await head_element.get_element_handler()):
            LOG.debug(
        ...
        if ul_or_listbox_element_id:
            try:
                await TestcharmvisionElement.create_from_incremental(incremental_scraped, ul_or_listbox_element_id)
                LOG.info(
                    "Confirm it's an opened dropdown menu since it includes <ul> or <role='listbox'>",
                    element_id=element_id,
                )
                return await TestcharmvisionElement.create_from_incremental(
                    incre_page=incremental_scraped, element_id=element_id
                )
            except Exception:
                LOG.debug(
        ...
        x, y = await testcharmvision_frame.get_scroll_x_y()
        screenshot = await head_element.get_locator().screenshot(timeout=settings.BROWSER_SCREENSHOT_TIMEOUT_MS)
        await testcharmvision_frame.scroll_to_x_y(x, y)

        # TODO: better to send untrimmed HTML without testcharmvision attributes in the future
        ...
        if is_opened_dropdown_menu:
            ...
            return await TestcharmvisionElement.create_from_incremental(incre_page=incremental_scraped, element_id=element_id)
    return None
...
async def try_to_find_potential_scrollable_element(
    ...
    task: Task,
    step: Step,
) -> TestcharmvisionElement:
    """
    check any <ul> or <role="listbox"> element in the chidlren.
    if yes, return the found element,
    else, return the orginal one
    """
    found_element_id = await testcharmvision_element.find_children_element_id_by_callback(
        cb=is_ul_or_listbox_element_factory(incremental_scraped=incremental_scraped, task=task, step=step),
    )
    if found_element_id and found_element_id != testcharmvision_element.get_id():
        LOG.debug(
        ...
        try:
            testcharmvision_element = await TestcharmvisionElement.create_from_incremental(incremental_scraped, found_element_id)
        except Exception:
            LOG.debug(
    ...
    return testcharmvision_element


@traced()
async def scroll_down_to_load_all_options(
    scrollable_element: TestcharmvisionElement,
    page: Page,
    testcharmvision_frame: TestcharmvisionFrame,
    incremental_scraped: IncrementalScrapePage,
    step: Step | None = None,
    ...
    while datetime.now(timezone.utc) < deadline:
        ...
        if dropdown_menu_element_handle is None:
            ...
            await testcharmvision_frame.scroll_to_element_bottom(dropdown_menu_element_handle, page_by_page)
    ...
    if dropdown_menu_element_handle is None:
        ...
        await testcharmvision_frame.scroll_to_element_top(dropdown_menu_element_handle)
    await testcharmvision_frame.safe_wait_for_animation_end(before_wait_sec=0.5)
...
async def normal_select(
    ...
    action.set_has_mini_agent()
    try:
        current_text = await testcharmvision_element.get_attr("selected")
    ...
    is_success = False
    locator = testcharmvision_element.get_locator()
    ...
    await testcharmvision_element.refresh_select_options()
    options_html = testcharmvision_element.build_HTML()
    field_information = (
    ...
    if not is_success and index is not None:
        if index >= len(testcharmvision_element.get_options()):
...
async def extract_information_for_navigation_goal(
    ...
    extract_information_prompt = load_prompt_with_elements(
        ...
        html_need_testcharmvision_attrs=False,
        navigation_goal=task.navigation_goal,
        navigation_payload=task.navigation_payload,
    ...
    while bfs_queue:
        ...
        if "attributes" in child and "role" in child["attributes"] and child["attributes"]["role"] == "option":
            ...
            if text and (text == action.option.label or text == action.option.value):
                dom = DomUtil(scraped_page=scraped_page, page=page)
                try:
                    testcharmvision_element = await dom.get_testcharmvision_element_by_id(child["id"])
                    locator = testcharmvision_element.locator
...
async def _get_input_or_select_context(
    ...
    testcharmvision_frame = await TestcharmvisionFrame.create_instance(testcharmvision_element.get_frame())
    try:
        depth = await testcharmvision_frame.get_element_dom_depth(await testcharmvision_element.get_element_handler())
    except Exception:
    ...
    if depth > ancestor_depth:
        ...
        locator = testcharmvision_element.get_locator().locator(path)
        try:
            element_handle = await locator.element_handle(timeout=settings.BROWSER_ACTION_TIMEOUT_MS)
            if element_handle is not None:
                elements, element_tree = await testcharmvision_frame.build_tree_from_element(
                    starter=element_handle,
                    frame=testcharmvision_element.get_frame_id(),
                )
                clean_up_func = app.AGENT_FUNCTION.cleanup_element_tree_factory(step=step)
                element_tree = await clean_up_func(testcharmvision_element.get_frame(), "", copy.deepcopy(element_tree))
