...
def testcharmvision_logs_processor(logger: logging.Logger, method_name: str, event_dict: EventDict) -> EventDict:
    """
    A custom processor to add testcharmvision logs to the context
    """
    if method_name not in ["info", "warning", "error", "critical", "exception"]:
        return event_dict
...
def setup_logger() -> None:
    ...
    structlog.configure(
        ...
        + additional_processors
        + [testcharmvision_logs_processor, structlog.stdlib.ProcessorFormatter.wrap_for_formatter],
    )
    ...
    for name in ("testcharmvision", "cloud", "workers", "scripts", "browser_controller"):
