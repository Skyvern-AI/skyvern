...
from testcharmvision.constants import BROWSER_CLOSE_TIMEOUT, BROWSER_PAGE_CLOSE_TIMEOUT, NAVIGATION_MAX_RETRY_TIME
...
from testcharmvision.schemas.runs import ProxyLocationInput
from testcharmvision.webeye.browser_artifacts import BrowserArtifacts, VideoArtifact
from testcharmvision.webeye.browser_factory import BrowserCleanupFunc, BrowserContextFactory
from testcharmvision.webeye.browser_state import BrowserState
from testcharmvision.webeye.scraper import scraper
from testcharmvision.webeye.scraper.scraped_page import CleanupElementTreeFunc, ScrapedPage, ScrapeExcludeFunc
from testcharmvision.webeye.utils.page import ScreenshotMode, TestcharmvisionFrame
...
class RealBrowserState(BrowserState):
    ...
    async def stop_page_loading(self) -> None:
        page = await self.__assert_page()
        try:
            await TestcharmvisionFrame.evaluate(frame=page, expression="window.stop()")
        except Exception as e:
    ...
    async def take_fullpage_screenshot(
        ...
        page = await self.__assert_page()
        return await TestcharmvisionFrame.take_scrolling_screenshot(
            page=page,
            file_path=file_path,
