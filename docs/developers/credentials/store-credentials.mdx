...
<Note>
Creating, updating, deleting, listing, or reading credentials, and submitting TOTP codes, require an administrator-scoped API key. On Enterprise plans, non-administrator members cannot call these endpoints and receive a 403 response. Organization API keys are administrator-scoped by default.
</Note>
...
Use `GET /v1/credentials/onepassword/status` to check whether the organization uses its own token, the instance default, or no token.
...
3. Reference vault items by ID in your agents
