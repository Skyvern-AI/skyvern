...
from testcharmvision.client import AsyncTestcharmvision, BrowserSessionResponse, TestcharmvisionEnvironment
from testcharmvision.client.core import RequestOptions
from testcharmvision.client.types.task_run_response import TaskRunResponse
from testcharmvision.client.types.workflow_run_response import WorkflowRunResponse
from testcharmvision.forge.sdk.api.llm.models import LLMConfig, LLMRouterConfig
from testcharmvision.library.constants import DEFAULT_AGENT_HEARTBEAT_INTERVAL, DEFAULT_AGENT_TIMEOUT, DEFAULT_CDP_PORT
...
from testcharmvision.schemas.runs import ProxyLocation, RunEngine, RunStatus

LOG = structlog.get_logger()


def _get_browser_session_url(browser_session_id: str) -> str:
    return f"https://app.testcharmvision.com/browser-session/{browser_session_id}"


class Testcharmvision(AsyncTestcharmvision):
    ...
    Example:
        ...
        # Local/embedded mode (run `testcharmvision quickstart` first):
        testcharmvision = Testcharmvision.local()

        # Launch a local browser (works only in local environment)
        browser = await testcharmvision.launch_local_browser(headless=False)
        page = await browser.get_working_page()

        # Or use a cloud browser (works only in cloud environment)
        browser = await testcharmvision.use_cloud_browser()
        page = await browser.get_working_page()
    ...
    You can also mix AI-powered tasks with direct browser control in the same session:
        ...
        credential = await testcharmvision.create_credential(
            name="my_user",
        ...
        await page.agent.login(
            credential_type=CredentialType.testcharmvision,
    ...
    def __init__(
        ...
        *,
        ...
        """Remote mode: Connect to Testcharmvision Cloud or self-hosted instance.

        Args:
            api_key: API key for authenticating with Testcharmvision.
                Can be found on the settings page: https://app.testcharmvision.com/settings
            environment: The Testcharmvision environment to connect to. Use TestcharmvisionEnvironment.CLOUD
                for Testcharmvision Cloud or TestcharmvisionEnvironment.PRODUCTION/STAGING for self-hosted
                instances. Defaults to TestcharmvisionEnvironment.CLOUD.
            base_url: Override the base URL for the Testcharmvision API. If not provided, uses the default URL for
    ...
    def local(
        ...
        *,
        ...
        """Local/embedded mode: Run Testcharmvision locally in-process.

        Prerequisites:
            Run `testcharmvision quickstart` first to set up your local environment and create a .env file
        ...
        Args:
            llm_config: Optional custom LLM configuration (LLMConfig or LLMRouterConfig).
                ...
                Example 1 - Using .env configuration (simplest, recommended):
                    ...
                    # Created by running `testcharmvision quickstart`
                    testcharmvision = Testcharmvision.local()
                    ```
                ...
                Example 2 - Custom LLM with environment variables:
                    ```python
                    from testcharmvision import Testcharmvision
                    from testcharmvision.forge.sdk.api.llm.models import LLMConfig
                ...
                Example 3 - Explicitly providing credentials:
                    ```python
                    from testcharmvision import Testcharmvision
                    from testcharmvision.forge.sdk.api.llm.models import LLMConfig, LiteLLMParams
                    ...
                    ```
            settings: Optional dictionary of Testcharmvision settings to override.
                These override the corresponding settings from your .env file.
                Example: {"MAX_STEPS_PER_RUN": 100, "BROWSER_TYPE": "chromium-headful"}

        Returns:
            Testcharmvision: A Testcharmvision instance running in local/embedded mode.
        """
        from testcharmvision.library.embedded_server_factory import create_embedded_server  # noqa: PLC0415

        if not os.path.exists(".env"):
            raise ValueError("Please run `testcharmvision quickstart` to set up your local Testcharmvision environment")

        load_dotenv(".env")
        api_key = os.getenv("TESTCHARMVISION_API_KEY")
        if not api_key:
            raise ValueError("TESTCHARMVISION_API_KEY is not set. Provide api_key or set TESTCHARMVISION_API_KEY in .env file.")

        obj = cls.__new__(cls)

        AsyncTestcharmvision.__init__(
            obj,
            environment=TestcharmvisionEnvironment.LOCAL,
        ...
        obj._environment = TestcharmvisionEnvironment.LOCAL
        obj._api_key = None
    ...
    @property
    def environment(self) -> TestcharmvisionEnvironment | None:
        """Get the current Testcharmvision environment (CLOUD, STAGING, LOCAL, or None for embedded mode)."""
    ...
    async def launch_local_browser(
        ...
        *,
        ...
        allowing Testcharmvision to control it via CDP. Useful for development and debugging.
        ...
        Returns:
            TestcharmvisionBrowser: A browser instance with Testcharmvision capabilities.
        """
        ...
        if user_data_dir:
            ...
            user_data_path = pathlib.Path(tempfile.gettempdir()) / "testcharmvision-browser"
        ...
        return TestcharmvisionBrowser(self, browser_context, browser_address=browser_address)

    async def connect_to_browser_over_cdp(self, cdp_url: str) -> TestcharmvisionBrowser:
        ...
        Returns:
            TestcharmvisionBrowser: A browser instance connected to the existing browser.
        """
        playwright = await self._get_playwright()
        browser = await playwright.chromium.connect_over_cdp(cdp_url)
        browser_context = browser.contexts[0] if browser.contexts else await browser.new_context()
        return TestcharmvisionBrowser(self, browser_context, browser_address=cdp_url)

    async def connect_to_cloud_browser_session(self, browser_session_id: str) -> TestcharmvisionBrowser:
        ...
        Returns:
            TestcharmvisionBrowser: A browser instance connected to the cloud session.
        """
        self._ensure_cloud_environment()
        browser_session = await self.get_browser_session(browser_session_id)
        if self._environment == TestcharmvisionEnvironment.CLOUD:
            LOG.info(
    ...
    async def launch_cloud_browser(
        ...
        *,
        ...
        Returns:
            TestcharmvisionBrowser: A browser instance connected to the new cloud session.
        """
        self._ensure_cloud_environment()
    ...
    async def use_cloud_browser(
        ...
        *,
        ...
        Returns:
            TestcharmvisionBrowser: A browser instance connected to an existing or new cloud session.
        """
        self._ensure_cloud_environment()
    ...
    def _ensure_cloud_environment(self) -> None:
        if self._environment not in (TestcharmvisionEnvironment.CLOUD, TestcharmvisionEnvironment.STAGING):
            raise ValueError("Cloud browser sessions are supported only in the cloud environment")

    async def _connect_to_cloud_browser_session(self, browser_session: BrowserSessionResponse) -> TestcharmvisionBrowser:
        ...
        browser_context = browser.contexts[0] if browser.contexts else await browser.new_context()
        return TestcharmvisionBrowser(self, browser_context, browser_session_id=browser_session.browser_session_id)
