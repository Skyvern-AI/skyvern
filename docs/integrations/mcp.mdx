...
The MCP server exposes 77 tools across many categories. Here are the key ones:
...
| **Batch operations** | `skyvern_observe`, `skyvern_execute` | Snapshot interactive page elements and run multiple browser actions in a single call to reduce round-trips |
...
## Batch operations with observe and execute

Use `skyvern_observe` and `skyvern_execute` to reduce round-trips when you need to perform multiple browser actions in sequence. Instead of making separate tool calls for each click, type, or scroll, you can batch them into a single `skyvern_execute` call.

### How it works

1. Call `skyvern_observe` to get a structured snapshot of interactive elements on the page. Each element gets a stable ref (e.g., `e0`, `e1`).
2. Call `skyvern_execute` with a list of steps that reference those elements by ref.

### skyvern_observe

Returns interactive page elements with stable refs for use in `skyvern_execute`.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `session_id` | string | — | Browser session ID (`pbs_...`) |
| `cdp_url` | string | — | CDP WebSocket URL (alternative to `session_id`) |
| `selector` | string | — | CSS selector to scope the snapshot (e.g., `form#login`). Omit for full page |
| `interactive_only` | boolean | `true` | Only return interactive elements (buttons, inputs, links) |
| `max_elements` | integer | `50` | Max elements to return (1–200) |

The response includes an `elements` array where each element has a `ref` field (e.g., `e0`), along with its role, name, and other accessibility attributes.

### skyvern_execute

Runs multiple browser operations in a single batch call.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `steps` | array | — | Array of `{tool, params}` step objects to execute sequentially |
| `session_id` | string | — | Browser session ID (`pbs_...`) |
| `cdp_url` | string | — | CDP WebSocket URL (alternative to `session_id`) |
| `stop_on_error` | boolean | `true` | Stop at first failure (`true`) or continue past errors (`false`) |

Each step in the `steps` array specifies a `tool` and its `params`. You can reference element refs from `skyvern_observe` using the `ref` param instead of a selector.

**Allowed tools in steps:** `navigate`, `click`, `type`, `press_key`, `select_option`, `hover`, `scroll`, `wait`, `observe`, `screenshot`, `evaluate`

### Example

```json
// Step 1: Observe the page to get element refs
// Call skyvern_observe → returns elements like {ref: "e0", role: "textbox", name: "Email"}, ...

// Step 2: Execute a batch of actions using those refs
{
  "steps": [
    {"tool": "click", "params": {"ref": "e0"}},
    {"tool": "type", "params": {"ref": "e0", "text": "user@example.com"}},
    {"tool": "click", "params": {"ref": "e3"}},
    {"tool": "type", "params": {"ref": "e3", "text": "mypassword"}},
...
<Tip>
Batch operations are especially useful for form filling, multi-step interactions, and any scenario where you already know what actions to perform. They reduce token usage and latency by combining multiple tool calls into one.
</Tip>
