...
| `browser_address` | `str \| None` | CDP address for connecting to the browser. Must be an `http`, `https`, `ws`, or `wss` URL with a resolvable host that passes network policy. Invalid or blocked addresses return a 422 validation error. |
