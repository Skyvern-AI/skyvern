...
| [Files](#files) | File Download, Data Export, Cloud Storage Upload |
...
Write schema-defined records to a Parquet file. The file is registered as a download on the run, alongside browser downloads.

| Field | Type | Description |
|-------|------|-------------|
| **Records** | text | The records to export. Usually a parameter reference such as `{{ extraction_output.extracted_information }}`. Must resolve to a JSON array of object records. |
| **Data Schema** | JSON | [JSON Schema](https://json-schema.org/) with `type: array` and an `items` object. The item properties define the Parquet columns. Missing properties and values that don't match their declared type become null. |
| **Filename** | text | Optional filename for the export. Defaults to the block label. The `.parquet` extension is added automatically. Inside a loop, each iteration's file gets a numeric suffix (`-0001`, `-0002`, ...). |
| **Parameters** | select | Output parameters from previous blocks to make available in the Records template |

In the API, this block uses `block_type: data_export` with `data`, `data_schema`, and optional `file_name` and `parameter_keys` fields.
