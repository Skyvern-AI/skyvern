...
from testcharmvision.core.script_generations.constants import SCRIPT_TASK_BLOCKS
from testcharmvision.core.script_generations.generate_script import _build_block_fn, create_or_update_script_block
...
from testcharmvision.exceptions import ScriptNotFound, ScriptTerminationException, StepTerminationError, WorkflowRunNotFound
...
from testcharmvision.forge.sdk.workflow.models.parameter import PARAMETER_TYPE, OutputParameter, ParameterType
from testcharmvision.forge.sdk.workflow.models.workflow import Workflow
...
from testcharmvision.webeye.actions.actions import Action, DecisiveAction
from testcharmvision.webeye.scraper.scraped_page import ElementTreeFormat

LOG = structlog.get_logger()
jinja_sandbox_env = SandboxedEnvironment()


class TestcharmvisionLoopItem:
    def __init__(
        self,
        index: int,
        value: Any,
    ...
    def __repr__(self) -> str:
        return f"TestcharmvisionLoopItem(current_value={self.current_value}, current_index={self.current_index})"
...
async def _detect_user_defined_errors(
    ...
    try:
        run_context = script_run_context_manager.ensure_run_context()
        testcharmvision_page = run_context.page
        scraped_page = await testcharmvision_page.scraped_page.refresh()
        testcharmvision_page.scraped_page = scraped_page
...
async def run_task(
    ...
    context: testcharmvision_context.TestcharmvisionContext | None = None
    if cache_key and cached_fn:
        ...
        task_block = NavigationBlock(
            ...
            totp_verification_url=totp_url,
            include_action_history_in_verification=True,
            engine=RunEngine.testcharmvision_v1,
            model=model,
        )
...
async def download(
    ...
    cache_key = cache_key or label
    cached_fn = script_run_context_manager.get_cached_fn(cache_key)
    context: testcharmvision_context.TestcharmvisionContext | None
    if cache_key and cached_fn:
        # Auto-create workflow block run and task if workflow_run_id is available
        workflow_run_block_id, task_id, step_id = await _create_workflow_block_run_and_task(
...
async def run_script(
    ...
    context = testcharmvision_context.current()
    if not context:
        context = testcharmvision_context.TestcharmvisionContext()
        testcharmvision_context.set(context)
...
async def loop(
    ...
    label: str | None = None,
) -> AsyncGenerator[TestcharmvisionLoopItem, None]:
    ...
    try:
        for index, value in enumerate(loop_values):
            ...
            # Build the TestcharmvisionLoopItem for this loop
            yield TestcharmvisionLoopItem(index, value)
