...
Type text character-by-character. Unlike `fill`, this triggers keystroke events for each character — use for fields that react to individual key presses (search autocomplete, OTP inputs). Python only.
...
| `data_extraction_schema` | `dict \| str` | No | JSON Schema for structured output. See [Extract Structured Data](/multi-step-automations/extract-structured-data). |
...
| `engine` | `RunEngine` | No | AI engine. Options: `skyvern-2.0` (recommended), `skyvern-1.0` (default), `openai-cua`, `anthropic-cua`, `ui-tars` |
...
**Returns:** `list[dict[str, Any]]` — each dict contains field name, type, options, and other metadata.
