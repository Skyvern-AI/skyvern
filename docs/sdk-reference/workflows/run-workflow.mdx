...
| `file_ids` | `list[str]` | No | `None` | IDs of files uploaded through `POST /v1/upload_file` to attach to this run (up to 50). Reference an attached file by its ID anywhere a file URL is accepted (for example in `parameters`) and no presigned URL is created for it. Skyvern deletes an attached file when the run finishes. A file can be attached to one run at a time and must not already be deleted. |
