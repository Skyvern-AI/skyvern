...
slug: multi-step-automations/extract-structured-data
keywords:
  - JSON Schema
  - extraction schema
  - typed output
...
When building browser automations, you can extract structured data from any page using `page.extract` with a JSON schema, or by passing a `data_extraction_schema` to `run_task` or `page.agent.run_task`. By default, Skyvern returns extracted data in whatever format makes sense for the action. Pass a schema to enforce a specific structure using [JSON Schema.](https://json-schema.org/)
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="/multi-step-automations/overview"
  >
