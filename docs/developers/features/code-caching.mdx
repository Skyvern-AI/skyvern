...
The first run uses the agent as normal. Create a workflow and run it with `run_with="agent"` so Skyvern can record the successful browser actions as generated code.
...
<Note>
The SDK and Cloud UI default to `skyvern-1.0` for new task creation. Use workflows plus `run_with="code"` for cached replays.
</Note>
