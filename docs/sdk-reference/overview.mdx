...
<Note>
`pip install skyvern` is the lightweight SDK for Skyvern Cloud and remote API calls. Embedded local SDK features such as `Skyvern.local()` and local browser control require `pip install "skyvern[local]"`. For a local server with the packaged web UI and default SQLite database, use `pip install "skyvern[all]"`. Use `skyvern[server]` or `skyvern[ui]` only when you intentionally want one side without the other.
</Note>
