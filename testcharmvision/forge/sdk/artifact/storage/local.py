...
class LocalStorage(BaseStorage):
    ...
    async def get_streaming_file(self, organization_id: str, file_name: str) -> bytes | None:
        ...
        Path(f"{get_testcharmvision_temp_dir()}/{organization_id}").mkdir(parents=True, exist_ok=True)
        file_path = Path(f"{get_testcharmvision_temp_dir()}/{organization_id}/{file_name}")
        try:
            async with aiofiles.open(file_path, "rb") as f:
                return await f.read()
        except Exception:
