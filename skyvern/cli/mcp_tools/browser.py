...
def _read_run_owned_upload(candidate_path: str, run_id: str, max_bytes: int) -> FilePayload:
    ...
    if nofollow_flag is None or directory_flag is None or not _LOCAL_UPLOAD_SUPPORTS_DIR_FD:
    ...
    file_open_flags = os.O_RDONLY | close_on_exec | nofollow_flag
...
def _read_run_owned_uploads(file_paths: list[str], run_id: str | None) -> list[FilePayload]:
    if run_id is None:
        raise PermissionError("Local file upload requires an active run")
...
async def skyvern_file_upload(
    file_paths: Annotated[
        ...
        Field(
            description=(
                ...
                "and require a selector, with at most 20 files and 50MB total. URLs are downloaded automatically."
            )
        ),
    ],
    selector: Annotated[
    ...
    Local paths must be owned by the active run and use a CSS/XPath selector.
    """
    if not file_paths:
        return make_result(
            "skyvern_file_upload",
        ...
        try:
            ...
            return make_result(
                ...
                error=make_error(
                    ...
                    "Use a regular file from the active run's download directory",
                ),
            )
