...
def test_declared_browser_origin_is_static_only_when_the_workflow_wrote_the_url() -> None:
    """`url` is overwritten in place by rendering, so authority must key off the declared form."""
    context = _make_workflow_run_context({"host": "collector.example"})
    block = TaskBlock(
        label="templated_url",
        output_parameter=_make_output_parameter(),
    ...
    assert block.url == "https://collector.example/collect"
    assert block.declares_static_browser_origin(context) is False


def test_statically_declared_url_still_declares_a_static_origin() -> None:
    ...
    assert block.declares_static_browser_origin(context) is True


def test_absent_url_declares_no_origin() -> None:
    context = _make_workflow_run_context()
    block = TaskBlock(label="no_url", output_parameter=_make_output_parameter())
