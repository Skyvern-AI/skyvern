...
class TestOTPExtractionPriority:
    """Tests that OTP is only extracted from navigation_payload, never navigation_goal."""

    @pytest.mark.asyncio
    async def test_agent_does_not_extract_otp_from_navigation_goal(self) -> None:
        """Agent should NOT extract OTP from navigation_goal — only navigation_payload is used."""
        with (
            patch(
                "skyvern.forge.agent.try_generate_totp_from_credential",
                return_value=None,
            ...
            agent = ForgeAgent.__new__(ForgeAgent)
            agent._build_extract_action_prompt = AsyncMock(return_value=("prompt", False, "extract-actions"))
            ...
            # Should NOT have extracted "520265" from navigation_goal
            assert "task_456" not in mock_context.totp_codes
            # Should have fallen through to credential and poll
