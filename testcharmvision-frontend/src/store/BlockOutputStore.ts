...
const getStorageKey = (wpid: string, blockLabel: string) => {
  return `testcharmvision.block-output.${wpid}.${blockLabel}`;
};

const getStorageKeyForUse = (wpid: string, blockLabel: string) => {
  return `testcharmvision.block-output.use.${wpid}.${blockLabel}`;
};
...
const loadAllUseOverrides = (): {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key?.startsWith("testcharmvision.block-output.use.")) {
      try {
        const value = localStorage.getItem(key);
        if (value) {
          const parsed = JSON.parse(value);
          // Extract wpid and blockLabel from key: testcharmvision.block-output.use.{wpid}.{blockLabel}
