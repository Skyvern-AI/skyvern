...
Open [Skyvern Cloud](https://app.skyvern.com) and enter a task in the prompt box.
...
1. Go to [Settings](https://app.skyvern.com/settings) on Skyvern Cloud.
2. Copy your API key.
...
If you don't have pipx installed, follow the [official pipx installation instructions](https://pipx.pypa.io/stable/installation/).
...
The SDK handles browser provisioning, task execution, and result retrieval. More details can be found in the [SDK Reference](/sdk-reference).
...
- Docker installed and running. [Get it here](https://www.docker.com/get-started/).
- An OpenAI/Anthropic/Gemini API key. Get it [here](https://platform.openai.com/api-keys), [here](https://console.anthropic.com/) or [here](https://ai.google.dev/)
...
You can then run the local server with:
...
Once you have Skyvern server running on a local port, such as `http://0.0.0.0:8000`, run the following python code:
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/authentication"
  >
    Learn how to store login credentials for workflows that involve signing-up or logging-in or even 2FA
  </Card>
  <Card
    title="Core Concepts"
    icon="book"
