...
def create_database_and_user() -> None:
    ...
    run_command("createuser testcharmvision")
    run_command("createdb testcharmvision -O testcharmvision")
...
def setup_postgresql(no_postgres: bool = False) -> None:
    """Set up PostgreSQL database for Testcharmvision."""
    ...
    if command_exists("psql") and is_postgres_running():
        ...
        if database_exists("testcharmvision", "testcharmvision"):
    ...
    if is_postgres_running_in_docker():
        ...
        if not no_postgres:
            start_postgres = Confirm.ask(
                ...
                '[tip: choose "n" if you plan to run Testcharmvision via Docker Compose instead of `testcharmvision run server`]'
            )
    ...
    with console.status("[bold green]Checking database user...[/bold green]"):
        _, code = run_command(
            'docker exec postgresql-container psql -U postgres -c "\\du" | grep -q testcharmvision', check=False
        )
        if code == 0:
            console.print("✅ [green]Database user exists.[/green]")
        else:
            console.print("🚀 [bold green]Creating database user...[/bold green]")
            run_command("docker exec postgresql-container createuser -U postgres testcharmvision")
            console.print("✅ [green]Database user created.[/green]")

    with console.status("[bold green]Checking database...[/bold green]"):
        _, code = run_command(
            "docker exec postgresql-container psql -U postgres -lqt | cut -d | -f 1 | grep -qw testcharmvision",
            check=False,
        )
        if code == 0:
            ...
            run_command("docker exec postgresql-container createdb -U postgres testcharmvision -O testcharmvision")
