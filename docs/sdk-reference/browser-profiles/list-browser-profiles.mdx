...
| `managed` | `bool` | No | `None` | Omit to return all profiles; `False` returns only user-created profiles; `True` returns only auto-managed profiles created by Save & Reuse Session workflows. |
