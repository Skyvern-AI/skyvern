...
An Agent is a reusable automation template built in the [Cloud UI agent editor](/cloud/building-agents/build-an-agent). You drag and drop blocks onto a canvas, wire them together, and save. Agents can be versioned, shared across your team, scheduled on a cron, and run repeatedly with different parameters.
...
Agents are the right choice when you want no-code automation, team-shared templates, scheduled recurring jobs, or visual drag-and-drop logic.

<Tip>
**Building agents?** See the [Agent Editor guide](/cloud/building-agents/build-an-agent). **Need to trigger an agent from code?** See [Run from Code](/cloud/building-agents/run-from-code) in the Cloud UI docs.
</Tip>

### Blocks

Blocks are the building units of agents. Each block performs one operation.
...
**Logic and control flow:** Conditional (if/else branching), For Loop (repeat over a list), While Loop (repeat while a condition remains true), Validation (assert conditions, halt on failure), and Code (custom Python/Playwright scripts).

**Communication:** HTTP Request (external API calls), Text Prompt (text-only LLM prompt, no browser), and Send Email.

For detailed block configuration, see [Block Types and Configuration](/cloud/building-agents/configure-blocks).

## Runs

Every time you execute a task or kick off an agent, Skyvern creates a Run to track progress and store outputs. A run moves through a lifecycle:
...
A Schedule runs an agent automatically on a recurring basis. You define a cron expression and timezone, and Skyvern triggers the agent at each interval.
...
The `cron_expression` is a standard 5-field cron (minimum 5-minute interval). To run on a fixed elapsed interval instead, set `interval_seconds` (minimum `300`) and optionally `first_fire_at` in place of `cron_expression`. Set exactly one of the two. The `timezone` is an IANA identifier like `America/New_York`, and `parameters` are passed to each scheduled run. You can pause a schedule by setting `enabled` to `false`.
...
**Browser Sessions** are live browser instances that maintain state across multiple operations. Cookies, storage, and page context persist for the duration of the session (up to 6 hours). They're useful for chaining operations in real-time or allowing human intervention between steps.
...
Every run generates artifacts for observability, debugging, and audit trails: end-to-end video recordings, screenshots captured after each action, downloaded files, JSON-structured logs at step/task/agent levels, and HAR files for network debugging.
...
Skyvern supports multiple AI engines. `skyvern-1.0` is the default for the Cloud UI and SDK. `skyvern-2.0` remains available for existing V2 agents and explicitly requested V2 task runs. Other options include `openai-cua` (OpenAI Computer Use Agent), `anthropic-cua` (Anthropic Computer Use Agent), and `ui-tars`.
...
| Build multi-step automation visually | [Agent](#workflows) |
| Trigger a UI-built agent from code | [Run from Code](/cloud/building-agents/run-from-code) |
...
<CardGroup cols={2}>
  ...
  >
    Run tasks, build agents visually, and monitor runs. No code required.
  </Card>
  <Card
