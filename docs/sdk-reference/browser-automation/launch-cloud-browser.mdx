...
|-------------------|----------------|------|----------|---------|-------------|
| `timeout` | `timeout` | `int` / `number` | No | `60` | Session timeout in minutes (min 5, max 240; higher values are capped at 240 with a warning). A running session can be extended to 360 with [`extend_browser_session`](/sdk-reference/browser-sessions/extend-browser-session). |
| `proxy_location` | `proxyLocation` | `ProxyLocation` | No | `None` / `undefined` | Geographic proxy location for browser traffic. |
| `extensions` | `extensions` | `list[Extensions]` / `Extensions[]` | No | `None` / `undefined` | Browser extensions to install in the session. Options: `"ad-blocker"`, `"captcha-solver"`. |
| `browser_profile_id` | `browserProfileId` | `str` / `string` | No | `None` / `undefined` | Load a [browser profile](/sdk-reference/browser-profiles/create-browser-profile) (cookies, localStorage) into the session. ID starts with `bp_`. |
