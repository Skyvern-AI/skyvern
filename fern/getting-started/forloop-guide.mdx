...
1. Loop Value, or the list that you are iterating over
    i. The structure for a properly formatted loop value is an array (not wrapped in a JSON object)
2. The Current Value, or a parameter placed into the block(s) within a ForLoop that allows that block to reference the current loop value
    i. The structure for adding the current value parameter is always {{blockname.current_value}} or {{current_value}}
