...
from testcharmvision._version import __version__
...
from testcharmvision.forge.sdk.artifact.models import Artifact, ArtifactType
from testcharmvision.forge.sdk.core import testcharmvision_context
from testcharmvision.forge.sdk.core.curl_converter import curl_to_http_request_block_params
from testcharmvision.forge.sdk.core.permissions.permission_checker_factory import PermissionCheckerFactory
from testcharmvision.forge.sdk.core.security import generate_testcharmvision_signature
from testcharmvision.forge.sdk.db.enums import OrganizationAuthTokenType
from testcharmvision.forge.sdk.executor.factory import AsyncExecutorFactory
from testcharmvision.forge.sdk.models import Step
from testcharmvision.forge.sdk.routes.code_samples import (
...
from testcharmvision.forge.sdk.schemas.ai_suggestions import AISuggestionBase, AISuggestionRequest
from testcharmvision.forge.sdk.schemas.organizations import (
    GetOrganizationAPIKeysResponse,
    GetOrganizationsResponse,
    Organization,
    OrganizationUpdate,
)
from testcharmvision.forge.sdk.schemas.prompts import CreateFromPromptRequest
from testcharmvision.forge.sdk.schemas.task_generations import GenerateTaskRequest, TaskGeneration
from testcharmvision.forge.sdk.schemas.task_v2 import TaskV2Request
from testcharmvision.forge.sdk.schemas.tasks import (
    CreateTaskResponse,
    ModelsResponse,
    OrderBy,
    SortDirection,
...
from testcharmvision.forge.sdk.schemas.workflow_runs import WorkflowRunTimeline
from testcharmvision.forge.sdk.services import org_auth_service
from testcharmvision.forge.sdk.workflow.exceptions import (
    FailedToCreateWorkflow,
    FailedToUpdateWorkflow,
    InvalidTemplateWorkflowPermanentId,
    WorkflowDefinitionValidationException,
)
from testcharmvision.forge.sdk.workflow.models.workflow import (
    RunWorkflowResponse,
    Workflow,
    WorkflowRequestBody,
    WorkflowRun,
...
from testcharmvision.schemas.artifacts import EntityType, entity_type_to_param
from testcharmvision.schemas.folders import Folder, FolderCreate, FolderUpdate, UpdateWorkflowFolderRequest
from testcharmvision.schemas.runs import (
    CUA_ENGINES,
    BlockRunRequest,
    BlockRunResponse,
    RunEngine,
...
from testcharmvision.schemas.webhooks import RetryRunWebhookRequest
from testcharmvision.schemas.workflows import (
    BlockType,
...
from testcharmvision.services import block_service, run_service, task_v1_service, task_v2_service, workflow_service
from testcharmvision.services.pdf_import_service import pdf_import_service
from testcharmvision.webeye.actions.actions import Action
...
async def run_task(
    ...
    analytics.capture("testcharmvision-oss-run-task", data={"url": run_request.url})
    ...
    if run_request.engine in CUA_ENGINES or run_request.engine == RunEngine.testcharmvision_v1:
        # create task v1
        ...
        return TaskRunResponse(
            ...
            app_url=f"{settings.TESTCHARMVISION_APP_URL.rstrip('/')}/runs/{task_v1_response.task_id}",
            run_request=TaskRunRequest(
                engine=run_request.engine,
    ...
    if run_request.engine == RunEngine.testcharmvision_v2:
        # create task v2
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Testcharmvision LLM failure to initialize task v2. Please try again later."
            )
        if settings.OTEL_ENABLED:
            span = trace.get_current_span()
            if span:
        ...
        return TaskRunResponse(
            ...
            app_url=f"{settings.TESTCHARMVISION_APP_URL.rstrip('/')}/runs/{task_v2.workflow_run_id}",
            run_request=TaskRunRequest(
                engine=RunEngine.testcharmvision_v2,
...
async def run_workflow(
    ...
    analytics.capture("testcharmvision-oss-run-workflow")
    ...
    return WorkflowRunResponse(
        ...
        recording_url=None,
        app_url=f"{settings.TESTCHARMVISION_APP_URL.rstrip('/')}/runs/{workflow_run.workflow_run_id}",
...
async def cancel_run(
    ...
    analytics.capture("testcharmvision-oss-agent-cancel-run")
...
async def create_workflow_legacy(
    ...
    analytics.capture("testcharmvision-oss-agent-workflow-create-legacy")
...
async def create_workflow(
    ...
    analytics.capture("testcharmvision-oss-agent-workflow-create")
    try:
...
async def convert_sop_to_blocks(
    ...
    analytics.capture(
        "testcharmvision-oss-workflow-sop-to-blocks",
        ...
        "x-fern-examples": [
            {
                "code-samples": [
                    {
                        "sdk": "curl",
                        "code": 'curl -X POST "https://api.testcharmvision.com/workflows/import-pdf" \\\n  -H "Authorization: Bearer YOUR_API_KEY" \\\n  -F "file=@sop_document.pdf"',
                    }
                ]
            }
        ],
...
async def import_workflow_from_pdf(
    ...
    analytics.capture("testcharmvision-oss-workflow-import-pdf")
...
async def update_workflow_legacy(
    ...
    analytics.capture("testcharmvision-oss-agent-workflow-update")
    ...
    try:
        ...
        raise e
    except (TestcharmvisionHTTPException, ValidationError) as e:
...
async def delete_workflow(
    ...
    analytics.capture("testcharmvision-oss-agent-workflow-delete")
...
async def create_folder(
    ...
    analytics.capture("testcharmvision-oss-folder-create")
...
async def delete_folder(
    ...
    analytics.capture("testcharmvision-oss-folder-delete")
...
async def get_artifact(
    ...
    analytics.capture("testcharmvision-oss-artifact-get")
...
async def get_run_artifacts(
    ...
    analytics.capture("testcharmvision-oss-run-artifacts-get")
...
async def retry_run_webhook(
    ...
    analytics.capture("testcharmvision-oss-agent-run-retry-webhook")
...
async def get_run_timeline(
    ...
    analytics.capture("testcharmvision-oss-run-timeline-get")
...
async def webhook(
    request: Request,
    x_testcharmvision_signature: Annotated[str | None, Header()] = None,
    x_testcharmvision_timestamp: Annotated[str | None, Header()] = None,
) -> Response:
    analytics.capture("testcharmvision-oss-agent-webhook-received")
    payload = await request.body()

    if not x_testcharmvision_signature or not x_testcharmvision_timestamp:
        LOG.error(
            "Webhook signature or timestamp missing",
            x_testcharmvision_signature=x_testcharmvision_signature,
            x_testcharmvision_timestamp=x_testcharmvision_timestamp,
            payload=payload,
        )
        raise HTTPException(
            status_code=http_status.HTTP_400_BAD_REQUEST,
    ...
    generated_signature = generate_testcharmvision_signature(
        payload.decode("utf-8"),
        settings.TESTCHARMVISION_API_KEY,
    )
    ...
    LOG.info(
        ...
        valid_signature=x_testcharmvision_signature == generated_signature,
    )
...
async def heartbeat() -> Response:
    ...
    """
    return Response(content="Server is running.", status_code=200, headers={"X-Testcharmvision-API-Version": __version__})
...
async def run_task_v1(
    ...
    analytics.capture("testcharmvision-oss-agent-task-create", data={"url": task.url})
...
async def get_task_v1(
    ...
    analytics.capture("testcharmvision-oss-agent-task-get")
...
async def retry_webhook(
    ...
    analytics.capture("testcharmvision-oss-agent-task-retry-webhook")
    task_obj = await app.DATABASE.get_task(task_id, organization_id=current_org.organization_id)
    if not task_obj:
        raise HTTPException(
            status_code=http_status.HTTP_404_NOT_FOUND,
...
async def get_tasks(
    ...
    """
    analytics.capture("testcharmvision-oss-agent-tasks-get")
...
async def get_runs(
    ...
    analytics.capture("testcharmvision-oss-agent-runs-get")
...
async def get_steps(
    ...
    """
    analytics.capture("testcharmvision-oss-agent-task-steps-get")
...
async def get_artifacts(
    ...
    analytics.capture("testcharmvision-oss-agent-entity-artifacts-get")
    params = {
...
async def get_step_artifacts(
    ...
    """
    analytics.capture("testcharmvision-oss-agent-task-step-artifacts-get")
...
async def get_actions(
    ...
    analytics.capture("testcharmvision-oss-agent-task-actions-get")
...
async def run_workflow_legacy(
    ...
    analytics.capture("testcharmvision-oss-agent-workflow-execute")
    context = testcharmvision_context.ensure_context()
    request_id = context.request_id
    await PermissionCheckerFactory.get_instance().check(
        current_org,
...
async def get_workflow_runs(
    ...
    analytics.capture("testcharmvision-oss-agent-workflow-runs-get")
...
async def get_workflow_run_with_workflow_id(
    ...
    analytics.capture("testcharmvision-oss-agent-workflow-run-get")
...
async def get_workflows(
    ...
    """
    analytics.capture("testcharmvision-oss-agent-workflows-get")
...
async def get_workflow_versions(
    ...
    """
    analytics.capture("testcharmvision-oss-agent-workflow-versions-get")
    if template:
        if workflow_permanent_id not in await app.STORAGE.retrieve_global_workflows():
            raise InvalidTemplateWorkflowPermanentId(workflow_permanent_id=workflow_permanent_id)
...
async def generate_task(
    ...
    analytics.capture("testcharmvision-oss-agent-generate-task")
...
async def run_task_v2(
    ...
    analytics.capture("testcharmvision-oss-agent-task-v2", data={"url": task_v2.url})
    await AsyncExecutorFactory.get_executor().execute_task_v2(
        request=request,
        background_tasks=background_tasks,
        organization_id=organization.organization_id,
