...
from skyvern.webeye.vnc_streaming import build_vnc_streaming_url
...
async def loop_stream_vnc(vnc_channel: VncChannel) -> None:
    ...
    vnc_url = build_vnc_streaming_url(
        browser_address=browser_session.browser_address,
        ip_address=browser_session.ip_address,
        vnc_port=vnc_channel.vnc_port,
    )
    if not vnc_url:
        raise Exception(f"{class_name} Browser session does not expose a VNC endpoint.")
