...
from testcharmvision.forge.sdk.schemas.task_v2 import TaskV2, TaskV2Metadata, TaskV2Status, ThoughtScenario, ThoughtType
from testcharmvision.forge.sdk.schemas.workflow_runs import WorkflowRunTimeline, WorkflowRunTimelineType
...
from testcharmvision.forge.sdk.workflow.models.parameter import PARAMETER_TYPE, ContextParameter
from testcharmvision.forge.sdk.workflow.models.workflow import Workflow, WorkflowRequestBody, WorkflowRun, WorkflowRunStatus
from testcharmvision.schemas.runs import ProxyLocation, ProxyLocationInput, RunEngine, RunType, TaskRunRequest, TaskRunResponse
...
from testcharmvision.utils.prompt_engine import load_prompt_with_elements
...
async def _summarize_max_steps_failure_reason(
    ...
    try:
        ...
        screenshots = await TestcharmvisionFrame.take_split_screenshots(page=page, url=str(task_v2.url), draw_boxes=False)
...
async def initialize_task_v2(
    ...
    try:
        ...
        task_v2 = await mark_task_v2_as_failed(
            task_v2_id=task_v2.observer_cruise_id,
            workflow_run_id=task_v2.workflow_run_id,
            failure_reason="Testcharmvision failed to setup the workflow run",
            organization_id=organization.organization_id,
        )
        raise
    ...
    try:
        ...
        task_v2 = await mark_task_v2_as_failed(
            task_v2_id=task_v2.observer_cruise_id,
            workflow_run_id=workflow_run.workflow_run_id,
            failure_reason="Testcharmvision failed to update the task 2.0 after initializing the workflow run",
            organization_id=organization.organization_id,
        )
        raise
...
async def run_task_v2_helper(
    ...
    page = await browser_state.get_working_page()
    if page:
        current_url = await TestcharmvisionFrame.get_url(page)
    ...
    for i in range(DEFAULT_MAX_ITERATIONS):
        ...
        if i == 0 and current_url != url:
            ...
            try:
                ...
                continue
            current_url = current_url if current_url else str(await TestcharmvisionFrame.get_url(frame=page) if page else url)
            ...
            if not task_type:
                ...
                task_v2 = await mark_task_v2_as_failed(
                    task_v2_id=task_v2_id,
                    workflow_run_id=workflow_run_id,
                    failure_reason="Testcharmvision failed to generate a task. Please try again later.",
                )
                break
...
async def mark_task_v2_as_failed(
    ...
    if workflow_run_id:
        await app.WORKFLOW_SERVICE.mark_workflow_run_as_failed(
            workflow_run_id, failure_reason=failure_reason or "Testcharmvision task 2.0 failed"
        )
...
async def build_task_v2_run_response(task_v2: TaskV2) -> TaskRunResponse:
    ...
    from testcharmvision.services import workflow_service  # noqa: PLC0415
...
async def send_task_v2_webhook(task_v2: TaskV2) -> None:
    ...
    try:
        ...
        signed_data = generate_testcharmvision_webhook_signature(payload=payload_dict, api_key=api_key.token)
