...
@pytest.mark.asyncio
async def test_handle_action_saves_download_event_to_active_run_directory(span_exporter: InMemorySpanExporter) -> None:
...
@pytest.mark.asyncio
async def test_handle_action_download_event_completion_timeout_propagates() -> None:
    now = datetime.now(UTC)
    organization = make_organization(now)
    task = make_task(
        now,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        mock_app.DATABASE.workflow_params.create_action = AsyncMock(return_value=action)
        mock_app.STORAGE = MagicMock()
        wait_for_downloads = AsyncMock(side_effect=asyncio.TimeoutError)
