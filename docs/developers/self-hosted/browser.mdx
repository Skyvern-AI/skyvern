...
`chrome://inspect/#remote-debugging` can be used only when it opens a
Docker-reachable listener. On Windows, check the listener before using that
path:

```powershell Windows
Get-NetTCPConnection -LocalPort 9222 | Select-Object LocalAddress, State
```

If the listener shows only `127.0.0.1`, Docker Desktop cannot reach it through
`host.docker.internal`; use the manual launch command above instead. Continue
only when the listener is `0.0.0.0` or `::`.
