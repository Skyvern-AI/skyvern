...
For when to use tasks vs agents, see [Run a Task](/running-automations/run-a-task).
...
| `engine` | `RunEngine` | No | `skyvern_v1` / `"skyvern-1.0"` | AI engine. Python enum options: `skyvern_v1`, `skyvern_v2`, `openai_cua`, `anthropic_cua`, `ui_tars`. TypeScript uses the wire values: `"skyvern-1.0"`, `"skyvern-2.0"`, `"openai-cua"`, `"anthropic-cua"`, `"ui-tars"`. The `openai_cua` and `anthropic_cua` engines are enterprise-only; non-enterprise callers receive HTTP 403. |
...
| `publish_workflow` | `bool` | No | `False` | Deprecated. Routes the request through the legacy Skyvern 2.0 publish path for backwards compatibility; prefer creating workflows with the workflow APIs. |
...
| `model` | `dict` | No | `None` | Override the LLM used for the run (e.g. `{"model_name": "<model>"}`). Anthropic Claude Opus 4.x models (`claude-opus-4-5-20251101`, `claude-opus-4-6`, `claude-opus-4-7`, `claude-opus-4-8`, `us.anthropic.claude-opus-4-20250514-v1:0`) are enterprise-only; non-enterprise callers receive HTTP 403. |
