...
Access geo-restricted websites and avoid bot detection with Skyvern's global network of residential proxies. 
...
For browser sessions, set `proxy_location` at creation—all tasks in that session use the same proxy:
