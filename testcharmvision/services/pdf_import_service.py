...
from testcharmvision.forge.sdk.utils.pdf_parser import extract_pdf_file
from testcharmvision.schemas.workflows import WorkflowCreateYAMLRequest
...
class PDFImportService:
    ...
    def _sanitize_workflow_json(raw: dict[str, Any]) -> dict[str, Any]:
        """Clean LLM JSON to match Testcharmvision schema conventions and avoid Jinja errors.
        ...
        for blk in blocks:
            ...
            if blk.get("block_type") in {"navigation", "action", "extraction", "login", "file_download"}:
                blk.setdefault("engine", "testcharmvision-1.0")
