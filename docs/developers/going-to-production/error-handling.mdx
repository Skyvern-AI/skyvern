...
description: Detect, classify, and recover from task and agent failures using run status, failure_reason, custom error_code_mapping, and validation blocks. Build programmatic error handling with retry logic and branching.
...
`output` is not always `null` on failure. Runs that end in `failed` or `terminated` can include data extracted before the run stopped. Check `status` to decide whether the run succeeded, not `output`.
