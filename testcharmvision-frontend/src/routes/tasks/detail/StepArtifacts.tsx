...
function StepArtifacts({ id, stepProps }: Props) {
  ...
  const testcharmvisionLog = artifacts?.filter(
    (artifact) => artifact.artifact_type === ArtifactType.TestcharmvisionLog,
  );
    ...
    >
      <TabsList className="grid h-16 w-full grid-cols-5">
        ...
        {enable_log_artifacts && (
          <TabsTrigger value="testcharmvision_log">Testcharmvision Log</TabsTrigger>
        )}
      </TabsList>
      ...
      {enable_log_artifacts && (
        <TabsContent value="testcharmvision_log">
          {testcharmvisionLog ? <Artifact type="text" artifacts={testcharmvisionLog} /> : null}
        </TabsContent>
      )}
    </Tabs>
  );
