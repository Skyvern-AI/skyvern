...
from testcharmvision.exceptions import UnknownElementTreeFormat
...
def json_to_html(element: dict, need_testcharmvision_attrs: bool = True) -> str:
    """
    ...
    # build children HTML
    children_html = "".join(
        json_to_html(child, need_testcharmvision_attrs=need_testcharmvision_attrs) for child in element.get("children", [])
    )
    # build option HTML
    option_html = "".join(
...
class ScrapedPage(BaseModel, ElementTreeBuilder):
    ...
    def build_element_tree(
        ...
        if fmt == ElementTreeFormat.HTML:
            return "".join(
                json_to_html(element, need_testcharmvision_attrs=html_need_testcharmvision_attrs)
