...
| `browser_address` | `str` | No | `None` | Connect to a browser at this CDP address instead of spinning up a new one. Must be an `http`, `https`, `ws`, or `wss` URL with a resolvable host that passes network policy. Invalid or blocked addresses return a 422 validation error. |
