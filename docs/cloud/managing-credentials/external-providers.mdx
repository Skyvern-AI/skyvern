...
description: Connect external credential vaults like Bitwarden, 1Password, Azure Key Vault, or a custom webhook API to the Skyvern Cloud UI so agents can pull secrets at runtime.
...
Skyvern Cloud can use an instance-wide 1Password service account for organizations in the deployment allowlist.
...
For password credentials, you can also set the optional **1Password TOTP Field**. Set it to the id or label of the item field that holds the TOTP secret. Use this when the OTP is stored in a plain custom field (for example, one named "digits") rather than 1Password's native one-time password field type. Leave it empty to auto-detect.
