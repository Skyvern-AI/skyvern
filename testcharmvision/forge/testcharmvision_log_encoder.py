...
class TestcharmvisionLogEncoder:
    """Encodes Testcharmvision logs from JSON format to human-readable string format"""
    ...
    def _format_value(cls, value: Any) -> str:
        return TestcharmvisionJSONLogEncoder.dumps(value, sort_keys=True)
    ...
    def _parse_json_entry(entry: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in entry.items():
            if key not in ("timestamp", "level", "event", "message"):
                clean_entry[key] = TestcharmvisionLogEncoder._format_value(value)
