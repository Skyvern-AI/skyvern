...
import { parameterIsTestcharmvisionCredential } from "../../types";
...
function ParametersMultiSelect({
  ...
  // Fetch credentials to check for orphaned Testcharmvision credential parameters
  ...
  // Build options with warning labels for orphaned Testcharmvision credential parameters
  const options = useMemo(() => {
    return keys.map((key) => {
      const param = workflowParameters.find((p) => p.key === key);

      // Check if this is an orphaned Testcharmvision credential parameter
      const isOrphanedCredential =
        isSuccess &&
        param &&
        param.parameterType === "credential" &&
        parameterIsTestcharmvisionCredential(param) &&
