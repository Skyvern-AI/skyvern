...
from testcharmvision.client import Testcharmvision
...
workflow_app = typer.Typer(help="Manage Testcharmvision workflows.")
...
def workflow_callback(
    ...
    api_key: str | None = typer.Option(
        None,
        "--api-key",
        help="Testcharmvision API key",
        envvar="TESTCHARMVISION_API_KEY",
    ),
) -> None:
...
def _get_client(api_key: str | None = None) -> Testcharmvision:
    """Instantiate a Testcharmvision SDK client using environment variables."""
    load_dotenv(resolve_backend_env_path())
    key = api_key or os.getenv("TESTCHARMVISION_API_KEY") or settings.TESTCHARMVISION_API_KEY
    return Testcharmvision(base_url=settings.TESTCHARMVISION_BASE_URL, api_key=key)
