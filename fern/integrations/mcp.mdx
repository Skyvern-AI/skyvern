...
**One-click install** (Cursor 0.43+, OAuth — no API key required):
...
Click the link above (or copy-paste it into your browser's address bar). Cursor opens and prompts you to authenticate via browser sign-in.
...
**OAuth (recommended, no API key required):**

```bash
codex mcp add skyvern --url https://api.skyvern.com/mcp/
```

Codex opens your browser for Skyvern sign-in and stores the token automatically. Requires `codex --version` ≥ 0.125.0.

**Manual setup with API key** (headless or CI environments):
...
1. In ChatGPT, open **Settings → Connectors** (look under **Integrations** or **Beta features** if Connectors is not visible).
...
5. Once connected, Skyvern tools appear in the tool panel for navigating pages, filling forms, extracting data, and running workflows.

No API key is required — OAuth handles authentication.
