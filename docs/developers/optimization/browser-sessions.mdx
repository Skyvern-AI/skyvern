...
subtitle: Persist live browser state across multiple tasks and agents
description: Create and manage live browser sessions that persist cookies, local storage, and page state between task or agent runs. Use sessions for back-to-back tasks, human-in-the-loop approval, and real-time agents.
...
|-----------|------|-------------|
| `timeout` | integer | Session lifetime in minutes. Min: 5. Max: 240 (4 hours) — requests above 240 are capped at 240 with a warning. Default: `60`. A running session can be [extended](#extend-a-session) up to 360 minutes (6 hours). |
...
| `browser_type` | string | Browser type: `chrome` or `msedge`. If omitted, Skyvern picks a managed default that may be a hardened stealth Chromium build rather than Chrome. Set it explicitly to select the engine when no `browser_profile_id` is used — a supplied profile's source browser type takes precedence. See [Choose the right browser type](#choose-the-right-browser-type) |
...
| `needs_live_view` | boolean | Declare that a person will watch this session's browser live. Default: `false`, which suits unattended automation. This requests a capability (the session runs somewhere that can serve a live view stream) — it does not select where the session runs. The Skyvern app sets this because a session opened in the UI is watched |
...
<Warning>
Sessions close automatically when the timeout expires, **even if a task is still running**. The timeout countdown begins when the browser launches. Set timeouts with enough margin for your longest expected task, or [extend the session](#extend-a-session) once you know how much work is queued.
</Warning>
...
A session is created with at most 240 minutes, but a running session can be extended, one call at a time, up to a total lifetime of 360 minutes (6 hours) counted from when it started. Extension is additive: each call adds `additional_minutes` to the session's current deadline. Ask for more than what remains and the session is extended by the remainder, with a `warning` on the response explaining the difference.

<CodeGroup>
```python Python
session = await skyvern.extend_browser_session(
    browser_session_id="pbs_490705123456789012",
    additional_minutes=60,
)
print(session.timeout)  # the session's new total budget in minutes, counted from started_at
```

```bash cURL
curl -X POST https://api.skyvern.com/v1/browser_sessions/pbs_490705123456789012/extend \
...
The call returns `409` when the session has already ended, is under two minutes from its deadline, or is already at the 360-minute maximum. Read `timeout` from the response rather than tracking the total yourself; a retried request adds again. The TypeScript SDK gains `extendBrowserSession` in its next release; until then call the REST endpoint directly.
...
If you omit `browser_type`, Skyvern selects a managed default for your account — this may be a hardened stealth Chromium build rather than Google Chrome, so don't rely on the default being Chrome. Set `browser_type` explicitly when you need a specific engine. Note that if you load a session from a `browser_profile_id`, the profile's source browser type takes precedence over `browser_type`.

Each engine has a trade-off:

- **`chrome`** — Google Chrome. Can work better on some fingerprint-sensitive sites where the managed stealth default gets challenged, but does **not** support the CAPTCHA-solver extension. This varies by site — test for yours.
- **`msedge`** — Microsoft Edge. Supports the CAPTCHA-solver extension, so use it (or the managed stealth default) when a site serves hCaptcha/reCAPTCHA and you need automatic solving.

Site behavior varies: if a site both fingerprints aggressively and serves CAPTCHAs, test both engines to find the best fit.

<CodeGroup>
```python Python
# Chrome - can help on some fingerprint-sensitive sites; no CAPTCHA-solver extension
session = await client.create_browser_session(browser_type="chrome")

# Edge - supports the CAPTCHA-solver extension for hCaptcha/reCAPTCHA
...
Extensions add startup time, so only enable them when needed. The ad-blocker removes overlay ads that can interfere with automation. The captcha-solver handles CAPTCHAs automatically but is only available on Cloud, and it requires the `msedge` browser type (or the managed stealth default) — the `chrome` browser type does not support it.
