...
| `tags` | `list[str]` | No | `None` | Filter by tags as `key:value` pairs (e.g. `["env:prod"]` or `["customer:acme,env:prod"]`). AND across distinct keys, OR within a key. Matches current tag values only. Not supported with `template=true`. |
