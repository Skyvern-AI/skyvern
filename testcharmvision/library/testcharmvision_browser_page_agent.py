...
from testcharmvision.client import GetRunResponse, TestcharmvisionEnvironment, WorkflowRunResponse
from testcharmvision.client.core import RequestOptions
from testcharmvision.library.constants import DEFAULT_AGENT_HEARTBEAT_INTERVAL, DEFAULT_AGENT_TIMEOUT
from testcharmvision.schemas.run_blocks import CredentialType
from testcharmvision.schemas.runs import RunEngine, RunStatus, TaskRunResponse
...
def _get_app_url_for_run(run_id: str) -> str:
    return f"https://app.testcharmvision.com/runs/{run_id}"


class TestcharmvisionBrowserPageAgent:
    """Provides methods to run Testcharmvision tasks and workflows in the context of a browser page.
    ...
    def __init__(self, browser: "TestcharmvisionBrowser", page: Page) -> None:
    ...
    async def run_task(
        self,
        prompt: str,
        engine: RunEngine = RunEngine.testcharmvision_v2,
        ...
        Args:
            ...
            engine: The execution engine to use. Defaults to testcharmvision_v2.
        ...
        task_run = await self._browser.testcharmvision.run_task(
            prompt=prompt,
            engine=engine,
            model=model,
            ...
            request_options=RequestOptions(additional_headers={"X-User-Agent": "testcharmvision-sdk"}),
        )
        if self._browser.testcharmvision.environment == TestcharmvisionEnvironment.CLOUD:
    ...
    async def login(
        self,
        *,
        credential_type: Literal[CredentialType.testcharmvision] = CredentialType.testcharmvision,
        credential_id: str,
        url: str | None = None,
    ...
    async def login(
        self,
        *,
        credential_type: CredentialType = CredentialType.testcharmvision,
        url: str | None = None,
        ...
        1. Testcharmvision credentials (default):
            ```python
        ...
        Args:
            credential_id: ID of the Testcharmvision credential to use (required for testcharmvision credential_type).
            credential_type: Type of credential store to use. Defaults to CredentialType.testcharmvision.
        ...
        workflow_run = await self._browser.testcharmvision.login(
    ...
    async def download_files(
        ...
        *,
        ...
        workflow_run = await self._browser.testcharmvision.download_files(
    ...
    async def run_workflow(
        ...
        workflow_run = await self._browser.testcharmvision.run_workflow(
    ...
    async def _wait_for_run_completion(self, run_id: str, timeout: float) -> GetRunResponse:
        async with asyncio.timeout(timeout):
            while True:
                task_run = await self._browser.testcharmvision.get_run(run_id)
