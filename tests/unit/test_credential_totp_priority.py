...
class TestPayloadVerificationCodePriority:
    """Tests for payload-provided verification code precedence."""

    @pytest.mark.asyncio
    async def test_agent_prefers_payload_code_over_credential_totp(self) -> None:
        ...
        with (
            ...
            patch(
                "skyvern.forge.agent.poll_otp_value",
                new_callable=AsyncMock,
                return_value=OTPValue(value="111111", type=OTPType.TOTP),
            ) as mock_poll,
            ...
            task.organization_id = "org_123"
            task.task_id = "task_456"
            task.workflow_run_id = "wfr_123"
    ...
    @pytest.mark.asyncio
    async def test_agent_ignores_navigation_goal_code_and_uses_credential_totp(self) -> None:
        """Navigation goal text should not override credential TOTP when payload has no code."""
        with (
            patch(
                "skyvern.forge.agent.try_generate_totp_from_credential",
                return_value=OTPValue(value="999999", type=OTPType.TOTP),
            ...
            task.navigation_goal = "Sign in and use the 2FA code 520265 when asked."
            task.organization_id = "org_123"
            task.task_id = "task_456"
            task.workflow_run_id = "wfr_123"
            task.totp_verification_url = None
            ...
            mock_credential_totp.assert_called_once()
            mock_poll.assert_not_called()
            assert mock_context.totp_codes["task_456"] == "999999"

    @pytest.mark.asyncio
    async def test_agent_ignores_navigation_goal_input_step_code_and_uses_credential_totp(self) -> None:
        """Input-step text in navigation goal should be ignored for inline OTP extraction."""
        with (
            patch(
                "skyvern.forge.agent.try_generate_totp_from_credential",
                return_value=OTPValue(value="999999", type=OTPType.TOTP),
            ...
            task.navigation_goal = (
                "GOAL: Submit MFA code\n\n"
                "2. SETUP (Conditional):\n"
                '- IF [input Hidden] AND ["Verify" Button Visible]: Click ONCE. Wait.\n'
                "- IF [input Missing]: TERMINATE.\n\n"
                "3. ACTION (Execute Once):\n"
                "- Close blocking popups.\n"
                "- Input 520265.\n"
                '- Click "Submit" or "Next" ONCE.\n'
            )
            task.organization_id = "org_123"
            task.task_id = "task_456"
            task.workflow_run_id = "wfr_123"
    ...
    @pytest.mark.asyncio
    async def test_cua_fallback_uses_payload_code_and_skips_poll(self) -> None:
        """CUA get_verification_code should use payload code directly when available."""
        mock_task = MagicMock()
        mock_task.totp_verification_url = None
        mock_task.totp_identifier = None
        mock_task.organization_id = "org_123"
        ...
        mock_task.workflow_run_id = None
        mock_task.navigation_goal = "test goal"
        mock_task.navigation_payload = {"mfaChoice": "520265"}
        mock_task.data_extraction_goal = None
        ...
        with (
            ...
            patch(
                "skyvern.webeye.actions.parse_actions.poll_otp_value",
                new_callable=AsyncMock,
                return_value=OTPValue(value="123456", type=OTPType.TOTP),
            ) as mock_poll,
            patch("skyvern.webeye.actions.parse_actions.app") as mock_app,
            patch("skyvern.webeye.actions.parse_actions.prompt_engine") as mock_prompt_engine,
        ):
            ...
            assert len(actions) == 1
            assert isinstance(actions[0], VerificationCodeAction)
            assert actions[0].verification_code == "520265"

    @pytest.mark.asyncio
    async def test_cua_fallback_ignores_navigation_goal_input_step_code_and_uses_credential_totp(
        self,
    ) -> None:
        """CUA fallback should ignore navigation goal input-step code when payload is missing."""
        mock_task = MagicMock()
        mock_task.totp_verification_url = None
        mock_task.totp_identifier = None
        mock_task.organization_id = "org_123"
        ...
        with (
            ...
            assert len(actions) == 1
            assert isinstance(actions[0], VerificationCodeAction)
            assert actions[0].verification_code == "999999"

    @pytest.mark.asyncio
    async def test_cua_fallback_ignores_navigation_goal_code_and_uses_credential_totp(self) -> None:
        """CUA fallback should ignore plain navigation goal code when payload is missing."""
        mock_task = MagicMock()
        mock_task.totp_verification_url = None
        mock_task.totp_identifier = None
        mock_task.organization_id = "org_123"
        mock_task.task_id = "task_456"
        mock_task.workflow_run_id = None
        mock_task.navigation_goal = "Login flow: enter your verification code 520265 when prompted."
        mock_task.navigation_payload = None
        mock_task.data_extraction_goal = None
