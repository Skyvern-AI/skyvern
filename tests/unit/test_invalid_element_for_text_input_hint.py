...
def _make_skyvern_element(tag_name: str, *, editable: bool, attributes: dict | None = None) -> SkyvernElement:
    locator = MagicMock()
    ...
    return SkyvernElement(
        locator=locator,
        frame=MagicMock(),
        static_element={"tagName": tag_name, "attributes": attributes or {}},
    )


@pytest.mark.asyncio
async def test_iframe_does_not_support_text_input() -> None:
    element = _make_skyvern_element("iframe", editable=False)
    assert await element.supports_text_input() is False


@pytest.mark.asyncio
@pytest.mark.parametrize("tag_name", ["input", "textarea", "select", "INPUT"])
async def test_form_controls_support_text_input(tag_name: str) -> None:
    element = _make_skyvern_element(tag_name, editable=False)
    assert await element.supports_text_input() is True


@pytest.mark.asyncio
async def test_editable_element_supports_text_input() -> None:
    element = _make_skyvern_element("div", editable=True)
    assert await element.supports_text_input() is True
...
@pytest.mark.parametrize(
    "class_name",
    ["custom blinking-cursor", "custom Blinking-Cursor"],
)
async def test_blinking_cursor_element_supports_text_input(class_name: str) -> None:
    element = _make_skyvern_element("div", editable=False, attributes={"class": class_name})
    assert await element.supports_text_input() is True
...
async def test_contenteditable_supports_text_input(value: str) -> None:
    element = _make_skyvern_element("div", editable=False, attributes={"contenteditable": value})
    assert await element.supports_text_input() is True


@pytest.mark.asyncio
async def test_contenteditable_false_does_not_support_text_input() -> None:
    element = _make_skyvern_element("div", editable=False, attributes={"contenteditable": "false"})
    assert await element.supports_text_input() is False
...
def _mock_iframe_element(*, hidden: bool = False) -> MagicMock:
    el = MagicMock()
...
@pytest.mark.asyncio
async def test_input_text_rejects_iframe_before_clear_or_fill() -> None:
    el = _mock_iframe_element()
    dom_instance = MagicMock()
    dom_instance.get_skyvern_element_by_id = AsyncMock(return_value=el)
...
@pytest.mark.asyncio
async def test_hidden_iframe_keeps_non_blocking_invisible_failure() -> None:
    el = _mock_iframe_element(hidden=True)
    dom_instance = MagicMock()
    dom_instance.get_skyvern_element_by_id = AsyncMock(return_value=el)
