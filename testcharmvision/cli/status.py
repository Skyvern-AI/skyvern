...
status_app = typer.Typer(help="Check status of Testcharmvision components.", invoke_without_command=True)
...
def _status_table() -> Table:
    ...
    components = [
        ("API server", _check_port(api_port), "testcharmvision run server"),
        ("UI server", _check_port(ui_port), "testcharmvision run ui"),
        ("PostgreSQL", _check_port(db_port), "testcharmvision init --no-postgres false"),
    ]

    table = Table(title="Testcharmvision Component Status")
