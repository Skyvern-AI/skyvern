...
4. Choose **Skyvern Managed** (credentials created via the API) or a direct vault integration
5. Link the parameter to a Login block

<Note>
**Skyvern Managed** credentials are created via `create_credential()` and use your configured vault (Bitwarden by default). Direct vault integrations let you reference existing vault items without importing them into Skyvern.
</Note>
