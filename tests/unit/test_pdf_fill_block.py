...
from skyvern.schemas.workflows import BlockStatus, BlockType, PdfFillBlockYAML, WorkflowDefinitionYAML
...
def test_pdf_fill_yaml_rejects_raw_custom_llm_key() -> None:
    yaml_block = PdfFillBlockYAML(
        label="fill_pdf",
        file_url="source.pdf",
        prompt="Fill using payload",
...
class FakePdfFillContext(FakeWorkflowRunContext):
...
def _install_fake_context(monkeypatch: pytest.MonkeyPatch, values: dict[str, Any]) -> FakePdfFillContext:
    context = FakePdfFillContext(values=values)
    monkeypatch.setattr(PdfFillBlock, "get_workflow_run_context", staticmethod(lambda _wr_id: context))
    mock_app = MagicMock()
...
async def test_acroform_fill_end_to_end_through_converter(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    source_pdf = _run_local_pdf_path(monkeypatch, tmp_path, "wr_e2e_acroform", "form.pdf")
    _write_fillable_pdf(source_pdf)
    ...
    context = _install_fake_context(monkeypatch, values={"source_pdf": str(source_pdf)})
    monkeypatch.setattr(PdfFillBlock, "_resolve_default_llm_handler", AsyncMock(return_value=_llm_response))

    result = await block.execute(workflow_run_id="wr_e2e_acroform", workflow_run_block_id="", organization_id=None)
...
async def test_flat_fill_end_to_end_with_fake_tesseract(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    source_pdf = _run_local_pdf_path(monkeypatch, tmp_path, "wr_e2e_flat", "flat.pdf")
    ...
    result = await block.execute(workflow_run_id="wr_e2e_flat", workflow_run_block_id="", organization_id=None)
...
async def test_file_url_unwraps_upstream_block_output(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    source_pdf = _run_local_pdf_path(monkeypatch, tmp_path, "wr_e2e_chained", "form.pdf")
    _write_fillable_pdf(source_pdf)
    ...
    result = await block.execute(workflow_run_id="wr_e2e_chained", workflow_run_block_id="", organization_id=None)
